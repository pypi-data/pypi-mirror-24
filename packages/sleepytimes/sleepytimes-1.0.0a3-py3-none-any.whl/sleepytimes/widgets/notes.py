...
from .utils import (convert_name_to_color, short_strings, notification_sound, 
                    ICON)
...
class Notes(QTabWidget):
    ...
    def detect_spindles(self, method, params, label=None):
        """Detect spindles and display as events on signal.
        ...
        params : dict
            Parameters used for detection. Can have been modified by user.
        label : str
        ...
        dur = (params['min_dur'], params['max_dur'])
        
        detector = DetectSpindle(method=method, frequency=freq, duration=dur)
        ...
        spindles = detector(self.data)
        
        for one_sp in spindles:
            self.annot.add_event(label,(one_sp['start_time'], 
                                        one_sp['end_time']), 
                                        chan=one_sp['chan'])
        self.update_annotations()

    def analyze_events(self, event_type, chan, stage, params, frequency=None,
        ...
        frequency : tuple of float
            frequencies for filtering. Defaults to None.
        ...
        events = [self.annot.get_events(
                    name=event_type, chan=chan, stage=stage, 
                    qual='Good'
                    )]
        summary = []
        f1 = frequency[0]
...
class SpindleDialog(QDialog):
    ...
    idx_method : QComboBox
        Combo box of the spindle detection methods.    
    index : dict of FormFloat
        Contains detection parameters, to be passed to detect_spindle.
    """
    def __init__(self, parent):
    ...
    def create_dialog(self):
        ...
        mbox = QComboBox()
        method_list = ['Wamsley2012', 'Nir2011', 'Moelle2011', 
                       'Ferrarelli2007', 'UCSD']
    ...
    def button_clicked(self, button):
        ...
        if button is self.idx_ok:
            ...
            self.parent.notes.read_data(chans, self.one_grp, stage=stage, 
                                        quality='Good')
            self.parent.notes.detect_spindles(self.method, params,
                                              label=self.label.get_value())
            
            notification_sound('Detection complete')        
...
class EventAnalysisDialog(QDialog):
    ...
    def button_clicked(self, button):
        ...
        if button is self.idx_ok:
            ...
            notification_sound('Event analysis complete.')
            self.accept()
...
def _create_data_to_analyze(data, analysis_chans, chan_grp, times):
    ...
    lg.info('data1 type: ' + str(type(data1)))
