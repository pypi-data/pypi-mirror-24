...
class AnnualizedRiskCalc(BaseCalculator):
    """
    计算区间年化方差
    """
    ...
    def _calc(self, nav_list, profit_list, risk_free_profit_list, index_nav_list,
              index_profit_list):
        return AnnualizedRiskCalc.value_calc(profit_list, self.frequency)
    ...
    def check_data(nav_list, profit_list, from_date, to_date):
        if len(profit_list) > 1 and len(nav_list) == (to_date - from_date).days + 1:
            return True
        return False

    @staticmethod
    def value_calc(profit_list, frequency):
        return decimal.Decimal(numpy.sqrt(numpy.cov(numpy.asarray(profit_list, 'float')) * FrequencyDays.get_days(frequency))).quantize(
