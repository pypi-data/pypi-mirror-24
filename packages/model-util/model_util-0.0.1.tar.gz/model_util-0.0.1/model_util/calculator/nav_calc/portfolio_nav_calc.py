...
from caijiajia.model_util.constant.constant import TransferType
...
class PortfolioNavCalc(object):
    """
    计算组合净值
    """

    def __init__(self, nav_date, portfolio_code, portfolio_type, portfolio_last_nav, last_fund_vol, last_fund_amount,
                 last_fund_weight, cur_fund_nav, last_fund_nav, transfer_record, fund_rate, is_transfer=True):
        """
        :param nav_date: 计算净值的日期
        :param portfolio_code: 组合code
        :param portfolio_type: 组合类型
        :param portfolio_last_nav: 组合前一天的净值
        :param last_fund_vol: 前一天的基金份额
        :param last_fund_amount: 前一天的基金金额
        :param last_fund_weight: 前一天的基金比例
        :param cur_fund_nav: 当天基金净值
        :param last_fund_nav: 前一天基金净值
        :param transfer_record: 调仓记录
        :param fund_rate: 基金费率
        :param is_transfer: 是否是转换
        :return:
        """
    ...
    def calc(self, ori_fund_weight, calc_with_rate):
        """
        计算组合净值
        :param ori_fund_weight:
        :param calc_with_rate:
        :return:
        """
        if self.transfer_record is not None and self.transfer_record.transfer_type == TransferType.first:
            ...
            return self._calc_for_normal(ori_fund_weight, calc_with_rate)
    ...
    def _calc_for_first(self, ori_fund_weight):
        ...
        nav = 1
        market_weight = PortfolioNavCalc._to_market_weight(market_amount, self.market_fund_dict, ori_fund_weight)
        return self.portfolio_code, self.nav_date, self.portfolio_type, nav, json.dumps(fund_vol), json.dumps(market_weight), json.dumps(fund_amount)

    def _calc_for_normal(self, ori_fund_weight, calc_with_rate):
        """
        计算组合的净值, 非第一天
        :return:
        """
        ...
        if self.transfer_record is not None:
            if self.transfer_record.transfer_type == TransferType.faa:
                # 仅适用产品调仓
                fund_vol = PortfolioNavCalc._calc_transfer_for_faa(self.last_fund_weight, self.last_fund_amount, self.transfer_record.target_weight, self.last_fund_nav, self.last_fund_vol)
            else:
                # 市场比例变更
                target_fund_assets = PortfolioNavCalc._calc_target_fund_assets(self.portfolio_last_nav, self.transfer_record.target_weight)
                transfer_out, transfer_assets_record, no_changed_codes = PortfolioNavCalc._calc_transfer(self.last_fund_amount, target_fund_assets)
                fund_vol, transfer_detail = self._calc_transfer_detail(transfer_out, transfer_assets_record, no_changed_codes, calc_with_rate)
        ...
        fund_weight = PortfolioNavCalc._to_market_weight(market_amount, self.market_fund_dict, ori_fund_weight)

        if transfer_detail:
            transfer = (self.portfolio_code, self.nav_date, self.portfolio_type, JsonUtil.encode_json(self.last_fund_weight), JsonUtil.encode_json(self.last_fund_vol), JsonUtil.encode_json(self.last_fund_amount),
                        JsonUtil.encode_json(fund_weight), JsonUtil.encode_json(fund_vol), JsonUtil.encode_json(fund_amount))
        return (self.portfolio_code, self.nav_date, self.portfolio_type, nav, JsonUtil.encode_json(fund_vol), JsonUtil.encode_json(fund_weight), JsonUtil.encode_json(fund_amount)), transfer, transfer_detail
    ...
    def _calc_transfer_for_faa(last_fund_weight, last_fund_assets_dict, target_fund_weight_dict, last_fund_nav_dict, last_fund_vol_dict):
        """
        计算FAA变更, 产品变更后份额
        FAA类型的调仓，目前只限于市场占比不变,基金数量不变的调仓，只包括基金之间的替换，且替换后的基金占比，与替换之前的基金占比完全一致
        :param last_fund_weight:
        :param last_fund_assets_dict:
        :param target_fund_weight_dict:
        :param last_fund_nav_dict:
        :param last_fund_vol_dict:
        :return:
        """
        ...
        for market_code, fund_weight_dict in target_fund_weight_dict.iteritems():
            if sum(fund_weight_dict.values()) == 0:
                continue
            # 市场下产品按比例从大到小排序, 依次替换
            if market_code not in last_fund_weight:
                logging.warn('Can not find %s in last fund weight!' % market_code)
            no_change_codes = filter(lambda e: e[0] not in fund_weight_dict, last_fund_weight[market_code].items())

            need_change_codes = filter(lambda e: e[0] not in fund_weight_dict, last_fund_weight[market_code].items())
            replace_codes = filter(lambda e: e[0] not in last_fund_weight[market_code], fund_weight_dict.items())

            if need_change_codes:
                ...
                if len(need_change_codes) != len(replace_codes):
                    ...
                    raise ImportError('The size of replaced fund is not equal with the size of origin funds!')

                for code, weight in no_change_codes:
                    target_fund_vol_dict[code] = last_fund_vol_dict[code]

                for i, code_weight in enumerate(replace_codes):
                    ori_code = need_change_codes[i][0]
                    adj_code = code_weight[0]
                    target_fund_vol_dict[adj_code] = round(last_fund_assets_dict[ori_code] / float(last_fund_nav_dict[ori_code]), Constant.VOL_NUM)
            else:
    ...
    def _calc_transfer_detail(self, transfer_out, transfer_record, no_changes_codes, use_rate):
        ...
        :return:
        """
        out_fund_vol, out_fund_fee, out_detail = PortfolioNavCalc._calc_out_fee(self.nav_date, self.portfolio_code, self.portfolio_type, transfer_out, self.last_fund_vol, self.last_fund_nav, self.cur_fund_nav, self.fund_rate, self.market_fund_dict, use_rate)
        in_fund_vol, in_detail = PortfolioNavCalc._calc_in_fee(self.nav_date, self.portfolio_code, self.portfolio_type, transfer_record, out_fund_fee, self.last_fund_vol, self.last_fund_nav, self.cur_fund_nav, self.fund_rate, self.market_fund_dict, use_rate, self.is_transfer)
    ...
    @staticmethod
    def _calc_in_fee(date, portfolio_code, portfolio_type, transfer_assets_record, transfer_out_fee, last_fund_vol, last_funds_nav, cur_funds_nav, fund_rate_dict, fund_market_dict, calc_with_rate, is_transfer=True):
        """
        计算转入后份额及费用
        ...
        for out_fund, in_fund, transfer_assets in transfer_assets_record:
            ...
            # print date, portfolio_code, out_fund, in_fund, transfer_assets, in_fee, op_vol, actual_in_assets, ideal_in_assets, target_vol
        ...
        new_transfer_program_detail = PortfolioNavCalc.merge_in_fund(transfer_program_detail)
