...
class Book(AMaaSModel):
    ...
    def utc_book_close(self):
        """
        The book close time in utc.
        """
        tz = pytz.timezone(self.timezone)
        close_time = tz.localize(datetime.datetime.combine(datetime.datetime.now(tz), datetime.datetime.min.time()) + self.close_time)
        return str(close_time.astimezone(pytz.utc).time())