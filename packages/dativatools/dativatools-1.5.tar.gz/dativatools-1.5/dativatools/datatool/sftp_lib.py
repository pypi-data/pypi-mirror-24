...
from dativatools.commonutilitytool import pysftp
from dativatools.datatool.file_transfer import FileTransfer
...
class SFTPLib(FileTransfer):
    ...
    def connect(self, settings):
        try:
            if settings["password_optional"] is not None:
                self.logger_obj.info("Setting up sftp connection")
                sftp_conn = pysftp.Connection(host=settings["remote_host"], username=settings["user"], password=settings["password_optional"])
                self.logger_obj.info("Connection established")
                return True, sftp_conn

            else:
                if settings["private_key_optional"] is not None and settings["is_protected_optional"] == "yes":
                    self.logger_obj.info("Setting up sftp connection")
                    sftp_conn = pysftp.Connection(host=settings["remote_host"], username=settings["user"], private_key=settings["private_key_optional"], private_key_pass=settings["key_phrase_optional"])
                    self.logger_obj.info("Connection established")
                    return True, sftp_conn

                else:
                    self.logger_obj.info("Setting up sftp connection")
                    sftp_conn = pysftp.Connection(host=settings["remote_host"], username=settings["user"], private_key=settings["private_key_optional"])
                    self.logger_obj.info("Connection established")
                    return True, sftp_conn
        ...
        try:
            ...
            if settings["put"]["recursive"] == "no":
                if os.path.isfile(settings["put"]["source_path"]):
                    sftp_conn.put(localpath=settings["put"]["source_path"], remotepath=settings["put"]["destination_path"], preserve_mtime=True)
                else:
                    files_list = [each for each in glob.glob(os.path.join(settings["put"]["source_path"], "*")) if os.path.isfile(each)]
                    for each in files_list:
                        sftp_conn.put(each, remotepath=settings["put"]["destination_path"], preserve_mtime=True)
            else:
