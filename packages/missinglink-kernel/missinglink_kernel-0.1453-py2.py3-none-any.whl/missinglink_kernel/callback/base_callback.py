...
class LoggerWrapper(object):
    ...
    def _create_remote_logger(self, session_id, endpoint, remote_log_level, remote_log_filter):
        self.info('remote_logger global level: %s filter: %s', remote_log_level, remote_log_filter)
        self.debug('__create_remote_logger %s %s %s %s', session_id, endpoint, remote_log_level, remote_log_filter)

        from .remote_logger import RemoteLoggerHandler

        return RemoteLoggerHandler(
            session_id, endpoint, remote_log_level, self.log_cache.log_records, remote_log_filter)

    def activate_remote_logging(self, session_id, endpoint, remote_log_level, remote_log_filter):
        self.remote_logger = self._create_remote_logger(session_id, endpoint, remote_log_level, remote_log_filter)
...
class BaseCallback(object):
    ...
    def _create_remote_logger_if_needed(self, create_experiment_result):
        ...
        if remote_logger is not None:
            ...
            self.logger.activate_remote_logging(session_id, endpoint, log_level, log_filter)
    ...
    @classmethod
    def _image_to_json(cls, current_img):
        return base64.b64encode(current_img).decode()

    @classmethod
    def _encode_images(cls, im_dict):
        encoded_images = defaultdict(list)
        for img in im_dict["images"]:
            if isinstance(img, six.string_types):
                ...
                encoded = cls._image_to_json(img)
            encoded_images[im_dict["image_key"]].append(encoded)
        im_dict["images"] = dict(encoded_images)

        return im_dict

    @classmethod
    def group_images(cls, image_objects):
        image_objects.sort(key=lambda x: x["meta"]["number"])
        for im_dict in image_objects:
            cls._encode_images(im_dict)

        return image_objects

    def upload_images(self, model_hash, images, meta, top_images=None):
