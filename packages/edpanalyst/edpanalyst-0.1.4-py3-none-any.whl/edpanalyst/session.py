...
class PopulationModel(object):
    ...
    def simulate(
        ...
        # Turn the given dict into a single-row column-major data set.
        req['given'] = {k: [v] for k, v in effective_givens.items()}
        if random_seed:
            req['random_seed'] = random_seed
        respjson = self._endpoint.simulate.post(json=req).json()
        assert 'rowids' not in respjson
        assert len(respjson) == 1
        return DataFrame(respjson[0]['columns'])

    def simulate_by_model(
            self,
            targets=None,  # type: List[str]
            given=None,  # type: Dict[str, Union[str, float, List[float]]]
            n=DEFAULT_LIMIT,  # type: int
        ...
        """Draw a sample of size `n` for each sub-model in a population model.

        Each returned row is a random draw from the posterior predictive. If
        there are `m` models in the ensemble, then the resulting data frame
        will be `n*m` rows long. There will be an additional column, `model`,
        containing an integer identifying which model that sample was drawn
        from.
        ...
        req = {'target': targets, 'n': n}  # type: Dict[str, Any]
        # Turn the given dict into a single-row column-major data set.
        req['given'] = {k: [v] for k, v in given.items()}
        if random_seed:
            req['random_seed'] = random_seed
        respjson = self._endpoint.simulate_by_model.post(json=req).json()
        # Stack the results, setting each data set's `model`. We only sent a
        # single row of givens, so we should only have one table per model.
        return pd.concat((DataFrame(d[0]['columns']).assign(model=i)
                          for i, d in enumerate(respjson)))

    def simulate_given_df(
            self,
            targets=None,  # type: List[str]
            given=None,  # type: DataFrame
            n=DEFAULT_LIMIT,  # type: int
            random_seed=None,  # type: int
    ):  # type: (...) -> DataFrame
        """Draw multiple samples of size `n` from the population model.

        Returns a data frame of length `n * len(given.index)` and a column for
        each column in `targets`, plus one additional column, `given_row`,
        corresponding to the row of `given` which that result row was
        conditioned on.
        ...
        Args:
            targets: The list of columns to simulate.
            given: Values to condition the simulation on. If a column is in
                both `target` and present in a row of `given`, that column will
                be the given value in every resulting row corresponding to that
                `given` row.
            n: The number of results to simulate.
            random_seed: optional random seed between 0 and 2**32
        """
        targets = self._columns_from_arg(targets)
        if given is None:
            raise ValueError('`given` must be specified')
        req = {'target': targets, 'n': n}  # type: Dict[str, Any]
        nulled_given = given.where(pd.notnull(given), None)
        req['given'] = nulled_given.to_dict(orient='list')
        if random_seed:
            req['random_seed'] = random_seed
        resp = self._endpoint.simulate.post(json=req)
        respjson = resp.json()
        if len(respjson) == 0:
            return DataFrame(columns=targets + ['given_row'])
        # Stack the results, setting each data set's `given_row`.
        return pd.concat((DataFrame(d['columns']).assign(given_row=i)
                          for i, d in enumerate(respjson)))
    ...
    def joint_probability_by_model(
            self,
            targets,  # type: Dict[str, List[Any]]
            given=None,  # type: Dict[str, Any]
            probability_column='p'  # type: str
    ):  # type: (...) -> DataFrame
        """The joint probability of a list of hypothetical rows by model.

        Returns a data frame similar to that returned by `joint_probability`,
        but with `len(targets) * num_models` rows, and an additional column,
        `model` which says which model in the ensemble the logpdf was computed
        from.

        The same table of targets and given are used in each model.
        """
        overlapping_cols = set(targets).intersection(given or [])
        if overlapping_cols:
            raise ValueError('%s in both `targets` and `given`' %
        ...
        # single row of givens, so we should only have one table per model.
        return pd.concat(
            (DataFrame(dict(targets, **{probability_column: _lp_to_prob(d)}))
