...
class PopulationModelExperimental(object):
    ...
    def logpdf_from_simulate_by_model(
        ...
        targets = [
            grouped.get_group(m).drop('model', 1).to_dict('list')
            for m in models
        ]
        req = {'targets': targets}
    ...
    def infer_plot(self, target, rowid=None,
                   given=None):  # type: (str, int, Dict[str, Any]) -> plt
        ...
        if rowid:
            ...
            given = self._popmod.select(rowids=[rowid]).to_dict(
                orient='records')[0]
        else:
