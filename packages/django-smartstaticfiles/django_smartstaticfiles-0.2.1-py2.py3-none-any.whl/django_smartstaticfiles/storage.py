...
class SmartManifestFilesMixin(CachedSettingsMixin, DynamicPatternsMixin,
    ...
    def url_converter(self, name, hashed_files, template=None):
        ...
        def converter(matchobj):
            ...
            url = groups[2]
            if len(groups) > 4:
                # Strip trailing characters if a minus is present at the end
                # of the ending tag (fix trailing newline after loud comments
                # in minified js)
                strip_trailing = groups[3] == '-'
                trailing_chars = groups[4] or ''
            else:
