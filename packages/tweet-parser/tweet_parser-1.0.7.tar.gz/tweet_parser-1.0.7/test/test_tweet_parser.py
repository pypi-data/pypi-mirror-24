...
class TestTweetMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tweet_ids = []
        for line in fileinput.FileInput("tweet_payload_examples/activity_streams_examples_hpt.json"):
            tweet = Tweet(json.loads(line))
            tweet_ids.append(tweet.id)
            tweet_payloads["activity_streams"][tweet.id] = tweet
        for line in fileinput.FileInput("tweet_payload_examples/original_format_examples_hpt.json"):
            tweet = Tweet(json.loads(line))
            tweet_ids.append(tweet.id)
