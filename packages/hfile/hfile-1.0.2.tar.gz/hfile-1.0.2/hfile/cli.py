...
fh = logging.handlers.RotatingFileHandler("/tmp/hfile.log", maxBytes=20*1024*1024*200,
                              backupCount=3)
...
def check(context, paths, beforehours, extension, remove):
    ...
    try:
        ...
        checkedfilelists = list(filter(lambda x: filterbymtime(x, beforehours=beforehours, extension_tuple=extension_tuple), mutfileslist))
