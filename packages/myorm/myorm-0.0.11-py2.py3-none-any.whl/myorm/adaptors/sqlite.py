...
class SQLiteAdaptor(object):
    ...
    def _build_filter(self, filter):
        ...
        for item in filter:
            if item[0].endswith('__in'):
                filters.append('%s IN (%s)' % (item[0].replace('__in', ''), ', '.join('?' * len(item[1]))))
            elif item[0].endswith('__lt'):
                filters.append('%s < %s' % (item[0].replace('__lt', ''), '?'))
            elif item[0].endswith('__lte'):
                filters.append('%s <= %s' % (item[0].replace('__lte', ''), '?'))
            elif item[0].endswith('__gt'):
                filters.append('%s > %s' % (item[0].replace('__gt', ''), '?'))
            elif item[0].endswith('__gte'):
                filters.append('%s >= %s' % (item[0].replace('__gte', ''), '?'))
            elif item[0] == 'order_by':
                direction = 'DESC' if item[1].startswith('-') else 'ASC'
                order += (' ORDER BY %s %s' % (item[1].replace('-', ''), direction))
            else:
                filters.append('%s=?' % item[0])
        if filters:
    ...
    def get_update_query(self, cls, **kwargs):
        ...
        settings = ', '.join(['%s=?' % item for item in keys])
