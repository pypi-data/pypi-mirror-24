...
def find_repos(dirpath):
    ...
    for root, dirnames, filenames in os.walk(dirpath):
        for dirname in fnmatch.filter(dirnames, '.git'):
            ...
            if 'tsantor' in repo_info['url'] or 'xstudios' in repo_info['url']:
...
def get_repo_info(dirpath, ignore=False):
    ...
    if output.startswith('https') or output.startswith('ssh') or output.startswith('git'):
