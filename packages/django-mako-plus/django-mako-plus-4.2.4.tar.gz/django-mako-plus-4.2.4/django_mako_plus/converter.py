...
class DefaultConverter(BaseConverter):
    ...
    def _check_default(self, value, parameter, task, default_chars):
        ...
        if value in default_chars:
            if parameter.default is inspect.Parameter.empty:
                raise ValueError('Value was empty, but no default value is given in view function for parameter: %s (%s)', parameter.position, parameter.name)
    ...
    def convert_number(self, value, parameter, task):
        ...
        '''
        value, defaulted = self._check_default(value, parameter, task, ( '', '-', None ))
    ...
    def convert_boolean(self, value, parameter, task, default=False):
        ...
        if isinstance(value, bool):
            return value
        return value[0] not in ( 'f', 'F', '0', False )
    ...
    def convert_datetime(self, value, parameter, task):
        ...
        for fmt in settings.DATETIME_INPUT_FORMATS:
            try:
                ...
                continue
        raise ValueError("'%s' does not match a format in settings.DATETIME_INPUT_FORMATS" % value)
    ...
    def convert_date(self, value, parameter, task):
        ...
        for fmt in settings.DATE_INPUT_FORMATS:
            try:
                ...
                continue
        raise ValueError("'%s' does not match a format in settings.DATE_INPUT_FORMATS" % value)
