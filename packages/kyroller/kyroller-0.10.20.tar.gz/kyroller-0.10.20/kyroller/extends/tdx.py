...
#        self._com_event_conn = cc.GetEvents( self._com_obj, sink=self._event_sink)
        con = cc.ShowEvents(self._com_obj)
        self._com_obj.AuthFile = 'D:\\tmp\\ZMStockCom\\Bin\\hl-TradeAuth.zmd'
    ...
    def sync_assets(self):
        ...
        for d in arrStocks:
            ...
            if symbol not in self._positionDic:
                self._positionDic[symbol] = PositionBook(symbol)
            # 可卖数量
            print(d)
    ...
    def mk_order(self, order: Order):
        ...
        print('同步下单')
        # 同步下单
        mRet = self._com_obj.SyncCommitOrder(
            True,
        ...
        if mRet[1] == None:
            raise TradeError(self._com_obj.LastErrDesc)
        print(mRet)
        # 异步下单
        # 提交api
        # self._com_obj.AddOrder(
