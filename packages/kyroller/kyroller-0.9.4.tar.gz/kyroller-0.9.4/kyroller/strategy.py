...
class Strategy:
    ...
    def account(self) -> Account:
        return self._counter.query_account_sync()
    ...
    @counter.setter
    def counter(self, value: Counter):
        self._counter = value

    def get_last_error(self):
        return self.counter.get_last_error()
    ...
    def cancel_order(self, order_cid):
        return self.counter.calcel_order(order_cid)
    ...
    def mk_order(self, symbol, order_side, volume,  price=0, order_type=None):
        ...
        return self.counter.mk_order_sync(order)
    ...
    def prepare(self, subs, start_time=None, end_time=None):
        ...
        self.make_account_snapshot(start_time)
    ...
    def make_account_snapshot(self, timestamp):
        s = self.account.make_snapshot()
    ...
    def handle_tick(self, tick):
        ...
        self.make_account_snapshot(tick.timestamp)
        self.on_tick(tick)

    def handle_bar(self, bar):
        self._timestamp = bar.timestamp
        self.make_account_snapshot(bar.timestamp)
        self.on_bar(bar)

    def handle_market_event(self, e):
        self._timestamp = e.timestamp
        self.make_account_snapshot(e.timestamp)
