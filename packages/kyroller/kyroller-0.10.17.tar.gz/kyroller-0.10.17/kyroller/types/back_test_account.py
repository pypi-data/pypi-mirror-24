...
class BackTestAccount(Account):
    ...
    def cost_volume(self, symbol, volume, price):
        ...
        if symbol not in self._positionBookDic:
            return False
        return self._positionBookDic[symbol].cost_volume(volume, price)
    ...
    def add_position(self, symbol, volume, price):
        ...
        return self._positionBookDic[symbol].add_position(volume, price, locked=self._locked_today_volume)
    ...
    def handle_market_event(self, e: MarketEvent):
        if e.status == MarketStatus.PH:
            for o in self._orders:
                self.cancel_order(o)
            for book in self._positionBookDic.values():
