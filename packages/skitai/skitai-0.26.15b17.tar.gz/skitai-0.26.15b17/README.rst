...
More About Cache Control: Model Synchronizable Cache
```````````````````````````````````````````````````````
...
Please note that first arg of startup, reload and shutdown is *wac* not *was*. *wac* is Python Class object of 'was', so mainly used for sharing Skitai server-wide object via was.object before instanclizating to *was*.
