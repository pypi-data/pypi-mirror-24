...
def create_zip(source_folder, target_folder, zip_file_name):
    if is_valid_folder(source_folder, os.W_OK) and is_valid_folder(target_folder, os.W_OK):
        try:
            ...
            eprint("%s | Unable to zip files from %s" % (datetime.utcnow(), source_folder))
            sys.exit(1)
...
def zip_directory(path, zip_file):
    for root, dirs, files in os.walk(path):
        for file in files:
            absolute_file_path = u(os.path.join(root, file))
            if os.path.isfile(absolute_file_path):
                try:
                    zip_file.write(absolute_file_path)
                except IOError:
                    eprint("%s | Error zipping file %s absolute path %s" % (datetime.utcnow(), file, absolute_file_path))
                    sys.exit(1)
            else:
...
def md5_file(file_path):
    try:
        ...
        eprint("%s | Unable to calculate checksum for %s" % (datetime.utcnow(), file_path))
        sys.stdout.flush()
        sys.exit(1)
    ...
    if is_valid_folder(sequence_folder, os.R_OK):
        for directory_path, directory_names, file_names in os.walk(sequence_folder):
            try:
                ...
                eprint("%s | Unable to traverse path %s" % (datetime.utcnow(), str(directory_path)))
...
def write_checksum(folder, checksum_entries):
    if is_valid_folder(folder, os.W_OK):
        try:
            checksum_file = open(os.path.join(folder, 'checksum.txt'), 'w')
            for entry in checksum_entries:
                checksum_file.write("%s\n" % entry)
            print("%s | Completed writing to checksum.txt" % (datetime.utcnow()))
        except:
            eprint("%s | Unable to write checksum.txt within folder %s" % (datetime.utcnow(), str(folder)))
            sys.exit(1)
...
def main():
    ...
    | \| | __|_   _| __| |  |_ _\ \/ /    
    | .` | _|  | | | _|| |__ | | >  <     
    |_|\_|___| |_| |_| |____|___/_/\_\    packager.py version 0.3.7
    ...
    try:
        ...
        if args.act == "zip":
            zip_file_name = u(args.file)
            source_folder = u(args.source)
            if args.target is None:
                target_folder = u(os.path.abspath(os.path.join(source_folder, '..')))
            else:
                target_folder = u(args.target)
