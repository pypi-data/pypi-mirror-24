...
def create_zip(source_folder, target_folder, zip_file_name):
    if is_valid_folder(source_folder, os.W_OK) and is_valid_folder(target_folder, os.W_OK):
        try:
            ...
            print("%s | Initializing zip file %s" % (datetime.utcnow(), str(zip_file_full_path)))
...
def main():
    ...
    | \| | __|_   _| __| |  |_ _\ \/ /    
    | .` | _|  | | | _|| |__ | | >  <     
    |_|\_|___| |_| |_| |____|___/_/\_\    packager.py version 0.3.6
    ...
    try:
        ...
        if args.act == "zip":
            ...
            if args.target is None:
                target_folder = os.path.abspath(os.path.join(source_folder, '..'))
            else:
