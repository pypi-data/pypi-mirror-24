...
from passhole import __version__
...
def init_database(args):
    ...
    if not os.path.exists(args.database):
        ...
        # generate a random AES256 keyfile
            ...
            log.debug("Looking for keyfile at {}".format(args.keyfile))
            if not args.keyfile:
                if os.path.exists(keyfile_path):
                    log.info("Found existing keyfile at {}  Exiting".format(bold(keyfile_path)))
                else:
