...
def data_from_file(file):
    ...
    with open(file) as f:
        data = json.load(f)
    url_string = "/users/download?data=%5B"
    for region in data:
        for az in data[region]:
            for instance_type in data[region][az]['instance_types']:
                url_string += "%7B%22az%22:%22" + az + "%22,%22instance_type%22:%22" + instance_type + "%22%7D,"
    url_string = url_string[0:-1]
    url_string += "%5D&response_type=json"
    return url_string

def sign_in(email, password, url_base):
    jwt_response = requests.post(url_base + '/user_token', json = { 'auth': { 'email': email, 'password': password } } )
    return jwt_response.json()['jwt']
