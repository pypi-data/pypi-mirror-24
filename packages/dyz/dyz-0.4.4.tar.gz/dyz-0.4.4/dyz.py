...
def process_yamls(ctx, name, paths):
    ...
    for path in paths:
        if os.path.isfile(path):
            ...
            new_paths = sorted(new_paths, lambda k: k.strip().lower())
...
def __get_ref_records(ctx, odoo, model, ref, typology, context):
    object_from_xml_id = ctx.obj['object_from_xml_id']
    if not ref:
        return model.browse([])
    ...
    if isinstance(ref, (list, tuple)) and (ref[0] in ['&', '|', '!'] or isinstance(ref[0], (list, tuple))):
        return model.browse(model.search(ref))
...
def __process_values(ctx, odoo, model, mode, values, fields, many2many, view_info, context):
    assert not values or isinstance(values, dict), 'The values should be a dictionnary, found %s' % type(values)
...
def __process_record(ctx, odoo, record, context):
    ...
    model = odoo.env[model_name]
    # SET VARS
    ALLOWED_TYPOLOGIES = ['model', 'one', 'multi']
    # GET DEFAULT PARAMS
    ...
    assert typology in ALLOWED_TYPOLOGIES, 'The typology %s should be in %' % (typology, ALLOWED_TYPOLOGIES)
    ...
    if message:
        ...
        click.secho('*' * message_len, fg='blue')
    records = __get_ref_records(ctx, odoo, model, ref, typology, context)
    ...
    for export_line in exports:
        if 'as' in export_line.lower():
            export_tuple = [x.strip() for x in export_line.split('as')]
...
def load_yaml(ctx, datas):
    ...
    for data in datas:
        ...
        block = data.copy()
        if not set(['record']) & set(block.keys()):
    ...
    for block in blocks:
        if 'record' in block:
            context = __process_record(ctx, odoo, block, context)
