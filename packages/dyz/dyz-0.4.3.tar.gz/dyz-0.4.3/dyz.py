    ...
    if value:
        for d in value:
            ...
            if operator.startswith('dt'):
                operator = operator[2:]
                value = parse_date_hash(value).strftime(DATETIME_FORMAT)
            if operator.startswith('d'):
                operator = operator[1:]
                value = parse_date_hash(value).strftime(DATE_FORMAT)
            domain.append((field, operator, value))
    return domain


def parse_date_hash(hash, dt=None):
    if not dt:
        dt = datetime.now()
...
def cli(ctx, database, host, port, user, password, superuserpassword, protocol, timeout, config, load, mode, export,
        prompt_login, prompt_connect, yes):
    """CLI for Odoo"""
