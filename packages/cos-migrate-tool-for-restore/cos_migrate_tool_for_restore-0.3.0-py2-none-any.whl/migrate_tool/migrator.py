...
class ThreadMigrator(BaseMigrator):
    ...
    def check_thread(self):
        ...
        try:
            while True:
                if self._stop and self._restore_check_queue.empty():
                    logger.info("stop flag is true, and restore check queue is empty, check thread will exit")
                    break
                ...
                while True:
                    ...
                    if self._share_queue.qsize() > self._max_task_queue_size:
                        logger.info("running task queue len: %d, larger than max size: %d, sleep 3 second...",
                                    self._share_queue.qsize(), self._max_task_queue_size)
                        time.sleep(self._sleep_seconds)
                        continue
                    else:
                        self._share_queue.put(task)
                        logger.info("add task: %s, to running task queue done, running task queue size: %d",
                                    task.key, self._share_queue.qsize())
                        break
