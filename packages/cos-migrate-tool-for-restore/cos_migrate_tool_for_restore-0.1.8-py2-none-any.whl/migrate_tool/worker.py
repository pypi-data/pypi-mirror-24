...
class Worker(object):
    ...
    def start(self):
        ...
        self._threads_pool.apply_async(func=work_thread, args=(self,))
...
def work_thread(worker):
    logger.info("multiprocessing worker is starting")
    while not worker._stop:
        ...
        try:
            ...
            try:
                ...
                pass
    logger.info("multiprocessing worker: %d, will exit", os.getpid())
pass