...
def main_thread():
    ...
    # init work pool
    work_dir = conf.get('common', 'workspace')
    start_pool(work_dir, output_service, input_service)
...
share_queue = multiprocessing.JoinableQueue()
...
def start_pool(work_dir, output_service, input_service):
    logger.info("multiprocessing thread pool is staring")
    threads_pool.apply(func=work_thread, args=(share_queue, work_dir, output_service, input_service))
...
def work_thread(share_q, work_dir, output_service, input_service):
