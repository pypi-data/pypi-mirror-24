...
class JasperCore(object):
	...
	def __init__(self, filepath, callback_table=None, port=5237, start_all=False, restarts_no=-1):
		logger.info('JASPER CORE ONLINE [PID %d]', os.getpid())
		self.__port = port
		self.__restarts_no = restarts_no
		if callback_table:
			self.__callback_table = callback_table
		else:
			self.__callback_table = dict()
		...
		while True:
			...
			if pid == 0:
				...
				try:
					...
					logger.error('execv failed (module \'%s\')', module)
					# $$$ CALLBACK "exec_fail"
	...
	def __APIgetall(self):
		for m in self.__modules:
			if self.__modules[m]['pid'] >= 0:
				p = psutil.Process(self.__modules[m]['pid'])
				self.__modules[m]['cpu'] = p.cpu_percent()
				self.__modules[m]['mem'] = round(p.memory_info().rss / (10 ** 6), 2)
			else:
				self.__modules[m]['cpu'] = -1
				self.__modules[m]['mem'] = -1
	...
	def __APIstop(self, module):
		if module == 'all':
			for m in self.__modules:
				self.__modules[m]['task_need_stop'] = True
			for m in self.__modules:
				counter = 0
				while self.__grep_pid(m) >= 0:
					try:
						os.kill(self.__modules[m]['pid'], SIGTERM)
						self.__modules[m]['pid'] = self.__grep_pid(m)
						# $$$ CALLBACK "stop"
						if 'stop' in self.__callback_table and callable(self.__callback_table['stop']):
							self.__callback_table['stop'](m)
					except:
						if counter > 3:
							break
						else:
	...
	def __APIrestart_on_crash(self, module):
		pass
