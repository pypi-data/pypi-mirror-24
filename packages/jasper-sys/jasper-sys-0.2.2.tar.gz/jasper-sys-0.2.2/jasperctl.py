...
class JasperCoreController:
	...
	def print_table_content(self, stdscr):
		...
		for module in sorted(self.modules):
			...
			while len(module_str) < tsize.columns / 4 - 10:
				module_str += ' '
			if self.modules[module]['pid'] >= 0:
				module_str += '{}'.format(self.modules[module]['pid'])
			else:
				module_str += '-'
			while len(module_str) < tsize.columns / 2 - 10:
				module_str += ' '
	...
	def main(self, stdscr):
		...
		stdscr.addstr(2, int(tsize.columns / 4) - 10, 'PID', curses.A_BOLD)
		stdscr.addstr(2, int(tsize.columns / 2) - 10, 'CPU', curses.A_BOLD)
		stdscr.addstr(2, int(tsize.columns / 2) + 10, 'Start-on-boot', curses.A_BOLD)
