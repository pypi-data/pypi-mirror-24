...
class IO(object):
	...
	def getData(self, filename_root):
		...
		if fType in self.axona_files:
			...
			data = f.read()
			st = data.find('data_start') + len('data_start')
			f.seek(st)
			if fType == '.log':
				f.seek(0)
...
#                    pdb.set_trace()
					raise ValueError()
					print "filter must contain a key / value pair"
					return
			if 'speed' in key:
				if filterDict[key][0] > filterDict[key][1]:
					raise ValueError()
					print "First value must be less than the second one"
				else:
...
class EEG(IO):
	...
	def eegfilter(self, E=None):
		...
		eegfilter = scipy.signal.firwin(self.sample_rate + 1, [self.x1/nyquist, self.x2/nyquist], window='black', pass_zero=False)
