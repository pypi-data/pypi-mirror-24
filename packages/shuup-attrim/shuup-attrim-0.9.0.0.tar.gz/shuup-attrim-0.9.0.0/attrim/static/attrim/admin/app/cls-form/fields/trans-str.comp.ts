...
@Component({
    ...
    // it's screwed up for dynamic components in 4.0.3
...
export class TransStrComponent implements OnInit {
    ...
    private langCodes: Array<LangCode> = window.DJANGO.langCodes
    private langCodeSelected: LangCode = window.DJANGO.defaultLang
    ...
    private selectLangCode(langCode: LangCode) {
        this.langCodeSelected = langCode
    }

    private updateTranslations(langCode: LangCode, value: string) {
    ...
    private setButtonClasses(langCode: LangCode): {[className: string]: boolean} {
