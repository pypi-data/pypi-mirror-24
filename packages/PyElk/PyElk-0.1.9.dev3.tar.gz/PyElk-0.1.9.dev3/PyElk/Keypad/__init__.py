...
class Keypad(Node):
    ...
    def unpack_event_user_code_entered(self, event):
        ...
        if user == 0:
            ...
            self._last_user_name = self._pyelk.USERS[user]
        ...
        if self._area > 0:
            ...
            self._pyelk.AREAS[self._area].last_user_name = self._pyelk.USERS[user]
            self._pyelk.AERAS[self._area].last_keypad = self._number
