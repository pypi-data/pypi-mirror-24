...
class Server:
    def __init__(self, host="localhost", port=0, conn_type=BaseConnection, on_new_conn=lambda x: None, max_clients=None, conn_args=()):
        self.host = host
        self.port = port
    ...
    def new_conn(self, socket_, client_addr):
        connection = self.conn_type(socket_, *self.conn_args)
