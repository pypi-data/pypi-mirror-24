...
class UserFactor(object):
    ...
    def get_uploaded_factor(self):
        ...
        """
        r, data = utils.get_factors(self.__cookies)
        if not r:
            print (data)
            return []
    ...
    def submit_factor(self, factor_name, chinese_name, chinese_description, category, factor_data_file_path,
        ...
        for item in category.split(','):
            ...
            'Common_Index', 'Per_share_Indicators', 'Others'):
                ...
                return
        print ('正在加载csv数据...')
        df, msg = factor.load_df_from_file(factor_data_file_path)
        if df is None:
            print (msg)
            return
        print ('加载csv数据完成')
        print ('正在校验csv数据格式...')
        r, msg = factor.check_factor_data(df)
        if not r:
            print (msg)
            return
        print ('校验csv数据格式完成')
        print ('正在提交因子...')
        r, f_id = utils.create_factor(self.__cookies, factor_name, category, chinese_name, chinese_description,
                                      function_list.split(','), universe)
        if not r:
            print ('创建因子失败：%s' % f_id)
            return
        print ('提交因子完成\n正在上传因子数据并触发因子报告计算...')
        r, msg = utils.upload_factor_data(self.__cookies, f_id, factor_data_file_path)
        if not r:
            print (msg)
        ...
        succ = False
        while True:
            if utils.check_factor_upload_done(self.__cookies, f_id):
                succ = True
                break
            if count >= 50:
                break
    ...
    def update_factor_data(self, factor_name, factor_data_file_path):
        ...
        if not r:
            ...
            return
        print ('校验csv数据格式完成\n正在上传因子数据...')
        r, msg = utils.update_uploaded_factor_data(self.__cookies, factor_name, factor_data_file_path)
        if not r:
            print (msg)
...
def upload_factor_data(f_id, file_path):
    import zlib
...
def check_factor_upload_done(f_id):
    try:
        url = Upload_FactorData()
...
def update_uploaded_factor_data(factor_name, file_path):
    import zlib
    with open(file_path) as f:
        data = f.read()
        try:
