...
class DataClient(object):
    ...
    def BDP(self, sec, field, overrides=None, RT=False):
        ...
        if overrides is None:
            ...
            overrides = json.dumps(overrides).upper()
        ...
        for k in range(NUM_TRIES):
            try:
                response = self.query(self.bdp_url, request, RT)
                break
    ...
    # -------------------------------------------------------------------------
    def BDH(self, sec, field, sd, ed, adj=True, overrides=None):
        # --- ensure security, field, and overrides are upper case as the
        #     request caching is case sensitive
        sec = sec.upper()
        field = field.upper()
    ...
    def query(self, base_url, request):
        # --- send http GET request to datafeed router which then
        #     dispatches the request to one of the available datafeed
        #     servers
...
#        if response["type"] == "StreamClosedError":
#            raise ConnectionResetError(response["payload"])
