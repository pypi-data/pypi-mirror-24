...
class DataRouter(tornado.web.Application):
    ...
    def drop_dataserver(self, server_id, msg):
        logger.info("dropping dataserver {0!s}: {1:s}".format(server_id, msg))
        try:
    ...
    def start(self, port=None):
        ...
        logger.info("listening on {0!s}:{1!s}".format(address, port))
    ...
    def process_request(self, req, real_time):
        ...
        if response is None:
            ...
            logger.debug("returning value from dataserver (RT={0!s})"
                         ", req_uid='{1!s}'".format(real_time, req_uid))

        else:
            logger.debug("returning value from "
                         "cache (RT={0!s})".format(real_time))
