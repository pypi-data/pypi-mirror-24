    ...
    def BDP(self, sec, field,
        ...
        for k in range(fail_after):
            try:
                ...
                # --- unless we are at the last iteration of the loop wait for
                #     a few seconds before retying
                if k == fail_after - 1:
                    raise
                else:
                    time.sleep(30)
            logger.info("retrying query, attempt number {0:d}".format(k+2))
        else:
            raise RuntimeError("query failed, but we should never get here!!!")
