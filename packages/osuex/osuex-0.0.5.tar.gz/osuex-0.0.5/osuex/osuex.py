...
def do(osufilesdir,beatmapsdir,skinsdir):
    ...
    user,err = subprocess.Popen("whoami",shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE).communicate()
    user = user.replace('\n','').replace('$','')
    beatmapsdir = beatmapsdir if beatmapsdir!='/.PlayOnLinux/wineprefix/osu_on_linux/drive_c/Program Files/osu!/Songs/' else "/home/"+user+beatmapsdir
    skinsdir = skinsdir if skinsdir!='/.PlayOnLinux/wineprefix/osu_on_linux/drive_c/Program Files/osu!/Skins/' else "/home/"+user+skinsdir
