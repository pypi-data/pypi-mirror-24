...
class NiceTrainer:
    def __init__(self,
        ...
        self._are_dynamic_feed_keys = hasattr(feed_keys, '__call__')
    ...
    def train(self):
        ...
        for batch in self.bm_train:
            t_start = time.time()
            if self._are_dynamic_feed_keys:
                feed_dict, _extra_vars = self.feed_keys(batch, is_training=True)
                assert isinstance(feed_dict, dict) and isinstance(_extra_vars, dict), 'Your custom feed_keys function should return 2 dicts: feed_keys, extra_vars'
            else:
                feed_dict = dict(zip(self.feed_keys, batch))
            res = self.sess.run(self.fetches, feed_dict=feed_dict)
            t_end = time.time()
    ...
    def validate(self):
        ...
        for batch in self.bm_val:
            if self._are_dynamic_feed_keys:
                feed_dict, _extra_vars = self.feed_keys(batch, is_training=False)
                assert isinstance(feed_dict, dict) and isinstance(_extra_vars, dict), 'Your custom feed_keys function should return 2 dicts: feed_keys, extra_vars'
            else:
                feed_dict = dict(zip(self.feed_keys, batch))
            res = self.sess.run(fetches, feed_dict)
...
def accuracy_calc_op(n=1, avg_preds_over=1, logits_from='probs'):
    '''
    ...
    def acc_op(extra_vars, batch):
        return _caclulate_batch_top_n_hits(extra_vars['logits_from'], batch[1], n, avg_preds_over) / float(len(batch[1]))
    return acc_op
