...
Usage:
  ...
  #opereto services <service-name> versions
  #opereto versions delete <version>
  opereto agents list [<search_pattern>]
  opereto environments list
  opereto globals list
  opereto (-h | --help)
...
Options:
    ...
    --params=JSON_PARAMS : process input parameters. Must be a JSON string (e.g. --params='{"param1": "value", "param2": true, "param3": 100}')
...
def wait_and_print_log(pid):
    ...
    def _print_log_entries(s):
        log_entries = client.get_process_log(pid, start=s,limit=100)
        if log_entries:
            for entry in client.get_process_log(pid):
                if entry['level']=='info':
                    print(entry['text'])
                else:
    ...
    while(True):
        ...
        new_start = _print_log_entries(start)
...
def list_globals(arguments):
    ...
    if arguments['<search_pattern>']:
        filter={'generic': arguments['<search_pattern>']}
    globals = client.search_globals(filter=filter, start=0, limit=1000, fset='clitool')
    if globals:
        print json.dumps(agents, indent=4, sort_keys=True)
    else:
