...
import sys, os
import string, types
from pNbody import mpi, error, io, libutil, parameters
...
class Nbody_gadget:
  ...
  def initComovingIntegration(self):
    ...
    if self.comovingintegration!=None:
      return
  ...
  def read_particles(self,f):
    ...
    ##########################################
    tpl = (24,48,float,float,np.int32,np.int32,24,np.int32,np.int32,float,float,float,float,np.int32,np.int32,24,np.int32,np.int32,np.int32,float,44)
    ...
    if np.fabs(flag_chimie_extraheader)>1:		# if the header is empty, it may be indetermined
      flag_chimie_extraheader=0
    ...
    if self.pio == 'no':
      ...
      for i in range(len(npart_tot)):				      
        if massarr[i] == 0:					      
          ...
          mass = np.concatenate((mass,np.ones(npart[i])*massarr[i]))    
    ...
    if flag_chimie_extraheader==1:
      ...
      nelts,ChimieSolarMassAbundances,labels = io.ReadBlock(f,tpl,byteorder=self.byteorder,pio=self.pio)
      ...
      ChimieSolarMassAbundances = np.fromstring(ChimieSolarMassAbundances,np.float32)
    ...
    ##########################################       
    vec='pos'
    if mpi.mpi_IsMaster(): print "reading %s..."%vec
    pos = io.ReadDataBlock(f,np.float32,shape=(nbody_read,3),byteorder=self.byteorder,pio=self.pio,npart=npart_read,skip=self.skip_io_block(vec))
    
    vec='vel'
    if mpi.mpi_IsMaster(): print "reading %s..."%vec
    vel = io.ReadDataBlock(f,np.float32,shape=(nbody_read,3),byteorder=self.byteorder,pio=self.pio,npart=npart_read,skip=self.skip_io_block(vec))

    vec='num'
    if mpi.mpi_IsMaster(): print "reading %s..."%vec    
    num = io.ReadDataBlock(f,np.int32  ,shape=(nbody_read,) ,byteorder=self.byteorder,pio=self.pio,npart=npart_read,skip=self.skip_io_block(vec))
    ...
    if nzero != 0:   
      ...
      vec='mass'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec
      massnzero = io.ReadDataBlock(f,np.float32,shape=(nzero,),byteorder=self.byteorder,pio=self.pio,npart=npart_m_read,skip=self.skip_io_block(vec))
    ...
    if not io.end_of_file(f,pio=self.pio,MPI=MPI) and ngas_read!=0:
      vec = 'u'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec 
      u   = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
      if u is not None: u   = np.concatenate((u,np.zeros(nbody-ngas).astype(np.float32)))
    
    if not io.end_of_file(f,pio=self.pio,MPI=MPI) and ngas_read!=0:
      vec = 'rho'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec       
      rho = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
      if rho is not None: rho = np.concatenate((rho,np.zeros(nbody-ngas).astype(np.float32)))
    
    if not io.end_of_file(f,pio=self.pio,MPI=MPI) and ngas_read!=0:
      vec = 'rsp'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec 
      rsp  = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
      if rsp is not None: rsp  = np.concatenate((rsp,np.zeros(nbody-ngas).astype(np.float32)))
    ...
    if flag_metals:		# gas metal properties
      ...
      if not io.end_of_file(f,pio=self.pio,MPI=MPI):
        vec = 'metals'
	if mpi.mpi_IsMaster(): print "reading %s..."%vec
        metals = io.ReadDataBlock(f,np.float32,shape=(ngas_read,NELEMENTS),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
        if metals is not None: metals = np.concatenate((metals,np.zeros((nbody-ngas,NELEMENTS)).astype(np.float32)))
    ...
    if flag_thermaltime:	# gas thermal(adiabatic) time
      ...
      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and ngas_read!=0:
        vec = 'thtsnii'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec       
        thtsnii = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
        if thtsnii is not None: thtsnii = np.concatenate((thtsnii,np.zeros(nbody-ngas).astype(np.float32)))

      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and ngas_read!=0:
        vec = 'thtsnia'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec       
        thtsnia = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
        if thtsnia is not None: thtsnia = np.concatenate((thtsnia,np.zeros(nbody-ngas).astype(np.float32)))
    ...
    if flag_age:		# stellar properties
      ...
      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and nstars_read!=0:
        vec = 'tstar'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
        tstar  = io.ReadDataBlock(f,np.float32,shape=(nstars_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
	if tstar is not None: tstar  = np.concatenate((-1*np.ones(ngas).astype(np.float32),tstar,-1*np.ones(nbody-ngas-nstars).astype(np.float32)))
	
      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and nstars_read!=0:
        vec = 'minit'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
        minit  = io.ReadDataBlock(f,np.float32,shape=(nstars_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
	if minit is not None: minit  = np.concatenate((0*np.ones(ngas).astype(np.float32),minit,0*np.ones(nbody-ngas-nstars).astype(np.float32)))	
		  	  	 
      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and nstars_read!=0:
        vec = 'idp'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
        idp = io.ReadDataBlock(f,np.int32,shape=(nstars_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
        if idp is not None: idp  = np.concatenate((-1*np.ones(ngas).astype(np.float32),idp,-1*np.ones(nbody-ngas-nstars).astype(np.float32)))

      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and nstars_read!=0:
        vec = 'rho_stars'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
        rho_stars = io.ReadDataBlock(f,np.float32,shape=(nstars_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block('rho'))
	if rho_stars is not None: 
	  if rho is None:
	    rho = np.concatenate((rho_stars,np.zeros(nbody-ngas-nstars).astype(np.float32)))
	  else:
	    rho = np.concatenate((rho[:ngas],rho_stars,np.zeros(nbody-ngas-nstars).astype(np.float32)))

      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and nstars_read!=0:
        vec = 'rsp_stars'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
        rsp_stars = io.ReadDataBlock(f,np.float32,shape=(nstars_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block('rsp'))
        if rsp_stars is not None: 
	  if rsp is None:
	    rsp = np.concatenate((rsp_stars,np.zeros(nbody-ngas-nstars).astype(np.float32)))
          else:
	    rsp = np.concatenate((rsp[:ngas],rsp_stars,np.zeros(nbody-ngas-nstars).astype(np.float32)))
    ...
    if flag_metals:	      # stars metal properties	 
      ...
      if not io.end_of_file(f,pio=self.pio,MPI=MPI) and nstars_read!=0:
        vec = 'metals_stars'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
    	metals_stars = io.ReadDataBlock(f,np.float32,shape=(nstars_read,NELEMENTS),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block('metals'))
    	if metals_stars is not None: metals = np.concatenate(( metals[:ngas,:],  metals_stars, np.zeros((nbody-ngas-nstars,NELEMENTS)).astype(np.float32) ))
    ...
    if not io.end_of_file(f,pio=self.pio,MPI=MPI):
      ...
      if not io.end_of_file(f,pio=self.pio,MPI=MPI):
        vec = 'pot'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
        pot   = io.ReadDataBlock(f,np.float32,shape=(nbody_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
    
      if not io.end_of_file(f,pio=self.pio,MPI=MPI):
        vec = 'acc'
        if mpi.mpi_IsMaster(): print "reading %s..."%vec
        acc = io.ReadDataBlock(f,np.float32,shape=(nbody_read,3),byteorder=self.byteorder,pio=self.pio,npart=npart_read,skip=self.skip_io_block(vec))
      '''
    
    if not io.end_of_file(f,pio=self.pio,MPI=MPI):
      vec = 'opt1'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec
      opt1  = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
      if opt1 is not None:
        if (len(opt1)==ngas): 
          opt1  = np.concatenate((opt1,np.zeros(nbody-ngas).astype(np.float32)))   

    if not io.end_of_file(f,pio=self.pio,MPI=MPI):
      vec = 'opt2'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec
      opt2 = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
      if opt2 is not None:
        if (len(opt2)==ngas): 
          opt2 = np.concatenate((opt2,np.zeros(nbody-ngas).astype(np.float32)))    
    ...
    if not io.end_of_file(f,pio=self.pio,MPI=MPI):
      vec = 'erd'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec    
      erd  = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
      if (len(erd)==ngas):
        erd  = np.concatenate((erd,np.zeros(nbody-ngas).astype(np.float32)))
	
    
    if not io.end_of_file(f,pio=self.pio,MPI=MPI):
      vec = 'dte'
      if mpi.mpi_IsMaster(): print "reading %s..."%vec    
      dte  = io.ReadDataBlock(f,np.float32,shape=(ngas_read,),byteorder=self.byteorder,pio=self.pio,npart=None,skip=self.skip_io_block(vec))
      dte  = np.concatenate((dte,np.zeros(nbody-ngas).astype(np.float32)))
    ...
    if type(self.massarr) == np.ndarray:
      self.massarr = self.massarr.tolist()
    if type(self.nall) == np.ndarray:
      self.nall = self.nall.tolist()	  
    if type(self.nallhw) == np.ndarray:
  ...
  def write_particles(self,f):
    ...
    if self.pio == 'yes':
      ...
      npart_m_write = np.array(self.npart) * (np.array(self.massarr)==0)
	...
	massarr_all_red = np.compress(npart_all[:,i]!=0,massarr_all[:,i])
    ...
    if nzero != 0:
      ...
      for i in range(len(self.npart)): 
    	if npart[i]!=0 and massarr[i]==0.:
    	  massnzero = np.concatenate((massnzero,self.mass[ni:ni+self.npart[i]]))
    ...
    # header																																								   
    tpl = ((npart,24),(massarr,48),(atime,float),(redshift,float),(flag_sfr,np.int32),(flag_feedback,np.int32),(nall,24),(flag_cooling,np.int32),(num_files,np.int32),(boxsize,float),(omega0,float),(omegalambda,float),(hubbleparam,float),(flag_age,np.int32),(flag_metals,np.int32),(nallhw,24),(flag_entr_ic,np.int32),(flag_chimie_extraheader,np.int32),(flag_thermaltime,np.int32),(critical_energy_spec,float),(empty,44))  
    ...
    if self.flag_chimie_extraheader:
      ...
      tpl = ((self.ChimieNelements,np.int32), (SolarMassAbundances,np.float32), (labels,len(labels)))
    ...
    # positions
    io.WriteArray(f,self.pos.astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=npart_write)
    # velocities
    io.WriteArray(f,self.vel.astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=npart_write)
    # id
    io.WriteArray(f,self.num.astype(np.int32),byteorder=self.byteorder,pio=self.pio,npart=npart_write)

    # write mass if needed
    if nzero != 0:
      io.WriteArray(f,massnzero.astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=npart_m_write)

    # write extension
    if self.has_array('u'):
      if self.u is not None and self.npart[0]>0:
        io.WriteArray(f,self.u[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write u"
    if self.has_array('rho'):
      if self.rho is not None and self.npart[0]>0:
        io.WriteArray(f,self.rho[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write rho"
    if self.has_array('rsp'):
      if self.rsp is not None and self.npart[0]>0:
        io.WriteArray(f,self.rsp[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write rsp"
    ...
    if self.flag_metals:
      if self.has_array('metals'):
        io.WriteArray(f,self.metals[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write metals"

    if flag_thermaltime:
      if self.has_array('thtsnii'):
        io.WriteArray(f,self.thtsnii[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write thtsnii"

      if self.has_array('thtsnia'):
        io.WriteArray(f,self.thtsnia[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write thtsnia"
    ...
    if self.flag_age:
      if self.has_array('tstar') and self.npart[1]>0:
        io.WriteArray(f,self.tstar[self.npart[0]:self.npart[0]+self.npart[1]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write tstar"

      if self.has_array('minit') and self.npart[1]>0:
        io.WriteArray(f,self.minit[self.npart[0]:self.npart[0]+self.npart[1]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write minit"

      if self.has_array('idp') and self.npart[1]>0:
        io.WriteArray(f,self.idp[self.npart[0]:self.npart[0]+self.npart[1]].astype(np.int32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write idp"

      if self.has_array('rho') and self.npart[1]>0:
        data = self.rho[self.npart[0]:self.npart[0]+self.npart[1]].astype(np.float32)
        if len(data)>0:
      ...
      if self.has_array('rsp') and self.npart[1]>0:
        data = self.rsp[self.npart[0]:self.npart[0]+self.npart[1]].astype(np.float32)
        if len(data)>0:
          io.WriteArray(f,data,byteorder=self.byteorder,pio=self.pio,npart=None)
          if mpi.mpi_IsMaster():
    ...
    if self.flag_metals:
      if self.has_array('metals') and self.npart[1]>0:
        data = self.metals[self.npart[0]:self.npart[0]+self.npart[1]].astype(np.float32)
        if len(data)>0:
          io.WriteArray(f,data,byteorder=self.byteorder,pio=self.pio,npart=None)
          if mpi.mpi_IsMaster():
    ...
    if self.has_array('pot'):
      if self.pot is not None:
        io.WriteArray(f,self.pot.astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write pot"

    if self.has_array('acc'):
      if self.acc is not None:
        io.WriteArray(f,self.acc.astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write acc"
    ...
    if self.has_array('opt1'):
      if self.opt1 is not None:
        io.WriteArray(f,self.opt1[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write opt1"

    if self.has_array('opt2'):
      if self.opt2 is not None:
        io.WriteArray(f,self.opt2[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
        if mpi.mpi_IsMaster():
          print "write opt2"
    ...
    if self.has_array('erd'):
      if self.erd is not None:
        io.WriteArray(f,self.erd[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
    if self.has_array('dte'):
      if self.dte is not None:
        io.WriteArray(f,self.dte[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
    #if self.has_array('pot'):
    #  if self.pot is not None:
    #    io.WriteArray(f,self.pot[:self.npart[0]].astype(np.float32),byteorder=self.byteorder,pio=self.pio,npart=None)
