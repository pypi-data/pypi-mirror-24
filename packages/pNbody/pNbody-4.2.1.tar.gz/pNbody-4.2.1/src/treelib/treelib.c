...
void domain_topsplit_local(Tree *self,int node, peanokey startkey)
{
  ...
  if(self->TopNodes[node].Size >= 8)
      ...
      for(i = 0; i < 8; i++)
	{
      ...
      for(p = self->TopNodes[node].Pstart; p < self->TopNodes[node].Pstart + self->TopNodes[node].Count; p++)
	{
	  ...
	  if(self->TopNodes[sub].Count == 0)
	    self->TopNodes[sub].Pstart = p;
	...
	{
	  sub = self->TopNodes[node].Daughter + i;
	  if(self->TopNodes[sub].Count > self->All.TotNumPart / (TOPNODEFACTOR * self->NTask * self->NTask))
...
void domain_determineTopTree(Tree *self)
{
  ...
  for(i = 0; i < self->NumPart; i++)
    {
      self->KeySorted[i] = self->Key[i] = peano_hilbert_key((self->P[i].Pos[0] - self->DomainCorner[0]) * self->DomainFac,
...
void domain_decompose(Tree *self)
{
  ...
  for(i = 0; i < 6; i++)
    self->Ntype[i] = self->NtypeLocal[i];
...
//      self->list_NumPart[0] = self->NumPart;
//      self->list_N_gas[0]   = self->N_gas;
...
void force_flag_localnodes(Tree *self)
{
  ...
  //for(i = DomainMyStart; i <= DomainMyLast; i++)					/* !!! warning !!! */
  for(i = 0; i < self->NTopleaves; i++)
    {
      /*
  ...
  while(no >= 0)
      ...
      if(no < self->All.MaxPart)	/* single particle */
	{
	  ...
	  if(NGB_PERIODIC_Z(P[p].Pos[2] - searchcenter[2]) > hsml)
	    continue;
#else
	  if(self->P[p].Pos[0] < searchmin[0])
	    continue;
	  if(self->P[p].Pos[0] > searchmax[0])
	    continue;
	  if(self->P[p].Pos[1] < searchmin[1])
	    continue;
	  if(self->P[p].Pos[1] > searchmax[1])
	    continue;
	  if(self->P[p].Pos[2] < searchmin[2])
	    continue;
	  if(self->P[p].Pos[2] > searchmax[2])
	    continue;
#endif
	  ...
	  if(numngb == MAX_NGB)
	      ...
	      if(numngb == MAX_NGB)
		{
		  printf("ThisTask=%d: Need to do a second neighbour loop for (%g|%g|%g) hsml=%g no=%d\n",
			 self->ThisTask, searchcenter[0], searchcenter[1], searchcenter[2], hsml, no);
		  *startnode = no;
		  return numngb;
		}
	    }
 ...
 * (Yves Revaz)
 */
double density_numngb_evaluate(Tree* self,FLOAT Pos[3],FLOAT Vel[3],double h)
{
...
double density_evaluate(Tree* self,FLOAT Pos[3],FLOAT Vel[3],double h, double *Density, double *NumNgb,double *DhsmlDensityFactor)
{
 ...
 */
double density(Tree* self,FLOAT Pos[3],FLOAT Vel[3],double *h, double *numNgb)
{
  ...
  do
    {
      ...
      density_evaluate(self,Pos,Vel,Hsml,&Density,&NumNgb,&DhsmlDensityFactor);

      /* do final operations on results */
      npleft = 0;
		    
      DhsmlDensityFactor = 1 / (1 + Hsml * DhsmlDensityFactor / (NUMDIMS * Density));
      ...
      if(NumNgb < (self->All.DesNumNgb - self->All.MaxNumNgbDeviation) ||
          ...
          && Hsml > (1.01 * self->All.MinGasHsml)))
        {
          ...
          if(iter >= MAXITER - 10)
            {
	      //printf("Hsml=%g Left=%g Right=%g NumNgb=%g Right-Left=%g\n",Hsml,Left,Right,NumNgb,Right-Left);
            }
          ...
          if(Right > 0 && Left > 0)
              ...
              if(Right == 0 && Left > 0)
        	{
        	  if(fabs(NumNgb - self->All.DesNumNgb) < 0.5 * self->All.DesNumNgb)
        	    {
        	      Hsml *= 1 - (NumNgb - self->All.DesNumNgb) / (NUMDIMS * NumNgb) * DhsmlDensityFactor;
        	    }
        	  else
        	    Hsml *= 1.26;
        	}
          ...
          if(Hsml < self->All.MinGasHsml)
            Hsml = self->All.MinGasHsml;
        }
      else
        {
	  npleft=0;
  ...
  //printf("Hsml=%g Density=%g NumNgb=%g\n",Hsml,Density,NumNgb);  
...
void sph_evaluate_rot(Tree* self,FLOAT Pos[3],FLOAT Vel[3],double h,double* rotvx, double* rotvy, double* rotvz)
{
      ...
      for(n = 0; n < numngb_inbox; n++)
	{
	  ...
	  if(r2 < h2)
	      ...
	      else
		{
	      ...
	      valA  += self->P[j].Mass*(self->P[j].Observable)/(self->P[j].Density) * wk;	
...
static PyObject *
Tree_Density(Tree* self, PyObject *args)
{
    ...
    if (! PyArg_ParseTuple(args, "OOdd",&pos,&hsml,&DesNumNgb,&MaxNumNgbDeviation))
        return PyString_FromString("error");
        
    PyArrayObject *vdensity,*vhsml;
    int i;
    double x,y,z,ldensity,lhsml,lnumNgb,weighted_numngb;
    npy_intp   ld[1];
    int input_dimension;
    FLOAT lpos[3],lvel[3];
    ...
    /* create a NumPy object */	  
    ld[0]=pos->dimensions[0];
    //vdensity = (PyArrayObject *) PyArray_FromDims(1,ld,PyArray_FLOAT);
    vdensity = (PyArrayObject *) PyArray_SimpleNew(1,ld,NPY_FLOAT);
    //vhsml    = (PyArrayObject *) PyArray_FromDims(1,ld,PyArray_FLOAT);  
    ...
    for (i = 0; i < pos->dimensions[0]; i++) 
      {
        lpos[0] = *(float *) (pos->data + i*(pos->strides[0]) + 0*pos->strides[1]);
        lpos[1] = *(float *) (pos->data + i*(pos->strides[0]) + 1*pos->strides[1]);
        lpos[2] = *(float *) (pos->data + i*(pos->strides[0]) + 2*pos->strides[1]);	
	lvel[0] = 0.0;
        lvel[1] = 0.0;
        lvel[2] = 0.0;
	lhsml = *(float *) (hsml->data + i*(hsml->strides[0])                    );
	

	ldensity = density(self,lpos,lvel,&lhsml,&lnumNgb);
	*(float *)(vdensity->data + i*(vdensity->strides[0]))  = ldensity;
	*(float *)(vhsml->data + i*(vhsml->strides[0]))  = lhsml;
...
static PyObject *
Tree_InitHsml(Tree* self, PyObject *args)
{
	...
	while(10 * self->All.DesNumNgb * self->P[i].Mass > self->Nodes[no].u.d.mass)
	  {
...
#ifndef TWODIMS
        lhsml = pow(3.0 / (4 * M_PI) * self->All.DesNumNgb * self->P[i].Mass / self->Nodes[no].u.d.mass, 1.0 / 3) * self->Nodes[no].len;
#else
	lhsml = pow(1.0 / (M_PI) * self->All.DesNumNgb * self->P[i].Mass / self->Nodes[no].u.d.mass, 1.0 / 2) * self->Nodes[no].len;
#endif
...
static PyObject *
Tree_SphEvaluate(Tree* self, PyObject *args)
{
    ...
    if (! PyArg_ParseTuple(args, "OOOOdd",&pos,&hsml,&Density,&Observable,&DesNumNgb,&MaxNumNgbDeviation))
        return PyString_FromString("error");
    ...
    if (Density->dimensions[0] != self->NumPart)
      PyErr_SetString(PyExc_ValueError,"len of third argument must equal NumPart\n");

    if (Observable->dimensions[0] != self->NumPart)
      PyErr_SetString(PyExc_ValueError,"len of fourth argument must equal NumPart\n");


    
    /* create a NumPy object */	  
    ld[0]=pos->dimensions[0];
    //vobservable    = (PyArrayObject *) PyArray_FromDims(1,ld,PyArray_FLOAT);  
    ...
    for(i = 0; i < self->NumPart; i++)
      {    
        self->P[i].Density    = *(float *) (Density->data    + i*(Density->strides[0]));
	self->P[i].Observable = *(float *) (Observable->data + i*(Observable->strides[0]));
      }
	...
	*(float *)(vobservable->data + i*(vobservable->strides[0]))  = lobservable;	
...
static PyObject *
Tree_SphEvaluateDiv(Tree* self, PyObject *args)
{
    ...
    if (! PyArg_ParseTuple(args, "OOOOdd",&pos,&vel,&hsml,&Density,&DesNumNgb,&MaxNumNgbDeviation))
        return PyString_FromString("error");
    ...
    for (i = 0; i < pos->dimensions[0]; i++) 
      {
        ...
        lpos[1] = *(float *) (pos->data + i*(pos->strides[0]) + 1*pos->strides[1]);
        lpos[2] = *(float *) (pos->data + i*(pos->strides[0]) + 2*pos->strides[1]);	
        lvel[0] = *(float *) (vel->data + i*(vel->strides[0]) + 0*vel->strides[1]);
        lvel[1] = *(float *) (vel->data + i*(vel->strides[0]) + 1*vel->strides[1]);
        lvel[2] = *(float *) (vel->data + i*(vel->strides[0]) + 2*vel->strides[1]);	
...
static PyObject *
Tree_SphEvaluateRot(Tree* self, PyObject *args)
{
    ...
    /* create a NumPy object */	  
    //vobservable = (PyArrayObject *) PyArray_FromDims(pos->nd,pos->dimensions,pos->descr->type_num);
    ld[0]=pos->dimensions[0];
    ld[1]=pos->dimensions[1];
    /* there is a kind of bug here ! I cannt replace ld by pos->dimensions */
    vobservable = (PyArrayObject *) PyArray_SimpleNew(pos->nd,ld,pos->descr->type_num);
	...
	*(float *)(vobservable->data + i*(vobservable->strides[0]) + 0*vobservable->strides[1])  = lrotx;
	*(float *)(vobservable->data + i*(vobservable->strides[0]) + 1*vobservable->strides[1])  = lroty;
	*(float *)(vobservable->data + i*(vobservable->strides[0]) + 2*vobservable->strides[1])  = lrotz;	
...
static PyObject *
Tree_SphEvaluateNgb(Tree* self, PyObject *args)
{
    ...
    /* create a NumPy object */	  
    ld[0]=pos->dimensions[0];
    //vnumngb    = (PyArrayObject *) PyArray_FromDims(1,ld,PyArray_INT);  
	...
	*(int *)(vnumngb->data + i*(vnumngb->strides[0]))  = numngb;	
...
static PyObject *
Tree_SphEvaluateNearestNgb(Tree* self, PyObject *args)
{
	...
	*(int *)(vjmin->data   + i*(vjmin->strides[0]  ))  = jmin;	
...
static PyObject *
Tree_SphGetNgb(Tree* self, PyObject *args)
{
    ...
    /* create a NumPy object */	  
    ld[0]=numngb;
    ngb_lst = (PyArrayObject *) PyArray_SimpleNew(1,ld,PyArray_INT);

    for (i = 0; i < ngb_lst->dimensions[0]; i++) 
      {
        
        *(int *)(ngb_lst->data + i*(ngb_lst->strides[0]))  = self->Ngblist[i];
...
static PyObject *
Tree_SphGetTrueNgb(Tree* self, PyObject *args)
{
    ...
    if (pos->dimensions[0]>1)
      return PyString_FromString("only one particle may be given here.");
    ...
    ld[0]=truenumngb;
    ngb_lst = (PyArrayObject *) PyArray_SimpleNew(1,ld,PyArray_INT);
    wij     = (PyArrayObject *) PyArray_SimpleNew(1,ld,PyArray_DOUBLE);
	...
	if(r2 < h2)
	  {
	    ...
	    *(int *)(ngb_lst->data + k*(ngb_lst->strides[0]))  = j;
	    
	    *(double *)(wij->data + k*(wij->strides[0]))  = wk;
...
static PyMethodDef Tree_methods[] = {
    ...
    {"InitHsml", (PyCFunction)Tree_InitHsml, METH_VARARGS,
     "This function is used to find an initial smoothing length for each particle"
    },   
    ...
    {"SphEvaluateNgb", (PyCFunction)Tree_SphEvaluateNgb, METH_VARARGS,
     "This function return the number of real neighbours."
      ...
      {"SphEvaluateNearestNgb", (PyCFunction)Tree_SphEvaluateNearestNgb, METH_VARARGS,
     "This function return the nearest neighbor of a particle."
    },    

    {"SphGetNgb", (PyCFunction)Tree_SphGetNgb, METH_VARARGS,
     "This function returns indexes of neighbour particles in a box (may be incomplete...)."
    },        
