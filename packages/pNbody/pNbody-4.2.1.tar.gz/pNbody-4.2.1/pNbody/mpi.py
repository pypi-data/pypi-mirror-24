...
#################################							         
def mpi_ReadAndSendBlock(f,data_type,shape=None,byteorder=sys.byteorder,split=None,htype=int32):	         
#################################							         
  '''	
...
#################################							         
def mpi_ReadAndSendArray(f,data_type,shape=None,byteorder=sys.byteorder,npart=None,skip=False,htype=int32):  
#################################							         
  '''			
  ...
  if ThisTask == 0:	
    ...
    if skip:
      nbytes = sum(npart)*nbytes_per_elt
...
#################################
def mpi_GatherAndWriteArray(f,data,byteorder=sys.byteorder,npart=None,htype=int32):
#################################
  '''
  ...
  if ThisTask==0:
    ...
    nbytes = array([data_nbytes],htype)
