...
# Aliases and copying
    ...
    def delete_index(self, index):
        if index != self.get_index(index)[0]:
            raise BadRequestError("Attempt to delete index with an alias id")
        aliases = self.get_aliases(index)  
        if len(aliases) > 0:
            raise BadRequestError("Attempt to delete index with an alias assigned")
        url = self.make_url(index)
        r = self.request("delete", url)
        return r
