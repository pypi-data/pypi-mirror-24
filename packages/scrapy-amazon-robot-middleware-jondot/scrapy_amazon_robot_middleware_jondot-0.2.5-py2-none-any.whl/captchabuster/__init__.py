...
def test():
    ...
    cb = CaptchaBuster(
        StringIO(session.get(soup.find('img').get('src')).content))
    print cb.guess
...
class RobotMiddleware(object):
    ...
    def request_image(self, request, response):
        ...
        meta = {
            'form_action': action,
            'form_params': params,
            'original_request': request.meta.get('original_request') or
            request,
    ...
    def process_response(self, request, response, spider):
        ...
        if request.meta.get('crack_retry_count', 0) > self.MAX_RETRY:
            raise IgnoreRequest('Max retries exceeded %s' %
                                request.meta.get('original_request', request))
