...
def gp_camera_wait_for_event(camera, timeout, context):
    ...
    * `context` :  
    ...
    timeout occurs before an event is received then *eventtype==GP_EVENT_TIMEOUT
    and eventdata is left unchanged. If an event is received then eventtype is
    set to the type of event, and eventdata is set to event specific data. See
    the CameraEventType enum to see which eventtype's match to which types of
    eventdata.  
