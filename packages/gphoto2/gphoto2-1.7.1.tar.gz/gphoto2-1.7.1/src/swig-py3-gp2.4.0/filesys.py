...
class _CameraFilesystem(object):
    ...
    The internals of the CameraFilesystem are only visible to gphoto2. You can
...
class CameraFileInfoFile(object):
    ...
    Contains information a regular file with fields being set depending on the
    bitmask in the fields member.  
...
class CameraFileInfoPreview(object):
    ...
    Contains information of a preview file with fields being set depending on
...
class CameraFileInfoAudio(object):
    ...
    Contains information of an audio file with fields being set depending on the
    bitmask in the fields member.  
...
class CameraStorageInformation(object):
    ...
    This structure contains the information of a specific camera storage. Only
    the members as specified by the *fields* member are valid.  
...
def gp_filesystem_append(fs: '_CameraFilesystem', folder: 'char const *', filename: 'char const *', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Tells the fs that there is a file called filename in folder called folder.
    Usually camera drivers will call this function after capturing an image in
    order to tell the fs about the new file. A front-end should not use this
    function.  
...
def gp_filesystem_set_info_noop(fs: '_CameraFilesystem', folder: 'char const *', info: 'CameraFileInfo', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    In contrast to gp_filesystem_set_info, gp_filesystem_set_info_noop will only
    change the file information in the fs. Typically, camera drivers will use
    this function in case they get file information "for free" on
...
def gp_filesystem_set_file_noop(fs: '_CameraFilesystem', folder: 'char const *', file: 'CameraFile', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Tells the fs about a file. Typically, camera drivers will call this function
    in case they get information about a file (i.e. preview) "for free" on
    gp_camera_capture() or gp_camera_folder_list_files().  
...
def gp_filesystem_reset(fs: '_CameraFilesystem') -> "int":
    ...
    * `fs` :  
    ...
    Resets the filesystem. All cached information including the folder tree will
    get lost and will be queried again on demand.  
...
def gp_filesystem_get_folder(fs: '_CameraFilesystem', filename: 'char const *', context: 'Context') -> "char **":
    ...
    * `context` :  
    ...
    Searches a file called filename in the fs and returns the first occurrency.
    This functionality is needed for camera drivers that cannot figure out where
    a file gets created after capturing an image although the name of the image
    is known. Usually, those drivers will call gp_filesystem_reset in order to
    tell the fs that something has changed and then gp_filesystem_get_folder in
...
def gp_filesystem_number(fs: '_CameraFilesystem', folder: 'char const *', filename: 'char const *', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Looks for a file called filename in the given folder. See gp_filesystem_name
    for exactly the opposite functionality.  
...
def gp_filesystem_set_list_funcs(fs: '_CameraFilesystem', file_list_func: 'CameraFilesystemListFunc', folder_list_func: 'CameraFilesystemListFunc', data: 'void *') -> "int":
    ...
    * `data` :  
    ...
    Tells the fs which functions to use to retrieve listings of folders and/or
    files. Typically, a camera driver would call this function on
    initialization. Each function can be NULL indicating that this functionality
    is not supported. For example, many cameras don't support folders. In this
    case, you would supply NULL for folder_list_func. Then, the fs assumes that
...
def gp_filesystem_list_folders(fs: '_CameraFilesystem', folder: 'char const *', context: 'Context') -> "CameraList *":
    ...
    * `context` :  
    ...
    Generates a list of subfolders of the supplied folder either using cached
    values (if there are any) or the folder_list_func if it has been supplied
    previously. If not, it is assumed that only a root folder exists (which is
    the case for many cameras).  
...
def gp_filesystem_set_info_funcs(fs: '_CameraFilesystem', get_info_func: 'CameraFilesystemGetInfoFunc', set_info_func: 'CameraFilesystemSetInfoFunc', data: 'void *') -> "int":
    ...
    * `data` :  
    ...
    Tells the filesystem which functions to call when file information about a
    file should be retrieved or set. Typically, this function will get called by
    the camera driver on initialization.  
...
def gp_filesystem_set_file_funcs(fs: '_CameraFilesystem', get_file_func: 'CameraFilesystemGetFileFunc', del_file_func: 'CameraFilesystemDeleteFileFunc', data: 'void *') -> "int":
    ...
    * `data` :  
    ...
    function can be NULL indicating that this functionality is not supported.
...
def gp_filesystem_get_file(fs: '_CameraFilesystem', folder: 'char const *', filename: 'char const *', type: 'CameraFileType', file: 'CameraFile', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Downloads the file called filename from the folder using the get_file_func
    if such a function has been previously supplied. If the file has been
    previously downloaded, the file is retrieved from cache. The result is
    stored in the passed file structure.  
...
def gp_filesystem_delete_file(fs: '_CameraFilesystem', folder: 'char const *', filename: 'char const *', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    called and, if this function returns without error, the file will be removed
    from the fs.  
...
def gp_filesystem_set_folder_funcs(fs: '_CameraFilesystem', put_file_func: 'CameraFilesystemPutFileFunc', delete_all_func: 'CameraFilesystemDeleteAllFunc', make_dir_func: 'CameraFilesystemDirFunc', remove_dir_func: 'CameraFilesystemDirFunc', data: 'void *') -> "int":
    ...
    * `data` :  
    ...
    Tells the filesystem which functions to call for file upload, deletion of
    all files in a given folder, creation or removal of a folder. Typically, a
    camera driver would call this function on initialization. If one
    functionality is not supported, NULL can be supplied. If you don't call this
    function, the fs will assume that neither of these features is supported.  

    The fs will try to compensate missing delete_all_func functionality with the
...
def gp_filesystem_put_file(fs: '_CameraFilesystem', folder: 'char const *', file: 'CameraFile', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Uploads a file to the camera if a put_file_func has been previously supplied
    to the fs. If the upload is successful, the file will get cached in the fs.  
...
def gp_filesystem_delete_all(fs: '_CameraFilesystem', folder: 'char const *', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Deletes all files in the given folder from the fs. If the fs has not been
    supplied with a delete_all_func, it tries to delete the files one by one
    using the delete_file_func. If that function has not been supplied neither,
