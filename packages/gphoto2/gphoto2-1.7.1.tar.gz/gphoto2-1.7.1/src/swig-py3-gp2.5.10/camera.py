...
class CameraText(object):
    ...
    A text structure containing translated text returned by various functions
    (about, manual, summary). You should not assume a size.  
...
def gp_camera_set_abilities(camera: 'Camera', abilities: 'CameraAbilities') -> "int":
    ...
    * `abilities` :  
    ...
    You need to call this function before calling gp_camera_init the first time
    unless you want gphoto2 to autodetect cameras and choose the first detected
    one. By setting the `abilities`, you tell gphoto2 what model the `camera` is
    and what camera driver should be used for accessing the `camera`. You can
    get `abilities` by calling gp_abilities_list_get_abilities.  
...
def gp_camera_set_port_speed(camera: 'Camera', speed: 'int') -> "int":
    ...
    * `speed` :  
    ...
    gp_camera_init for debugging purposes. Normally, a camera driver will try to
    figure out the current speed of the camera and set the speed to the optimal
    one automatically. Note that this function only works with serial ports. In
    other words, you have to set the camera's port to a serial one (using
    #gp_camera_set_port_path or #gp_camera_set_port_name) prior calling this
    function.  
...
def gp_camera_autodetect(context: 'Context') -> "CameraList *":
    ...
    * `context` :  
    ...
    CameraList *list; gp_list_new (&list); gp_camera_autodetect (list, context);
    ... done! ...  
...
def gp_camera_init(camera: 'Camera', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    autodetect any cameras and chooses the first one if any cameras are found.
    It is generally a good idea to call gp_camera_exit() after transactions have
    been completed in order to give other applications the chance to access the
    camera, too.  
...
def gp_camera_exit(camera: 'Camera', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Closes a connection to the camera and therefore gives other application the
    possibility to access the camera, too.  

    It is recommended that you call this function when you currently don't need
    the camera. The camera will get reinitialized by gp_camera_init()
...
def gp_camera_get_single_config(camera: 'Camera', name: 'char const *', context: 'Context') -> "CameraWidget **":
    ...
    * `context` :  
    ...
    This `widget` will then contain the current and the possible values and the
    type.  
...
def gp_camera_set_config(camera: 'Camera', window: 'CameraWidget', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Typically, a `window` is retrieved using gp_camera_get_config and passed to
    this function in order to adjust the settings on the camera.  
...
def gp_camera_capture(camera: 'Camera', type: 'CameraCaptureType', context: 'Context') -> "CameraFilePath *":
    ...
    * `context` :  
    ...
    The resulting file will be stored on the camera. The location gets stored in
...
def gp_camera_wait_for_event(camera: 'Camera', timeout: 'int', context: 'Context') -> "void **":
    ...
    * `context` :  
    ...
    This function blocks and waits for an event to come from the camera. If a
    timeout occurs before an event is received then eventtype will be
    GP_EVENT_TIMEOUT and eventdata is left unchanged.  

    If an event is received then eventtype is set to the type of event, and
    eventdata is set to event specific data. See the CameraEventType enum to see
    which eventtypes match to which types of eventdata.  

    Note that this function will return one event after each other, you need to
    be able to call it multiple times, e.g. in a loop, when waiting for specific
    events.  
...
def gp_camera_get_storageinfo(camera: 'Camera', context: 'Context') -> "int *":
    ...
    * `context` :  
    ...
    specified filesystem, if supported by the device. The storage information is
    returned in an newly allocated array of CameraStorageInformation objects, to
    which the pointer pointed to by #sifs will be set.  

    The variable pointed to by #nrofsifs will be set to the number of elements
    in that array.  
...
def gp_camera_set_timeout_funcs(camera: 'Camera', start_func: 'CameraTimeoutStartFunc', stop_func: 'CameraTimeoutStopFunc', data: 'void *') -> "void":
    ...
    * `data` :  
    ...
    If your frontend has something like idle loops, it is recommended you use
    gp_camera_set_timeout_funcs in order to give the camera driver the
...
def gp_camera_start_timeout(camera: 'Camera', timeout: 'unsigned int', func: 'CameraTimeoutFunc') -> "int":
    ...
    * `func` :  
    ...
    This function should be called by the camera driver during camera_init() if
    the camera needs to be sent messages periodically in order to prevent it
    from shutting down.  
...
def gp_camera_stop_timeout(camera: 'Camera', id: 'unsigned int') -> "void":
    ...
    * `id` :  
    ...
    Call this function in the camera driver if you want to stop a periodic call
    to a function that has been started using gp_camera_start_timeout.  
