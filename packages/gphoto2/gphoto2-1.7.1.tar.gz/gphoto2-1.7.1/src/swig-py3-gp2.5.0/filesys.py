...
def gp_filesystem_read_file(fs: '_CameraFilesystem', folder: 'char const *', filename: 'char const *', type: 'CameraFileType', offset: 'uint64_t', buf: 'char *', size: 'uint64_t *', context: 'Context') -> "int":
    ...
    * `context` :  
    ...
    Downloads the file called filename from the folder using the read_file_func
    if such a function has been previously supplied. If the file has been
    previously downloaded, the file is retrieved from cache.  

    The file is read partially into the passed buffer. The read starts at offset
    on the device and goes for at most size bytes. Reading over the end of the
    file might give errors, so get the maximum file size via an info function
    before.  
