...
def gp_file_get_name_by_type(file: 'CameraFile', basename: 'char const *', type: 'CameraFileType') -> "char **":
    ...
    * `newname` :  
    ...
    depending on the gphoto filetype and the mime type in the file. The gphoto
    filetype will be converted to a prefix, like thumb_ or raw_, the mimetype
    will replace the current suffix by a different one (if necessary).  

    This can be used so that saving thumbnails or metadata will not overwrite
    the normal files.  
...
def gp_file_get_data_and_size(arg1: 'CameraFile') -> "unsigned long *":
    ...
    * `size` :  
    ...
    The pointer to data that is returned is still owned by libgphoto2 and its
    lifetime is the same as the #file.  
