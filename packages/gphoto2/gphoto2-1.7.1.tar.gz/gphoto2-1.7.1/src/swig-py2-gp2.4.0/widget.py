...
class CameraWidget(object):
    ...
    The internals of the CameraWidget are only visible to gphoto2. You can only
    access them using the functions provided by gphoto2.  
...
def gp_widget_set_value(widget, value):
    ...
    * `value` :  
    ...
    (float) for GP_WIDGET_RANGE, (int) for GP_WIDGET_DATE, GP_WIDGET_TOGGLE, and
...
def gp_widget_changed(widget):
    ...
    * `widget` :  
    ...
    Returns 1 if the state of the CameraWidget has been changed or 0 if not. In
    addition, it resets the changed flag to 0.  
