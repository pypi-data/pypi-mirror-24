...
def get_ip_domains_list(ip):
    # 不再用bing接口查询旁站
    resp = get_request(
        "https://x.threatbook.cn/7e2935f1ac5e47fd8ae79305f36200c8/ipRelatives?ip=%s&domain=fuck.com" % ip)
    html = resp['content']
    import re
...
http_domain varchar(70) not null)" % each_pang_urls_table_name
        ...
        for each_domain in tmp_domain_list:
            if each_domain not in domain_list and getIp(
                ...
                # 增加存活检测17/8/16
                if get_request("http://" + each_domain)['code'] != 0 or get_result("https://" + each_domain)['code'] != 0:
...
def get_sub_domains(target, use_tool="Sublist3r"):
    ...
    def Sublist3r(domain):
        ...
        for each_sub_domain in alllines:
            ...
            # 17/8/16增加存活检测
            if get_request("http://" + each_sub_domain)['code'] != 0 or get_request("https://" + each_sub_domain)['code'] != 0:
