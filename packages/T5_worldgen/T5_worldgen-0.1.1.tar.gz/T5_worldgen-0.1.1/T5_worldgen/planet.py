...
class Satellite(Planet):
    '''Satellite class'''
    hospitable_satellite_type_table = Table()
    hospitable_satellite_type_table.add_row(1, 'Worldlet')
    hospitable_satellite_type_table.add_row(2, 'Inferno')
    hospitable_satellite_type_table.add_row(3, 'Inner World')
    hospitable_satellite_type_table.add_row(4, 'BigWorld')
    hospitable_satellite_type_table.add_row(5, 'StormWorld')
    hospitable_satellite_type_table.add_row(6, 'RadWorld')
    hospitable_satellite_type_table.add_row(7, 'BigWorld')
    hospitable_satellite_type_table.dice = 1

    outer_satellite_type_table = Table()
    outer_satellite_type_table.add_row((1, 2), 'Worldlet')
    outer_satellite_type_table.add_row((3, 4), 'IceWOrld')
    outer_satellite_type_table.add_row(5, 'StormWorld')
    outer_satellite_type_table.add_row(6, 'RadWorld')
    outer_satellite_type_table.add_row(7, 'BigWorld')
    outer_satellite_type_table.dice = 1

    def __init__(self, primary):
        self.primary = primary
        self.system = primary.system
        super(Satellite, self).__init__(self.system)

        self.orbit = None
        if self.primary.orbit > self.system.stellar.habitable_zone:
            self.type = self.outer_satellite_type_table.roll()
        else:
            self.type = self.hospitable_satellite_type_table.roll()

    def worldlet(self):
        '''Worldlet'''
        max_pop = int(self.primary.population - 1)
        size = D3.roll(1, -3, floor=0)
        self.size = uwp.Size(size)
        self.determine_atmosphere()
        self.determine_hydrographics()
        self.determine_population()
        if self.population > max_pop:
            self.population = uwp.Population(max_pop)
        self.determine_government()
        self.determine_tech()
        self.starport = self.spaceport_table.roll()
