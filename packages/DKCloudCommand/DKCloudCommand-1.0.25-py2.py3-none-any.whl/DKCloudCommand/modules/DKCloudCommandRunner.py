...
class DKCloudCommandRunner(object):
        ...
        if rc.ok():
            ...
            if kl is not None and len(kl) > 0:
                ...
                if kitchen not in tree_kitchen_dict or tree_kitchen_dict[kitchen] is None or len(tree_kitchen_dict[kitchen]) == 0:
                    print 'No children kitchens found.'
                else:
                    depth = 0
                    print DKCloudCommandRunner.tree_view(kitchen, tree_kitchen_dict, depth)
            else:
    ...
    @staticmethod
    @check_api_param_decorator
    def kitchen_merge_preview(dk_api, from_kitchen, to_kitchen):
        """
        returns a string.
        :param dk_api: -- api object
        ...
        try:
            ...
            for line in results:
                print "%s\t\t%s" % (line['status'].rjust(8),line['file'])
                if line['status'] == 'conflict':
                    for item in ['base', 'left', 'right', 'merge']:
                        full_path = working_dir+'/'+line['file']+'.'+item
                        contents = line[item]
    ...
    @staticmethod
    @check_api_param_decorator
    def file_diff(dk_api, file_path, from_kitchen, to_kitchen):
        """
        returns a string.
        :param dk_api: -- api object
    ...
    def kitchen_merge(dk_api, from_kitchen, to_kitchen):
        ...
        try:
            ...
            print 'looking for merged (.base) files in temporary directory %s ' % working_dir
                ...
                if len(filenames) > 0:
                    for filename in filenames:
                        if filename.endswith('.base'):
                            ...
                            contents = DKFileUtils.read_file(full_path,encoding=encoding)
            ...
            rc.set(rc.DK_SUCCESS, 'Merge done.')
            return rc
    ...
    def kitchen_settings_get(dk_api, kitchen):
        ...
        if not rc.ok():
            ...
            rc.set_message('DKCloudCommand.kitchen_settings_get %s succeeded\n' % kitchen)
        return rc
