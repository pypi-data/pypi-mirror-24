...
def kitchen_delete(backend, kitchen, yes):
    ...
    """
    if not yes:
        confirm = raw_input('Are you sure you want to delete the remote copy of the Kitchen %s ? [yes/No]' % kitchen)
...
@click.pass_obj
def kitchen_merge_preview(backend, source_kitchen, target_kitchen):
    """
    ...
    """
    click.secho('%s - Previewing merge Kitchen %s into Kitchen %s' % (get_datetime(), source_kitchen, target_kitchen), fg='green')
    check_and_print(DKCloudCommandRunner.kitchen_merge_preview(backend.dki, source_kitchen, target_kitchen))
...
def kitchen_merge(backend, source_kitchen, target_kitchen, yes):
    ...
    check_and_print(DKCloudCommandRunner.kitchen_merge(backend.dki, source_kitchen, target_kitchen))
...
@click.argument('filepath', required=True)
@click.pass_obj
def file_diff(backend, source_kitchen, target_kitchen, filepath):
    """
    To be used after kitchen-merge-preview command.
    Launch the merge tool of choice, to resolve conflicts.

    """
    click.secho('%s - File Diff for file %s, source kitchen (%s), target kitchen(%s)' %
                (get_datetime(), filepath, source_kitchen, target_kitchen), fg='green')
    check_and_print(DKCloudCommandRunner.file_diff(backend.dki, filepath, source_kitchen, target_kitchen))
...
def kitchen_settings_get(backend):
    """
    Get Kitchen Settings (kitchen-settings.json) for the current Kitchen.
    """
    ...
    click.secho("%s - Getting a local copy of kitchen-settings.json for Kitchen '%s'" % (get_datetime(), kitchen), fg='green')
...
def kitchen_settings_update(backend, filepath):
    """
    Update Kitchen Settings (kitchen-settings.json) for the current Kitchen.
    """
    kitchen = DKCloudCommandRunner.which_kitchen_name()
    if kitchen is None:
        raise click.ClickException('You are not in a Kitchen')
    ...
    click.secho("%s - Updating the settings of Kitchen '%s'" % (get_datetime(), kitchen), fg='green')
