...
class DKCloudCommandRunner(object):
    ...
    @staticmethod
    @check_api_param_decorator
    def kitchen_settings_update(dk_api, kitchen, message, filepath):
        """
        """
        if kitchen is None or message is None or filepath is None:
            s = 'ERROR: DKCloudCommandRunner bad input parameters'
            rc = DKReturnCode()
            rc.set(rc.DK_FAIL, s)
            return rc
        rc = dk_api.kitchen_settings_json_update(kitchen, message, filepath)
        if not rc.ok():
