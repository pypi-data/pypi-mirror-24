...
__pragma__ ('endif')
    ...
    */
    ...
    Array.prototype.py_update = function () {   // O (n log n)
        var that = this.slice ();   // Make a shallow copy of existing set
        for (var i = 0; i < arguments.length; i++) {
            var argument = arguments [i];
            if (Array.isArray (argument)) {
                that = that.concat (argument);
            }
            else {                  // Needed for strings
                for (var j = 0; j  < argument.length; j++) {
                    that.push (argument [j]);
                }
            }
        }
        
        that.sort ()                // Put identical elements next to each other

        this.clear ();              // Clear existing set to prevent dupplicates
