...
class Classifiers(APIBase):
    def is_positive_or_negative(self, text):
        '''
        This API receives a text and returns the language of the text.
        :param text: Any unicode object
        :return: A Language code
        ...
        blueprint = self._hp.manage.services.show_all.data['positive_or_negative']
        payload = {
            'query': {'text': text},
            'meta': self._get_meta(blueprint)
        }
...
class NLP(APIBase):
    def __init__(self, highlyprobable):
        ...
        self.classifiers = Classifiers(highlyprobable)
