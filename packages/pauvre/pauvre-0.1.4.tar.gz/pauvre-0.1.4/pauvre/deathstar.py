...
class GFFParse():
    def __init__(self, filename):
        ...
        self.features = pd.DataFrame.from_dict(self.featureDict, orient='columns')
        #get rid of the random annotations that some programs add
        self.features = self.features.query("featType not in ['region', 'source']")
        ...
        self.features['width'] = abs(self.features['stop'] - self.features['start'])
        self.features['lmost'] = self.features.apply(self.determine_lmost, axis=1)
        self.features['rmost'] = self.features.apply(self.determine_rmost, axis=1)
    ...
    def parse(self, filename):
        with open(filename,"r") as f:
            for line in f:
                line=line.strip()
                #prevents the program from crashing on blank lines
                if line:
                    if line[0:2] != "##":
                        item = line.split()
                        self.featureDict["featType"].append(item[2])
                        self.featureDict["start"].append(int(item[3]))
                        self.featureDict["stop"].append(int(item[4]))
                        self.featureDict["strand"].append(item[6])
                        try:
                            name = item[8][5:].split(';')[0]

                        except:
                            name = item[0]
                            print("{} feature from {} to {} has no name in GFF file.".format(item[2], item[3], item[4]))
                        self.featureDict["name"].append(name)

    def determine_lmost(self, row):
...
def fix_query_reflength(sequence_length, queries, doubled):
    ...
    """
    print("hey I'm in the 'fix_query_reflength' method")
    if not doubled:
...
def plot_gff(args, panelCircle, gff_path, radius):
    ...
    plottable_features = gffParser.features.query("featType != 'tRNA'")
