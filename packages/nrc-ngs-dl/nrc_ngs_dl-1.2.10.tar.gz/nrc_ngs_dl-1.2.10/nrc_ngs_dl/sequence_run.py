    ...
    def rename_files(self):
        ...
        for dirpath, dirname,filename in os.walk(self.path_destination_folder):
            for a_dirname in dirname:
                if a_dirname.endswith('.failed'):
                    a_dir = os.path.join(dirpath,a_dirname)
                    os.rmdir(a_dir)
        
        for dirpath, dirname,filename in os.walk(self.path_destination_folder):           
            for a_file in filename:
                #a_path_name = os.path.join(self.path_destination_folder, a_file)
                #a_path = os.path.dirname(a_path_name)    
        ...
        for a_path in path_to_old_file:    
            for dirpath, dirname,filename in os.walk(a_path):
                for a_file in filename:
		            a_path1 = os.path.join(a_path, a_file)
                    #self.rename_a_file(a_file, a_path1)
