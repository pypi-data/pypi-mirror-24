...
CATALOG_NAME=bdcatalog-${OSCLASS}-${SANITIZED_DISTRO}-${VERSION}
...
DEST_DECOMP=${STAGING_DIR}/decompress-${OSCLASS}-${SANITIZED_DISTRO}.sh
...
sed -i "s|@@@OSCLASS@@@|${OSCLASS}|g" ${DEST_DECOMP}
sed -i "s|@@@OSMAJOR@@@|${OSMAJOR}|g" ${DEST_DECOMP}
