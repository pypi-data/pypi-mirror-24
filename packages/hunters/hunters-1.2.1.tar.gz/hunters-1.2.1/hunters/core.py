...
class TaskMeta(object):
    ...
    def to_dict(self):
        return {'max_deep': self.max_deep, 'max_count': self.max_count, 'count': self.count}
...
class Spider(object):
    ...
    def add_url(self, url, base_url=None, deep=0, headers=None, task_meta=None, extra={}):
        ...
        if task_meta.overlimit() or len(absolute_url) == 0:
            return self
...
class Tab(Browser, Response):
    ...
    @property
    def title(self):
        return self.dom().cssselect('title')[0].text
