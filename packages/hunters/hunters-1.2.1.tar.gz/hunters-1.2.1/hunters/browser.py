...
class MiniBrowser(Browser):
    ...
    def head(self, url, **kwargs):
        try:
            ...
            logger.error("HEAD url[%s], msg[%s]" % (url, e))
            r = Response()
            r.headers = {}
            r.url = url
    ...
    def get(self, url, **kwargs):
        ...
        try:
            return self.__detect_encoding(self.session.get(url, **kwargs))
    ...
    def post(self, url, data=None, json=None, **kwargs):
        return self.__detect_encoding(self.session.post(url, data, json, **kwargs))
    ...
    def __detect_encoding(self, response):
        ...
        before_encoding = response.encoding
        response.encoding = "UTF-8"  # 默认用UTF8
        ...
        if Regex.RE_TYPE_HTML.search(content_type):
            match = Regex.RE_MATA_CHARSET.search(response.text)
            if match:
                response.encoding = match.group(1)  # 解决乱码问题
                logging.info("DETECT ENCODING, url[%s], encoding[%s], before[%s]", response.url, response.encoding,
                             before_encoding)
...
class ViewBrowser(Browser):
    ...
    def screenshot_as_png(self, filename):
        ...
        try:
            if filename:
                filename = os.path.join(self.local_store, filename)
                if filename.startswith(self.local_store) and "../" not in filename:
