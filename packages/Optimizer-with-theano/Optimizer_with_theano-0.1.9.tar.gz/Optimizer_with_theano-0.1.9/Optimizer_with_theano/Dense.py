...
class Dense_layer(Layer):
    def __init__(self, obj, n_out, name=None):
        ...
        self.n_out   = (n_out,)
        self.theta   = theano.shared(np.random.uniform(0, 0.05, self.n_in, n_out).astype(theano.config.floatX),
                                    borrow=True)
        self.b       = theano.shared(np.random.uniform(0, 0.05, n_out).astype(dtype=theano.config.floatX),
                                    borrow=True)
