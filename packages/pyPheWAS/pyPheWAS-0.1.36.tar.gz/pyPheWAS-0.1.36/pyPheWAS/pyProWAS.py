...
def generate_feature_matrix(genotypes, phenotypes, reg_type, phewas_cov=''):  # diff - done
    ...
    for i in genotypes['id']:
        if reg_type == 0:
            ...
            match = prowas_codes['prowas_code'].isin(list(phenotypes[phenotypes['id'] == i]['prowas_code']))
            feature_matrix[0][count, match[match == True].index] = 1
            age = pd.merge(prowas_codes, temp, on='prowas_code', how='left')['MaxAgeAtCPT']
            ...
            if reg_type == 1:
                ...
                temp = pd.DataFrame(
                    phenotypes[phenotypes['id'] == i][['prowas_code', 'MaxAgeAtCPT', 'duration']]).drop_duplicates()
                dura = pd.merge(prowas_codes, temp, on='prowas_code', how='left')['duration']
...
def plot_data_points(y, thresh, save='', imbalances=np.array([])):  # same
    ...
    for i in idx:
        ...
        if y[i] > thresh and imbalances[i]>0:
            artists.append(plt.text(e, y[i], c['prowas_desc'][i], rotation=40, va='bottom'))
        e += 1
