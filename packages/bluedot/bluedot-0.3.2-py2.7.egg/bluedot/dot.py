...
class BlueDotInteraction():
    ...
    A BlueDotInteraction can be active or inactive, i.e. it is active because the Blue Dot has 
    not been released, or inactive because the Blue Dot was released and the interaction finished.
    ...
    def duration(self):
        ...
        If the interaction is still active ``None`` is returned.
        """
        if self.active:
    ...
    def released(self, released_position):
        """
        Stops the BlueDotInteraction called when the Blue Dot is released.
...
class BlueDotSwipe():
    ...
    :param BlueDotInteraction interaction:
        The BlueDotInteraction object to be used to determine the swipe.

    :param int time_threshold:
        The time in seconds in which a swipe has to be completed (i.e. the maximum time 
        between the Blue Dot being pressed, moved and released). Defaults to ``0.25``.

    :param int distance_threshold:
        The distance as a value between 0 - 1 (1 being the radius of the Blue Dot) 
        that a swipe has to be greater than. Defaults to ``0.5``.    
    """
    def __init__(self, interaction, time_threshold = 0.25, distance_threshold = 0.5):
    ...
    def swiped(self):
        ...
        """
        if self._swiped == None:
            # was the interaction quick enough and long enough to be a swipe?
            print(self._time_threshold)
            print(self.interaction.duration)
            print(self._time_threshold > self.interaction.duration)
            if self._time_threshold > self.interaction.duration:
                if self._distance_threshold < self.distance:
                    self._swiped = True
                else:
                    self._swiped = False
            else:
    ...
    def up(self):
        ...
        """
        return True if self.swiped and self.angle > -45 and self.angle <= 45 else False
    ...
    def down(self):
        ...
        """        
        return True if self.swiped and (self.angle > 135 or self.angle <= -135) else False
    ...
    def left(self):
        ...
        """        
        return True if self.swiped and self.angle > -135 and self.angle <= -45 else False
    ...
    def right(self):
        ...
        """
        return True if self.swiped and self.angle > 45 and self.angle <= 135 else False
...
class BlueDot():
    ...
    def _pressed(self):
        ...
        self._when_callback(self.when_pressed)
    ...
    def _released(self):
        ...
        self._when_callback(self.when_released)
    ...
    def _moved(self):
        ...
        self._when_callback(self.when_moved)
    
        self._is_moved_event.clear()
    
    def _when_callback(self, callback):
        if callback:
