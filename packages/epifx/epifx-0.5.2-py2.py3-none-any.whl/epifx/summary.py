...
class ThresholdMonitor(Monitor):
    ...
    def __init__(self, threshold):
        ...
        # TODO --- may want to accept a dict that maps obs_type to values?
    ...
    def add_rows(self, hist, weights, fs_date, dates, obs_types, insert_fn):
        ...
        for (d, d_ix, hist_ix) in dates:
            ...
            if obs:
                ...
                # TODO: stripping weights **may** cause an issue **here**!!
                # Yes, this function *requires* the complete matrix.
                # So ... add dimension checks throughout the code?!?
                # Good idea, especially throughout pypfilt!!!
                #  -- in particular, any externally-facing function!!!
                # TODO: can we ensure that *all* calls to model functions
                #       *only* pass the state-vector portion of the matrix?
                #       and then *check* this in model functions!
                # TODO: provide convenience functions:
                #       - pypfilt.is_entire_matrix(hist, raise=True)
                #       - pypfilt.is_statevec_only(hist, raise=True)
                #       - put these in a new module? e.g., pypfilt.check
