...
class DatabaseSetter(object):
    ...
    def recreate(self, schema, environment):
        ...
        if environment in ['dev', 'staging']:
            ...
            server = self.read_config_value(environment, 'db_server')
            username = self.read_config_value(environment, 'db_username')
            password = self.read_config_value(environment, 'db_password')
        else:
...
if __name__ == '__main__':
    DatabaseSetter().recreate('asset_managers', 'staging')