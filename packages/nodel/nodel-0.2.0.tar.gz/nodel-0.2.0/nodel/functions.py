...
import os
from . import structure, nodel_core
...
def run_service(params):
	nodel_core.execute('gunicorn --chdir %s --workers 10 --bind %s:%s core.wsgi:application' % (os.path.join(nodel_core.root_path(), 'core'), environ.get('ADDRESS', 'localhost'), environ.get('PORT', '8000')))


@nodel_core.register(group='make', action='service')
def make_service(params):
	name = os.path.basename(nodel_core.root_path())
	temp = os.path.join(os.path.abspath(environ.get('WORKON_HOME')), name)

	virtual_env_home = input('enter python home directory: (%s)' % temp)
	if not virtual_env_home:
		virtual_env_home = temp

	temp = environ.get('USER')
...
def make_project(params):
	...
	if django:
		django.make_project(nodel_core.root_path())
	else:
...
def install(params):
	nodel_core.execute('python -m pip install -r modules/%s.txt' % environ.get('ENV', 'dev'))
