...
class geneticoptimize(object):
    ...
    def row_score(self, vec, rang):
        ...
        [w1.append(math.ceil(vec[c])) for c in range(len(vec)) if c % 2 == 0]
        [w2.append(math.ceil(vec[c])) for c in range(len(vec)) if c % 2 != 0]
    ...
    def predict(self, x, gene):
        ...
        [w1.append(math.ceil(gene10[c])) for c in range(len(gene10)) if c % 2 == 0]
        [w2.append(math.ceil(gene10[c])) for c in range(len(gene10)) if c % 2 != 0]
        ...
        for c in columns:
            i = columns.index(c)
            if rang[(i + 1) * 2 - 1][3] > 0:
                row_score[c] = map(lambda q: w1[i] + min(q * w2[i], rang[(i + 1) * 2 - 1][3]) if q >= rang[i * 2][3] else 0, x[c])
            else:
                row_score[c] = map(lambda q: w1[i] + max(q * w2[i], rang[(i + 1) * 2 - 1][3]) if q >= rang[i * 2][3] else 0, x[c])
        row_score["col_sum"] = row_score.apply(lambda c: c.sum(), axis=1)
        return row_score
    ...
    def rule_weight(self, gene):
        ...
        w1 = []
        w2 = []
        [w1.append(math.ceil(gene10[0][i])) for i in range(len(gene10[0])) if i % 2 == 0]
        [w2.append(math.ceil(gene10[0][i])) for i in range(len(gene10[0])) if i % 2 != 0]
        rule_output = []
