...
class geneticoptimize(object):
    ...
    for example:
        if __name__=="__main__":
            ...
            BETA = 0.5
            test = geneticoptimize(rule_list, x_train, y_train, popsize=100, mutprob=0.3, maxmut_prop=0.5, elite=0.1, maxiter=100, beta=BETA)
    ...
    def costf(self, vec, rang):
        ...
        [w1.append(vec[_]) for _ in range(len(vec)) if _ % 2 == 0]
        [w2.append(vec[_]) for _ in range(len(vec)) if _ % 2 != 0]

        row_score = pd.DataFrame()
        columns = []
        [columns.append(_) for _ in self.x.columns]

        for c in columns:
            i = columns.index(c)
            if rang[(i + 1) * 2 - 1][3] > 0:
                row_score[c] = map(lambda _: w1[i] + min(_ * w2[i], rang[(i + 1) * 2 - 1][3]) if _ >= rang[i * 2][3] else 0, self.x[c])
            else:
                row_score[c] = map(lambda _: w1[i] + max(_ * w2[i], rang[(i + 1) * 2 - 1][3]) if _ >= rang[i * 2][3] else 0, self.x[c])

        row_score["col_sum"] = row_score.apply(lambda _: _.sum(), axis=1)
        result = pd.DataFrame()
    ...
    def issame(self, new_sample, pop2):
        max_sum = 0
        for _ in range(pop2.shape[0]):
            tmp = (pop2[_] == new_sample).sum()
            if tmp > max_sum:
                max_sum = tmp
        return max_sum
    ...
    def predict(self, x, gene):
        ...
        [w1.append(gene10[_]) for _ in range(len(gene10)) if _ % 2 == 0]
        [w2.append(gene10[_]) for _ in range(len(gene10)) if _ % 2 != 0]

        row_score = pd.DataFrame()
        columns = []
        [columns.append(_) for _ in x.columns]
        for c in columns:
            i = columns.index(c)
            if rang[(i + 1) * 2 - 1][3] > 0:
                row_score[c] = map(lambda _: w1[i] + min(_ * w2[i], rang[(i + 1) * 2 - 1][3]) if _ >= rang[i * 2][3] else 0, x[c])
            else:
                row_score[c] = map(lambda _: w1[i] + max(_ * w2[i], rang[(i + 1) * 2 - 1][3]) if _ >= rang[i * 2][3] else 0, x[c])
        row_score["col_sum"] = row_score.apply(lambda _: _.sum(), axis=1)
        return row_score
...
def get_weight_and_index(string, rang, rule_list, x, y, beta, risk_score=80):
    ...
    w1 = []
    w2 = []
    [w1.append(gene10[0][_]) for _ in range(len(gene10[0])) if _ % 2 == 0]
    [w2.append(gene10[0][_]) for _ in range(len(gene10[0])) if _ % 2 != 0]
