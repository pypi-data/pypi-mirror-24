...
over HTTPS, although this is usually not necessary. In this case, a few
more labels need to be added:
...
This is a lot like the ``HAPROXY_0_HTTP_FRONTEND_ACL_WITH_PATH``
template—we just add a redirect to ``marathon-acme``.
...
We redirect the HTTP challenge requests to HTTPS. **Note** that this can
only be switched on after the first certificate has been issued for
``marathon-acme``'s domain.
