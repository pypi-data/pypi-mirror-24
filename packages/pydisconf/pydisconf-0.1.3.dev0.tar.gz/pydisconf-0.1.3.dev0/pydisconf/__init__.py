...
class FileWatcher:
    ...
    def __call__(self, event):
        ...
        self.client.watch_file(self.file_name)
...
class ItemWatcher:
    ...
    def __call__(self, event):
        ...
        self.client.watch_item(self.item)
...
class Client:
    ...
    def watch_file(self, file_name, value=''):
        watch_node = '{prefix}/{app_name}_{version}_{env}/file/{file_name}'.format(
        ...
        try:
            ...
            self.zk.get(watch_node, watch=FileWatcher(self, file_name, value))
        except:
            pass

    def watch_item(self, item, value=''):
        watch_node = '{prefix}/{app_name}_{version}_{env}/item/{item}'.format(
            prefix=self.zk_prefix,
            app_name=self.app,
            version=self.version,
            env=self.env,
        ...
        try:
            self.zk.create(node_name, value=item, ephemeral=True)
            self.zk.get(watch_node, watch=ItemWatcher(self, item, value))
        except:
            pass
