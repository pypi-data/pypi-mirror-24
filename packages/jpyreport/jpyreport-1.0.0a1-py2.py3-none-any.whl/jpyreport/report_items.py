    ...
    with tag('div', klass='summary'):
      ...
      with tag('p', klass='attribute'):
        ...
        for level in self._status_levels:
          count = self.report[level]
          klass = 'summary %s' % level
          if count == 0:
            klass = klass + ' default'
          line('a', '%s %s' % (count, level), href=self._js_show_status_level(level),
               klass='status_link %s' % klass)
          if self._status_levels.index(level) < len(self._status_levels) - 1:
            text(', ')
