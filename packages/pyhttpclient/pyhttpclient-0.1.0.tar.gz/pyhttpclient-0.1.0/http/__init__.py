...
def put(url, data=None, headers=None):
    ...
    request = urllib2.Request(url, data=(data and urllib.urlencode(data) or ''))
    request.get_method = lambda: 'PUT'
    return read_and_log(opener.open(request))
...
def post(url, query=None, data=None, headers=None, dataGzip=False, dataJson=False):
    opener = urllib2.build_opener(urllib2.HTTPHandler)
...
def get(url, data=None, headers=None, dataGzip=False):
    opener = urllib2.build_opener(urllib2.HTTPHandler)
    data = data and urllib.urlencode(data) or ''
    url = '%s?%s' % (url, data)
    if dataGzip:
