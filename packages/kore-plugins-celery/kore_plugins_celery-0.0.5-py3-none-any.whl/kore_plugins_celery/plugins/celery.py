...
class CeleryPluginComponent(BasePluginComponent):
    ...
    def post_hook(self, container):
        ...
        task_always_eager = config.get('task_always_eager', False)
        task_eager_propagates = config.get('task_eager_propagates', False)
        ...
        worker_pool = config.get('worker_pool', 'prefork')
