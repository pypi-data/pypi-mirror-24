    ...
    **kwargs
    ...
    if not isinstance(MerchantID, str):
        ...
        try:
            ...
            raise Exception('Unable to transfer MerchantID to string')

    if not isinstance(MerchantOrderNo, str):
        try:
            MerchantOrderNo = str(MerchantOrderNo)
        except:
            raise Exception('Unable to transfer MerchantOrderNo to string')
    ...
    if not isinstance(Amt, int):
        ...
        try:
            ...
            raise Exception('Unable to transfer Amt to integer')

    if not isinstance(ItemDesc, str):
        try:
            ItemDesc = str(ItemDesc)
        except:
            raise Exception('Unable to transfer ItemDesc to string')
        if len(ItemDesc) > 50:
    ...
    if TimeStamp is None:
        ...
        try:
            ...
            warnings.warning('Unable to parse TimeStamp by fromtimestamp. Bypass then')
