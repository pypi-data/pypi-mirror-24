...
class TrackedSession(requests.Session):
    ...
    def cleanup(self):
        LOGGER.info("Made %d %s requests with %s seconds total delay", len(self.requests), self.__class__.__name__, self.delay)
...
class PrivilegedSession(TrackedSession):
    pass

class UserSession(TrackedSession):
    pass
...
def add_request_handler(app):
    ...
    def _show_session_usage(response):
        if SESSIONS.privileged_session:
            SESSIONS.privileged_session.cleanup()
        if SESSIONS.user_session:
