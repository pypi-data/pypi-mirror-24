...
class LLBModelTrainer(object):
    ...
    def test_model(self, classifier, test_sites, regions,
                   clinical_column, first_group="Above_6", use_clinical_column=True):
        clf_pred = classifier.predict(test_sites.transpose()[regions])

        test_classes = clin_col_to_binary(
            test_sites.transpose()[regions].merge(anndf, right_on="Sample_Name", left_index=True)[clin_col],
            first_class=first_group)

        print classification_report(test_classes, clf_pred)

        clf_proba = classifier.predict_proba(test_sites.transpose()[regions])
        print "AUC = ", metrics.roc_auc_score(test_classes, clf_proba[:, 1])

        fpr, tpr, thresholds = metrics.roc_curve(test_classes, clf_proba[:, 1])

        plt.figure()
        lw = 2
        plt.plot(fpr, tpr, color='darkorange',
                 lw=lw, label='ROC curve (area = %0.2f)' % metrics.roc_auc_score(test_classes, clf_proba[:, 1]))
        ...
        return zip(clf_proba, test_classes)
