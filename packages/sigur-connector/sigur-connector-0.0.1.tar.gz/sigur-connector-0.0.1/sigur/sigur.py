...
class Sigur:
    def __init__(self, login, pwd, host='127.0.0.1', port=3312):
        self.login = login
        self.pwd = pwd
        self.BUFFER = 65565
        ...
        self.main_log_lvl = logging.DEBUG
    ...
    def _login(self):
        ...
        """
        auth_str = '"LOGIN" 1.8 {} {}'.format(self.sphinx_login, self.sphinx_pwd)
    ...
    def get_ap_list(self)->list:
        ...
        return result.replace('APLIST', '').strip().split()
    ...
    def get_ap_state(self, ap_id: int)->dict:
        ...
        result.strip().split()
        # Sample result
        # APINFO ID 1 NAME "Турникет 1" ZONEA 18 ZONEB 0 STATE ONLINE_NORMAL CLOSED
        if len(result) != 12:
            raise SigurException('Wrong answer in get_ap_state - ' + ' '.join(result))
        return {'Name': result[4],
                'LinkState': 'ONLINE' if 'ONLINE' in result[-2] else 'OFFLINE'
                }
