...
class Gateway(object):
    ...
    def _register_outer_server_handlers(self):
        ...
        def create_conn(conn):
            ...
            conn.id = uuid.uuid4().bytes

            if ':' in conn.address[0]:
                conn.client_ip_num = socket.inet_pton(socket.AF_INET6, conn.address[0])
                conn.ipv6 = True
            else:
                conn.client_ip_num = socket.inet_pton(socket.AF_INET, conn.address[0])
                conn.ipv6 = False
