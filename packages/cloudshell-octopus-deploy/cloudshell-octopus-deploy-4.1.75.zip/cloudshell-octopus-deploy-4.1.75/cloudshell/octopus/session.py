...
class OctopusServer:
    ...
    def add_Variables_to_Environment_in_tenant(self, project_name, environment_name, tenant_name):
        ...
        self._valid_status_code(result, 'Failed to add environment to project for tenant; error: {0}'.format(result.text))
        revised_var_set = result.json()['ProjectVariables'][project['Id']]['Variables'][environment['Id']].update(
            PROF_ENV_VARS)
        result = requests.put(api_url, params=self.rest_params, json=revised_var_set)
        self._valid_status_code(result, 'Failed to add environment to project for tenant var; error: {0}'.format(result.text))
        return json.dumps(revised_var_set)
    ...
    def deploy_release(self, release_id, environment_id, tenant_id):
        """
