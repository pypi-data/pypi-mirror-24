...
AnalyseReponse
    ...
    d'exception dans un format lisible. Si aucune anomalie se trouve dans la
...
""")
    ...
    def get_message_xopmtom(self, cert, service, **kwargs):
        ...
        if service == "numUnique":
            return Template(self._message_xopmtom(["cert", "xml"])).render(
                boundary=BOUNDARIES,
            ...
            return Template(self._message_xopmtom(["cert"])).render(
                boundary=BOUNDARIES,
                service=service[0].upper() + service[1:],
                soaproot=SOAP_ROOT,
                soap=self.get_message_soap(
            ...
            return Template(self._message_xopmtom(["xml"])).render(
                boundary=BOUNDARIES,
                service=service[0].upper() + service[1:],
                soaproot=SOAP_ROOT,
                soap=self.get_message_soap(
...
class ErreurInattendueTraitementReponse(AnomaliesException):
    ...
    def __init__(self, service):
        ...
        if service == "doublons":
            message = (
                "Le serveur SNE a renvoyé l'erreur : 'Une erreur inattendue "
                "est survenue lors du traitement de la demande.'\nCette "
                "erreur connue est due au serveur SNE et apparaît lorsque la "
                "recherche de doublon concerne une association.\nSi tel est "
                "le cas, veuillez effectuer une recherche de doublon sur une "
                "personne physique.\nSinon, il s'agit probablement d'une "
                "erreur dans la structure XML."
            )
        else:
            message = (
                "Le serveur SNE a renvoyé l'erreur : 'Une erreur inattendue "
                "est survenue lors du traitement de la demande.'\nIl s'agit "
                "probablement d'une erreur dans la structure XML ou dans la "
                "librairie libsne."
            )
