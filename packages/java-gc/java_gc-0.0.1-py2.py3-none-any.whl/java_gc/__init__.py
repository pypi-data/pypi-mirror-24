...
def sighandler(signum, frame):
  logging.debug("Got signal %d, Terminating PID: %d" % (signum, jcmd_proc.pid))
  jcmd_proc.terminate()
...
def parse_cmdline():
  ...
  p.add_argument('-t', '--timeout', type=int, default=30, metavar='T', help='seconds to wait for GC before raising error')
