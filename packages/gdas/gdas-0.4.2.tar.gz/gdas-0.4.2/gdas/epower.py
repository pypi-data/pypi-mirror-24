    ...
    while t_idx_max <= len(ts_data):        
        ...
        for nc_sum in range(0, int(math.log(nchans, 2)))[::-1]:
            ...
            for j in [2**l for l in xrange(0, int(math.log(max_dof, 2)))]:
                ...
                if make_plot:
                    plot_spectrogram(dof_tiles.T,dt,df,ts_data.sample_rate/us_rate,start_time,end_time,
                                     fname='segments/%i-%i/tf_%02ichans_%02idof.png'%(start_time,end_time,nc_sum,2*j))
                    #plot_tiles(dof_tiles,2*j,df,ts_data.sample_rate/us_rate,start_time,end_time,
                    #           fname='segments/%i-%i/ts_%02ichans_%02idof.png'%(start_time,end_time,nc_sum,2*j))
