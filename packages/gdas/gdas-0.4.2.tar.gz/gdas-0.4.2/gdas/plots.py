...
def plot_spectrogram(spec,dt,df,sample_rate,t0,t1,fname="specgram.png"):
    plot = SpectrogramPlot()
    ax = plot.gca()
...
def plot_tiles(tdb,ndof,df,sample_rate,t0,t1,fname="tiles.png"):
    ...
    for i, tdf in enumerate(tdb):
        ...
        ax.plot(TimeSeries(tdf,epoch=float(t0),sample_rate=sample_rate))
        ax.set_xlim(t0,t1)
        if i>0:
