...
def get_covariates_raw(fichier,missing_value='NA'):
    ''' Get raw data from covariate file '''
    f=open(fichier)
...
def get_covariates_matrix(filename,fact_names,qcov_names,popnames):
    ''' 
    Returns a design matrix of all covariates listed in fact_names and qcov_names. 
    popnames : list of populations providing the required ordering of rows.
    '''
    ...
    for q in cofact:
        ...
        print 'Co-factor %s with %d levels'%(q,len(u))
        M=np.zeros((npop,len(u)),dtype=np.int)
        M[range(npop),aa]=1
        Matrices.append(M)
    for q in covar:
        M=np.zeros((npop,1),dtype=np.float)
	for i in range(npop):
            try:
                val=float(covariates[q][popnames[i]])
                M[i,0]=val
            except ValueError:
