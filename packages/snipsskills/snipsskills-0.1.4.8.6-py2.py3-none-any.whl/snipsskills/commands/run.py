...
class Run(Base):
    ...
    def run(self):
        ...
        print("Stopping the Snips Docker.")
        p1 = subprocess.Popen("docker stop snips".split())
        output, error = p1.communicate()
        
        print("Starting the Snips SDK.")
        subprocess.call("snips &> /dev/null &".split())
        
        # output, error = p1.communicate()
        # p1 = subprocess.Popen("snips".split(), stdout=DEVNULL)
        # output, error = p1.communicate()
        # os.system("snips &> /dev/null")

        print("[SKILLS] Starting the Snips Skills server.")
