...
class Systemd:
    ...
    def write_file(username, snips_home_path, snipsskills_path):
        ...
        with open(systemd_snipsskills_path, 'r') as systemd_snipsskills:
            ...
            local_output_filename = "{}@{}.service".format(SERVICE_NAME, username)
            with open(local_output_filename, "w") as output_file:
                ...
                command = "sudo cp {} {}".format(local_output_filename, system_output_filename)
                print(command)
    ...
    def enable_service(username):
        execute_command("sudo systemctl --system daemon-reload")
        execute_command("sudo systemctl enable {}@{}".format(SERVICE_NAME, username))
        execute_command("sudo systemctl start {}@{}".format(SERVICE_NAME, username))
