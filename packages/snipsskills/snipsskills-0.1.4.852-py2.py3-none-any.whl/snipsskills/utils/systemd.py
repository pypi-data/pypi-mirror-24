...
class Systemd:
    ...
    def write_file(username, snips_home_path, snipsskills_path):
        ...
        with open(template_filename, 'r') as template_file:
            ...
            execute_command("sudo echo \"{}\" > {}".format(
                contents, output_filename))
            execute_command("sudo chmod a+rwx {}".format(output_filename))

    @staticmethod
    def enable_service(username):
        execute_command("sudo systemctl --system daemon-reload")
        execute_command("sudo systemctl enable {}@{}".format(
            SERVICE_NAME, username))
        execute_command("sudo systemctl start {}@{}".format(
            SERVICE_NAME, username))
