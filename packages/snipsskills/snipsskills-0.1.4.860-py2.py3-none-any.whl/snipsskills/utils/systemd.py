...
class Systemd:
    ...
    @staticmethod
    def setup():
        (username, snips_home_path, snipsskills_path) = Systemd.get_snipsskills_params()
        Systemd.write_snips_file(username)
        Systemd.write_snipsskills_file(username, snips_home_path, snipsskills_path)
        Systemd.enable_services(username, SNIPS_SERVICE_NAME)
        Systemd.enable_services(username, SNIPSSKILLS_SERVICE_NAME)
