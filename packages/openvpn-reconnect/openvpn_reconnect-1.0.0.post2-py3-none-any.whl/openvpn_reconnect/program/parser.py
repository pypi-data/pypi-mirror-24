...
class Parser:
    def __init__(self):
        ...
        self.parser.add_argument('-i', '--interface', default=None, help='La interfaz por la que queremos enviar los paquetes ping para comprobar la conectividad del host.')
        self.parser.add_argument('-p', '--packets', type=int, default=5, help='El número de paquetes que enviaremos con ping para comprobar la conectividad del host.')
    ...
    def __parse_options(self):
        ...
        if (Statics.SECONDS // Statics.PACKETS) < 10:
            print('Error: Por cada paquete que se quiera enviar con ping, es necesario esperar al menos 10 segundos más para la siguiente iteración del programa, para evitar colapsar a peticiones '
                  'el servidor OpenVPN.', file=sys.stderr)
