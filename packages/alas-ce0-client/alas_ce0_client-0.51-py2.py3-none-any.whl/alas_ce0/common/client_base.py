...
class ApiClientBase(object):
    ...
    def http_get_json(self, url):
        if self.for_app_engine:
            ...
            response = requests.get(self.base_url + url, auth=self.requests_auth, verify=False)
    ...
    def http_put_json(self, url, obj):
        if self.for_app_engine:
            ...
            response = requests.put(self.base_url + url, auth=self.requests_auth, verify=False, json=obj)
            return self._process_response(response, response.content)
        else:
            with self._get_client() as _client:
    ...
    def http_post_json(self, url, obj):
        if self.for_app_engine:
            ...
            response = requests.post(self.base_url + url, auth=self.requests_auth, verify=False, json=obj)
            return self._process_response(response, response.content)
        else:
            with self._get_client() as _client:
    ...
    def http_delete(self, url):
        if self.for_app_engine:
            ...
            response = requests.delete(self.base_url + url, auth=self.requests_auth, verify=False)
            return self._process_response(response, response.content)
        else:
            with self._get_client() as _client:
