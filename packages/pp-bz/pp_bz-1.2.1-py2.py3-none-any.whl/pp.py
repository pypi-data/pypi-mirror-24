...
def copyToClip(password):
    ...
    if platform.system() == 'Linux':
        process = subprocess.Popen(['xsel', '-pi'], env={'LANG': 'en_US.UTF-8'}, stdin=subprocess.PIPE)
        process.communicate(password.encode('utf-8'))
        process = subprocess.Popen(['xsel', '-bi'], env={'LANG': 'en_US.UTF-8'}, stdin=subprocess.PIPE)
        process.communicate(password.encode('utf-8'))
    else:
