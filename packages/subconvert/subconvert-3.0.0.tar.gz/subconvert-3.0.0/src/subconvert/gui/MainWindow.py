...
from subconvert.gui.Detail import ask_about_changes
...
class MainWindow(QMainWindow):
    ...
    def __connectSignals(self):
        ...
        self._tabs.tabChanged.connect(self._openLinkedVideo)
        ...
        self._subtitleData.fileChanged.connect(self._openLinkedVideo)
        ...
        self._tabs.fileList.requestSaveAndRemove.connect(self.saveAndRemove)
    ...
    def __initActions(self):
        ...
        self._actions["exit"] = af.create(
            "application-exit", _("&Exit"), _("Exit Subconvert."), None, self.close)
        ...
        self._actions["frameStepFwd"] = af.create(
            None, _("Next &frame"), _("Go to the next frame in a video"), ".",
            self._videoWidget.nextFrame)
        self._actions["frameStepBwd"] = af.create(
            None, _("Previous f&rame"), _("Go to the previous frame in a video"), ",",
            self._videoWidget.prevFrame)
    ...
    def __initMenuBar(self):
        ...
        playbackMenu.addAction(self._actions["frameStepFwd"])
        playbackMenu.addAction(self._actions["frameStepBwd"])
        videoMenu.addSeparator()
    ...
    def _writeFile(self, filePath, newFilePath=None):
        ...
        content = data.outputFormat(subtitles=data.subtitles).output
    ...
    def closeEvent(self, ev):
        ...
        ch, modified = ask_about_changes(self._subtitleData,
                                         self._tabs.fileList.filePaths,
                                         self)
        ev.ignore()
        if ch == QMessageBox.Cancel:
            return
        elif ch == QMessageBox.Save:
            if not self.saveAll(modified):
                return
        ev.accept()
    ...
    def saveFileAs(self):
        ...
        if fileDialog.exec():
            ...
            ret = self.saveFile(newFilePath)
            self._settings.setLatestDirectory(os.path.dirname(newFilePath))
            return ret
        return False

    def saveAll(self, paths=None):
        dialog = MessageBoxWithList(self)
        dialog.setIcon(QMessageBox.Critical)

        if not paths:
            paths = self._tabs.fileList.filePaths
    ...
    @pyqtSlot(list, list)
    def saveAndRemove(self, to_save, to_remove):
        if self.saveAll(to_save):
            for path in to_remove:
                self._tabs.removeFile(path)
    ...
    def offset(self):
        ...
        dialog = OffsetDialog(self, FrameTime(0, 25))
        if dialog.exec():
            if currentTab.isStatic:
                currentTab.offsetSelectedFiles(dialog.frametime.ms)
            else:
                currentTab.offset(dialog.frametime.ms)
    ...
    def openHelp(self):
        url = "https://mgoral.gitlab.io/subconvert"
    ...
    def _openLinkedVideo(self):
        ...
        if not data.videoPath or data.videoPath == self._videoWidget.videoPath:
            return

        self._videoWidget.openFile(data.videoPath)
