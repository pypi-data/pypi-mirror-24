...
class MessageBoxWithList(QMessageBox):
    ...
    def exec(self):
        if self._details:
            self.setDetailedText(self._details)
        super().exec()
...
def ask_about_changes(model, files, parent=None):
    modified = [f for f in files if not model.isCleanState(f)]
    if modified:
        dialog = QMessageBox(parent)
        dialog.setIcon(QMessageBox.Question)
        dialog.setWindowTitle(_('Unsaved changes'))
        dialog.setText(_('There are unsaved changes that will be lost. Continue?'))
        dialog.setStandardButtons(QMessageBox.Save | QMessageBox.Discard | \
                                  QMessageBox.Cancel)
        dialog.setDefaultButton(QMessageBox.Cancel)
        return dialog.exec(), modified
    return QMessageBox.Discard, modified
