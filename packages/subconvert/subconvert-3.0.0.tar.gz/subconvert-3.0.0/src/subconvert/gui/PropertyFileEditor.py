...
class PropertyFileEditor(QDialog):
    ...
    def __initGui(self):
        ...
        self._changeFormat.toggled.connect(self._outputFormat.setEnabled)
    ...
    def _createFpsBox(self):
        ...
        self._autoFps.setChecked(True)

        self._fps = QComboBox(self)
        self._fps.addItems(["23.976", "24", "25", "29.97", "30"])
        self._fps.setEditable(True)
        fpsLabel = QLabel(_("Default FPS value"))

        layout.addWidget(self._autoFps, 0, 0)
        layout.addWidget(self._fps, 1, 0)
        layout.addWidget(fpsLabel, 1, 1)
    ...
    def _createEncodingBox(self):
        ...
        self._autoEncoding.setChecked(True)
    ...
    def _createFormatBox(self):
        ...
        self._changeFormat = QCheckBox(_("Change output format"), self)
        self._changeFormat.setChecked(False)
        self._outputFormat = QComboBox(self)
        self._outputFormat.addItems(displayedFormats)
        self._outputFormat.setEnabled(self._changeFormat.isChecked())
        formatLabel = QLabel(_("Output format"))

        layout.addWidget(self._changeFormat, 0, 0)
        layout.addWidget(self._outputFormat, 1, 0)
        layout.addWidget(formatLabel, 1, 1)
        groupbox.setLayout(layout)
        return groupbox
    ...
    def _createSubtitleProperties(self):
        ...
        subProperties.changeFormat = self._changeFormat.isChecked()
    ...
    def changeProperties(self, subProperties):
        ...
        self._changeFormat.setChecked(subProperties.changeFormat)
