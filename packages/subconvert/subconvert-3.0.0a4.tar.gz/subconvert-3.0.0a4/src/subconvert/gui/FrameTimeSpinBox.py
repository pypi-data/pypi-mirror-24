...
class FrameTimeSpinBox(QAbstractSpinBox):
    ...
    def setFrameTime(self, frameTime):
        ...
        fps = frameTime.fps
        self._miliStep = FrameTime.InitTimeStr("0:00:00.001", fps)
        self._secondStep = FrameTime.InitTimeStr("0:00:01.000", fps)
        self._minuteStep = FrameTime.InitTimeStr("0:01:00.000", fps)
        self._hourStep = FrameTime.InitTimeStr("1:00:00.000", fps)
    ...
    def _validateLineEdit(self):
        ...
        if text != self._frameTime.toStr():
            ...
            if isValid == QValidator.Acceptable:
                ...
                self._frameTime = FrameTime.InitTimeStr(text, fps)
            else:
...
class _FrameTimeValidator(QValidator):
    def validate(self, input_, pos):
        try:
            ft = FrameTime.InitTimeStr(input_, 1)
