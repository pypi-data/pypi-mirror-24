...
class MicroDVD(SubFormat):
    ...
    def frametime(self, fps, string):
        return FrameTime.InitFrames(string, fps)
...
class SubRip(SubFormat):
    ...
    def frametime(self, fps, string):
        ...
        tstr = "%d:%02d:%02d.%03d" % (int(time.group('h')), 
                                      int(time.group('m')),
                                      int(time.group('s')),
                                      int(time.group('ms')))
        return FrameTime.InitTimeStr(tstr, fps)
...
class TMP(SubFormat):
    ...
    def frametime(self, fps, string):
        if string:
            time = self.time_fmt.search(string)
            tstr = "%d:%02d:%02d" % (int(time.group('h')),
                                     int(time.group('m')),
                                     int(time.group('s')))
            return FrameTime.InitTimeStr(tstr, fps)
        return None
...
class MPL2(SubFormat):
    ...
    def frametime(self, fps, string):
        ...
        return FrameTime(1000 * seconds + ms, fps)
