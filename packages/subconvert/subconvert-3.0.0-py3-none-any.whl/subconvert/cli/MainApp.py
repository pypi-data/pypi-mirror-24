...
from subconvert.parsing.Core import SubParsingError
...
class SubApplication:
    ...
    def getOutputFormat(self):
        ...
        elif self._args.pfile.changeFormat and self._args.pfile.outputFormat.NAME != SubFormat.NAME: # fmt != 'Generic'
    ...
    def printSubtitles(self, convertedSubtitles, encoding):
        for line in convertedSubtitles:
    ...
    def run(self):
        try:
            ...
            for filePath in self._args.files:
                ...
                if data is not None:
                    out  = data.outputFormat(subtitles=data.subtitles).output
                    outputFilePath = self._parsePathTemplate(self._args.outputPath, subFile.path)
                    if outputFilePath:
                        self.writeSubtitles(out, outputFilePath, data.outputEncoding)
                    else:
                        self.printSubtitles(out, data.outputEncoding)
    ...
    def printData(self, filePath, data):
        outFmtName = data.outputFormat.NAME if data.outputFormat \
                                            else _('not changed')
        ...
        log.debug(_("Output format   : %s") % outFmtName)
