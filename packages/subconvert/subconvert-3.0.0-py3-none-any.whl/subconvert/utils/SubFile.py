...
class File:
    ...
    def read(self, encoding = None):
        ...
        try:
            with open(self._filePath, mode='r', encoding=encoding) as file_:
                return list(_preprocess_line(line) for line in file_)
    ...
    def detectFpsFromMovie(cls, movie, default = 23.976):
        ...
        if not track:
            if movie:
                log.warning(_("Could not find a movie track in a video: %s") % movie)
        else:
...
def _preprocess_line(line):
    # Universal newlines mode (enabled by default) allows us to only check the
    # last character
    if line.endswith('\n'):
        return line[:-1]
    return line
