...
class FileList(SubTab):
    ...
    requestSaveAndRemove = pyqtSignal(list, list)
    ...
    def __initContextMenu(self):
        ...
        # Key shortcuts are actually only a hack to provide some kind of info to
        # user that he can use "enter/return" and "delete" to open/close
        # subtitles. Keyboard is handled via keyPressed -> _handleKeyPress. This
        # is because __fileList has focus most of time anyway (I think...)

        actionOpenInTab = af.create(
            icon="window-new", title=_("&Open in tab"), shortcut="Enter",
            connection=lambda: self._handleKeyPress(Qt.Key_Enter))
        actionOpenInTab.setEnabled(anyItemSelected)
        self._contextMenu.addAction(actionOpenInTab)

        actionClose = af.create(
            icon='window-close', title=_("&Close subtitles"), shortcut="Delete",
            connection=lambda: self._handleKeyPress(Qt.Key_Delete))
    ...
    def _handleKeyPress(self, key):
        ...
        if key in (Qt.Key_Enter, Qt.Key_Return):
            ...
            files = [item.text(0) for item in items]
            ch, modified = ask_about_changes(self._subtitleData, files, self)
            if ch == QMessageBox.Cancel:
                return
            elif ch == QMessageBox.Save:
                self.requestSaveAndRemove.emit(modified, files)
                return

            for path in files:
                self.requestRemove.emit(path)
    ...
    def _offsetDialog(self):
        ...
        if dialog.exec():
            self.offsetSelectedFiles(dialog.frametime.ms)

    def offsetSelectedFiles(self, ms):
        if ms == 0:
            return
...
class SubtitleEditor(SubTab):
    ...
    def _createNewSubtitle(self, data, subNo):
        ...
        if subNo < data.subtitles.size():
            try:
                ...
                timeEnd = FrameTime.InitFrames(0, fps)
        else:
            timeEnd = timeStart + FrameTime.InitFrames(50, fps)
    ...
    def offset(self, ms):
        ...
        if fps is None:
            ...
            return
        ft = FrameTime(ms, data.subtitles.fps)
        data.subtitles.offset(ft)
        command = ChangeData(self.filePath, data,
                             _("Offset by: %s") % ft.toStr())
        self._subtitleData.execute(command)
...
class SearchBar(QWidget):
    ...
    def __init__(self, parent):
        ...
        self._prevButton = QPushButton(QIcon.fromTheme("go-previous"), "", self)
        self._nextButton = QPushButton(QIcon.fromTheme("go-next"), "", self)
