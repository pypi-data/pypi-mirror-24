...
class Match(Base):
    def __init__(self, left, right, name=None):
        if not name:
            left_name = left.name if isinstance(left, Base) else left
            right_name = right.name if isinstance(right, Base) else right
            name = 'match_' + left_name + '_and_' + right_name
        super(Match, self).__init__(column=None, name=name)
        self.left = left
        self.right = right
    ...
    def transform_datum(self, datum):
        if not self.datum[self.left]:
            if not self.datum[self.right]:
                return 0
            return 1
        if not self.datum[self.right]:
            return 2
        if self.datum[self.left] == self.datum[self.right]:
            return 3
        return 4
...
class Token(Unique):
    ...
    """
    PUNCTUATION_FILTER = re.compile('\W+\s\W+|\W+(\s|$)|(\s|^)\W+', re.UNICODE)
...
class Glove(Token):
    ...
    def fit(self, data):
        ...
        with lore.util.timer("Loading GloVe parameters: %s" % (local)):
