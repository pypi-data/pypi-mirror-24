...
class Match(Base):
    """ Provides elementwise comparison of left and right, see also numpy.equal
    """
    def __init__(self, column, other, name=None):
        ...
        super(Match, self).__init__(column=column, name=name)
        self.other = other
    ...
    def transform(self, data):
        with timer(('transform %s:' % self.name), logging.DEBUG):
            return numpy.equal(self.series(data), self.other_series(data))
...
class Discrete(Base):
    ...
    def transform_datum(self, datum):
        if datum is None or \
            ...
            limited = min(self.__range, max(self.zero, datum - self.__min))
            normed = limited * self.__norm
            binned = normed // self.__range
            return binned
