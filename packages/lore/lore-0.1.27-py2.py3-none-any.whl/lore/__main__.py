...
def install_requirements(args):
    ...
    if not os.path.exists(source):
        print(
            ansi.warning() + ' %s is missing. You should check it in to version '
...
def freeze_requirements():
    ...
    if not os.path.isfile(source):
        print(ansi.success('COPY') + ' ' + destination + ' -> ' + source)
    ...
    if added_index:
        ...
        present = present[0:added_index - 1]
        safe = []
        unsafe = []
    ...
    if unsafe:
        if vcs:
            print(
                ...
                ansi.underline('requirements.in.txt') + ' that can not be '
                'frozen by pip. ' + os.linesep + os.linesep +
                os.linesep.join(vcs)
            )
    
        print(
            ansi.info() + ' You should check the following packages in to ' +
            ansi.underline('requirements.in.txt') + ' or `lore pip uninstall` them'
        )
        if vcs:
    ...
    with open(destination, 'w', encoding='utf-8') as f:
        f.write(
            '# Versions are locked in requirements.txt' + os.linesep +
            '# This file is automatically maintained by `lore install`.' + os.linesep +
            '# Prefer editing minimal requirements.in.txt or `lore install --upgrade`.' + os.linesep
        )
...
def split_vcs_lines():
    with open(env.requirements_in, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    ...
    with open(env.requirements_in, 'w', encoding='utf-8') as f:
