...
def launch_virtual_env():
    ...
    if not os.path.exists(env.prefix):
        ...
        # TODO gather lore version from env.requirements first
...
def freeze_requirements():
    ...
    frozen = frozen.decode('utf-8').split(os.linesep)
    missing = missing.decode('utf-8').split(os.linesep)
    ...
    if extras:
        print(
            ansi.warning() + ' Packages are installed that are not listed '
            'in ' + ansi.underline(os.path.basename(env.requirements_in)) +
            ':' + os.linesep + '  ' +
            (os.linesep + '  ').join(frozen[extras+1:-1])
        )

        if vcs:
            print(
                ansi.info() + ' The previously listed packages should be '
                'added unless they are from the following non pypi '
                'requirements:' + os.linesep + '  ' +
                (os.linesep + '  ').join(vcs) + os.linesep
            )

        frozen = frozen[:extras]
    
    frozen = [f for f in frozen if not f.startswith('#')]
    with open(destination, 'w') as f:
        f.write(
            '# DO NOT EDIT requirements.txt\n# This file is automatically '
            'maintained by `lore install`.\n# Edit requirements.in.txt '
            'instead.' + os.linesep
        )
        f.write(os.linesep.join(vcs) + os.linesep)
        f.write(os.linesep.join(frozen).strip() + os.linesep)


def split_vcs_lines():
    with open(env.requirements_in, 'r') as f:
        lines = f.read().decode('utf-8').split(os.linesep)
    ...
    if os.path.exists(env.requirements_vcs):
        with open(env.requirements_vcs, 'r') as f:
            new = set(f.read().decode('utf-8').split(os.linesep))
            vcs = list(set(vcs).union(new))
    
    lines = list(set(lines) - set(vcs))
    with open(env.requirements_in, 'w') as f:
        f.write(os.linesep.join(sorted(lines)))
    
    with open(env.requirements_vcs, 'w') as f:
        f.write(os.linesep.join(sorted(vcs)))
    return vcs
...
def restore_vcs_lines(vcs):
    ...
    with open(env.requirements_in, 'w') as f:
        f.write(os.linesep.join(vcs) + os.linesep + original)
...
def check_requirements():
    ...
    with open(env.requirements, 'r') as f:
