...
def freeze_requirements():
    ...
    if extras:
        if vcs:
            print(
                ansi.info() + ' Extra installed packages are assumed to come '
                'from the following non pypi requirements:\n\n' +
                os.linesep.join(vcs) + '\n\nThese packages should account for '
                'all of the following:\n\n' + os.linesep.join(frozen[extras:-1])
            )
        else:
            print(
                ansi.warning() + ' Packages are installed that are not listed '
                'in ' + ansi.underline('requirements.in.txt') + ':\n' +
                '\n  '.join(frozen[extras:-1])
            )
