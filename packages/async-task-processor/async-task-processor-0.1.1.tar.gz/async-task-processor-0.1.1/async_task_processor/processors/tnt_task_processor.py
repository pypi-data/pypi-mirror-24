...
class TntTaskProcessor:
    ...
    def __init__(self, atp, tarantool_host='localhost', tarantool_port=8123, tarantool_user=None,
                 tarantool_password=None, encoding='utf-8'):
        """Tarantool queue handler.

        Listen tarantool queue and if data there run async function.

        :param tarantool_host: tarantool host
        :param tarantool_port: tarantool port
        :param tarantool_user: tarantool user
        :param tarantool_password: tarantool password
        :param encoding: tarantool data encoding
        """
        self._atp = atp
        self._loop = atp.loop
        self._queue = aiotarantool_queue.Queue(host=tarantool_host, port=tarantool_port, user=tarantool_user,
                                               password=tarantool_password, loop=self._loop, encoding=encoding)
    ...
    def add_task(self, queue, foo, args=None, bind=None, max_workers=1, timeout=0, max_retries=0, retry_countdown=0):
        ...
        :type queue: str
        :param foo: Function that will work asynchronously
