...
class EvalAPI(Resource):
    ...
    def get(self, eval_id):
        ...
        if not eval_:
            error = {
                    'code': 404,
                    'message': f"Evaluation id:{exercise_id} does not exist",
                    'status': 'NOT_FOUND'
                    }
            abort(**error)
        return eval_
...
def booleanStringParser(string):
...
def updateQuestion(quest):
    ...
    qu.update({
            ...
            'is_leveled': booleanStringParser(quest['is_leveled']),
