        ...
        def test_font(font):
            if __debug__:
                with self.assertRaises(TecplotValueError):
                    font.size_units = Units.Grid

            self.internal_test_text_font_round_trip(font)

        test_font(self.line_legend.font)
        test_font(self.contour_legend.header_font)
        test_font(self.contour_legend.number_font)
...
class TestLegendFont(unittest.TestCase):
