...
class ReferenceVector(session.Style):
    ...
    .. code-block:: python
        ...
        infile = path.join(exdir, '2D', 'Flow.lpk')
        ...
        vector_contour.variable = dataset.variable('R/RFR')
        vector_contour.colormap_name = 'Magma'
        vector_contour.colormap_filter.reversed = True

        vorticity_contour = plot.contour(1)
        vorticity_contour.variable = dataset.variable('Vorticity')
        vorticity_contour.colormap_name = 'GrayScale'
        vorticity_contour.colormap_filter.reversed = True

        vector = plot.vector
        vector.u_variable = dataset.variable('U/RFC')
        vector.v_variable = dataset.variable('V/RFC')
        vector.relative_length = 0.4
        vector.arrowhead_size = 0.3
        ...
        fmap.contour.flood_contour_group = vorticity_contour
...
class Vector2D(Vector):
    ...
    .. code-block:: python
        ...
        exdir = tp.session.tecplot_examples_directory()
        infile = path.join(exdir, '2D', '3element.lpk')
        tp.load_layout(infile)
