...
namespace SeqLib {
  ...
  void BWAWrapper::SetMinChainWeight(int min_chain_weight) {
    if (min_chain_weight < 0) 
      throw std::invalid_argument("BWAWrapper::SetMinChainWeight - min_chain_weight must be non-negative");
    memopt->min_chain_weight = min_chain_weight;
  }

  void BWAWrapper::SetMinSeedLength(int min_seed_len) {
    if (min_seed_len <= 0) 
      throw std::invalid_argument("BWAWrapper::SetMinSeedLength - min_seed_len must be > zero");
    memopt->min_seed_len = min_seed_len;
  }
