...
class CSFtpMoveFmHost:
    ...
    def __init__(self, dictHPUP):
        ...
        PrintTimeMsg('CSFtpMoveFmHost.connect(%s)...' % sHostPort)
        try:
            ...
            PrintTimeMsg('CSFtpMoveFmHost.connect.Error=(%s)' % str(e))
            sys.exit(-101)
        if not self.ssh:
            PrintTimeMsg('CSFtpMoveFmHost.connect.ssh=(%s)' % self.ssh)
            sys.exit(-1)
    ...
    def __del__(self):
        ...
        if hasattr(self, 'ssh'):
            self.ssh.close()
        PrintTimeMsg('CSFtpMoveFmHost.__del__')
    ...
    def sftpListDir(self, sDir):
        ...
        for oDir in lsDir:
            PrintTimeMsg("sftpListDir(%s): %s=%s=" % (sDir, oDir, oDir.st_mode))
    ...
    def sftpDownDeleteFile(self, sFileName, sDirRemote, sDirLocal):
        # 从主机上下载移除一个文件
        try:
            self.sHintFileName = 'get(%s/%s)' % (sDirRemote, sFileName)
            sR = sDirRemote + '/' + sFileName
            sL = sDirLocal + os.path.sep + sFileName
            sT = sL + '.filepart'  # 临时文件
            ...
            PrintTimeMsg('sftpDownDeleteFile.sftp.get(%s,%s).Error=(%s)' % (
                sR, sT, str(e)))
            sys.exit(-102)
        else:
            PrintTimeMsg("sftpDownDeleteFile.sftpDownDelete(%s)=OK!" % sFileName)
        finally:
            try:
    ...
    def sftpDownDeleteDir(self, sDirRemote, sDirLocal):
        ...
        for oDir in lsDir:
            if stat.S_ISREG(oDir.st_mode):  # 一般文件
                ...
                self.sftpDownDeleteFile(oDir.filename, sDirRemote, sDirLocal)
