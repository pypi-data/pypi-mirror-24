...
class Client(object):
    ...
    def get_node_boot_info(self, node_id):
        ...
        if response['status_code'] == 200:
            ...
            product_name = response['body']['productName'].upper()
            ...
            # for mt in SUPPORTED_MACHINE_TYPES:
            #     if product_name.contains(mt.upper()):
            #         machine_type = mt
            #         break
#
            # if machine_type != 'Unknown':
            #     return boot_devices[machine_type][first_boot_item]
            # else:
            #     raise FailToSetBootInfoException(node_id=node_id)
    ...
    """
    def _set_real_boot_device(self, machine_type, boot_order, singleuse):
        for item in boot_order['bootOrder']['bootOrderList']:
            if item.get('possibleBootOrderDevices', None) is not None:
                del (item['possibleBootOrderDevices'])

            if item['bootType'].lower() == 'singleuse':
                singleuse_item = item
            elif item['bootType'].lower() == 'permanent':
                permanent_item = item
        ...
        current[0] = BOOT_DEVICES[machine_type][current[0]]
        print("After change, the real boot device is %s" % current[0])
        return boot_order
    ...
    """
        ...
        :param singleuse, Bool, point out this is configured to
                singleuse item (True) or permanent item
    """
    def set_node_boot_info(self, node_id, boot_order, singleuse):
        try:
            ...
            input_boot_order = self._set_real_boot_device(machine_type, boot_order, singleuse)
