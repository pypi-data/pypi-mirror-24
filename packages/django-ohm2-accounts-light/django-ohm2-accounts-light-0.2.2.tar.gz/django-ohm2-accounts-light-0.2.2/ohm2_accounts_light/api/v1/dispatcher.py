...
def process_social_login_and_get_token(request, username, email, **options):
	...
	if user is None:
		...
		ret, error = signup(request, params, **options)
		if error:
...
def facebook_login_and_get_token(request, params):
	...
	try:
		me = graph.get_object(id = 'me?fields=email,picture.type(large)')
	except Exception as e:
	...
	picture_data = me.get("picture", {}).get("data")
	if picture_data:
		url = picture_data.get("url")
		if url:
			options["profile_url"] = url
