...
class Segment(object):
    def __init__(self, form, features):
        assert isinstance(form, StringTypes)
        self.form = form
        assert isinstance(features, DictType)
        self.features = features

    def __repr__(self):
        return 'Segment("{}", {})'.format(self.form, repr(self.features)).encode('utf-8')
...
class Diacritic(object):
    def __init__(self, marker, position, conditions, exclude, content):
        ...
        assert isinstance(exclude, ListType)
        self.exclude = exclude
        assert isinstance(conditions, ListType)
        self.conditions = conditions
        assert isinstance(content, DictType)
...
def sort_all_segments(sort_order, all_segments):
    ...
    for field in field_order:
        all_segments_list.sort(key=lambda seg: seg.features[field['name']], reverse=field['reverse'])
