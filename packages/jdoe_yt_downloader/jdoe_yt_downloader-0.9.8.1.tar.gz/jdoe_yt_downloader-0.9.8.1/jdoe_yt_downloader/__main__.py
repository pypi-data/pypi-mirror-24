...
def main(args=None):
	...
	parser.add_argument(
		"--list", help="Txt file with video titles to download", default=None)
...
def progress(count, blockSize, totalSize):
	...
	if totalSize <= 0:
		...
		print("Progressbar is not available (totalSize < 0)")
		return
...
def youtube_search(options):
	...
	if options.list != None:
		...
		DOWNLOADS_FOLDER_PATH = os.path.dirname(file.name) # Set the download path to be the same folder the list is in
		...
		for search_result in search_response.get("items", []):
			if search_result["id"]["kind"] == "youtube#video":
				...
				try:
					...
					print("Direct link: %s" % Url)

				list_watcher.remove(raw_old_title)
				with open(options.list, "w") as file:
