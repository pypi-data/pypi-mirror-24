...
class Connection(object):
    ...
    # 发送buf，也是因为kcp底层限制了send buf大小，而且send要不就全失败，要不就全成功
    _write_buffer = None
    ...
    def kcp_check(self):
        ...
        self.kcp_obj.timer.set(interval / 1000.0, self._on_kcp_timer_ticked, force=True)
    ...
    def _handle_write_buf(self):
        # 返回发送成功的长度
        ret = self.kcp_obj.send(self._write_buffer)
        if ret > 0:
            # 还是为了性能
            if ret == len(self._write_buffer):
                self._write_buffer = ''
            else:
                self._write_buffer = self._write_buffer[ret:]
