        ...
        try:
            ...
            for item in result:
                ...
                if 'product_type' in chunk:
                    try:
                        p_type = ProductType.objects.get(title=chunk['product_type'])
                ...
                if 'vendor' in chunk:
                    try:
                        vendor = Vendor.objects.get(title=chunk['vendor'])
                    except Vendor.DoesNotExist:
                        vendor = Vendor(
                            title=chunk['vendor'],
                            slug=get_unique_slug(chunk['vendor'], Vendor),
                        )
                        vendor.save()
                ...
                if 'collection' in chunk:
                    try:
                        collection = Collection.objects.get(title=chunk['collection'])
