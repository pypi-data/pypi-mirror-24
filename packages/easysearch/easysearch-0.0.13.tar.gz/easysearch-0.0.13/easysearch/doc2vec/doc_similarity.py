...
class DocumentSimilarity(object):
    ...
    def train(self):
        ...
        # PV-DBOW
        model = Doc2Vec(dm=0, dbow_words=1, size=300, window=8, min_count=2, iter=100, workers=self.cores)
    ...
    def save_documents_to_csv(self, solr_client, total_docs, step=500):
        ...
        with open(filename, 'w', encoding='utf-8') as csvfile:
            ...
            for i in range(0, total_docs, step):
                ...
                for doc in res.docs:
                    try:
                        writer.writerow([doc['ctr_id'], ' '.join(text.get_text_for_doc2vec(doc, self.text_fields))])
