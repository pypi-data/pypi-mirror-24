...
class DocumentSimilarity(object):
    ...
    def save_documents_to_csv(self, solr_client, total_docs, step=500):
        ...
        with open(filename, 'w') as csvfile:
            writer = unicodecsv.writer(csvfile, delimiter=' ', quotechar='\'', quoting=unicodecsv.QUOTE_MINIMAL)
            start = 0
            rows = step
            end = rows
    ...
    def make_labeled_csv(self, filename):
        with open(filename) as f:
            r = unicodecsv.reader(f, delimiter=' ', quotechar='\'', quoting=unicodecsv.QUOTE_MINIMAL)
            for row in r:
