...
def add_condition_to_graph(graph, condition, file_name):
    ...
    if condition.expression:
        ...
        for prop in xml_parser.iterfind("simpleConditionExpression"):
            ...
            add_property_edge_unless_exists(graph, guid, reference, prop)
...
def add_template_to_graph(graph, template):
    ...
    if template.data:
        ...
        for prop in xml_parser.bound_properties():
            ...
            add_property_edge_if_exists(graph, template.guid, reference, l_dict)
...
def add_property_edge_unless_exists(graph, parent, prop, attrs):
    """Only add property edge if it doesn't exist

    Otherwise we end up with multiple references that
    don't add any real value to our relationship graph
    """
    base_prop = prop.rsplit(".")[-1]
    if not graph.has_edge(parent, base_prop):
        graph.add_edge(parent, base_prop, attr_dict=attrs)
