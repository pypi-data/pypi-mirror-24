import pandas as pd
import numpy as np
from ModelBuild import model_coefficients
from ModelPrediction import base_data_creator

#function to get the churn percentages
def churn_percentage_matrix(model, coefficients = None, Month = 'March', Year = 2016, No_of_Years = 3):
    """
    ...
    if coefficients is None:
        coeffs = model_coefficients(model, Year = Year, No_of_Years = No_of_Years)
        ...
        for year in range(No_of_Years):
            for i in range(len(months)-Month):
                ...
                percentage_matrix = np.concatenate((percentage_matrix, [model.predict(coeffs, exog = base)]), axis = 0)
...
def customers_on_base(base_customers, percentage_matrix, Month = 'March', Year = 2016, No_of_Years = 3):
    ...
    for j in range(len(temp)-1):
        for k in range(30):
            if k+1 == 30:
                temp[j+1][k+1] = temp[j][k+1] * percentage_matrix[j+1][k+1] + temp[j][k] * percentage_matrix[j+1][k]
            else:
