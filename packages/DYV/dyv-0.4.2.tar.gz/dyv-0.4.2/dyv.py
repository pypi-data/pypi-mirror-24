...
@click.option('--show-version', is_flag=True, callback=print_version, expose_value=False, is_eager=True,
...
def __execute_git_command(cmd, git_url, git_dest, git_branch, git_depth, folder_name=None):
    ...
    if cmd == 'clone':
        ...
        if folder_name:
            os.chdir(os.path.join(git_dest, folder_name))
        cmds = ['git', 'log', '--pretty=format:"%h%x09%x09%ad%x09%s', '-4', '--date', 'iso']
    click.secho('Current path : %s' % os.getcwd(), fg='yellow')
...
@click.pass_context
def clone(ctx, repo, path, branch, all, depth):
    """Clone a repository"""
    all_keys = __get_all_keys(ctx)
    repo_arg = repo
    ...
    click.echo("Clone the repository %s to %s" % (repo.get('path'), path))
    click.echo("Branches %s, depth %s " % (branch_names, depth))
    ...
    for br_path, br_name in branches:
        ...
        click.echo("Clone the branch %s to %s" % (br_name, branch_path))
        __execute_git_command('clone', repo.get('path'), branch_path, br_name, depth, folder_name=repo_arg)
...
@click.option('--depth', type=click.INT, default=1)
@click.pass_context
def pull(ctx, repo, path, branch, all, depth):
    """pull a repository"""
    all_keys = __get_all_keys(ctx)
    repo_arg = repo
    repo = all_keys.get('repos', {}).get(repo_arg, False)
    ...
    branch_paths = [x[0] for x in branches]
    branch_names = [x[1] for x in branches]
    click.echo("Pull the repository %s from %s" % (repo.get('path'), path))
    click.echo("Branches %s, depth %s " % (branch_names, depth))
    if not os.path.isdir(path):
        click.secho('The path %s is not found' % path, fg='red')
        return
    ...
    for br_path, br_name in branches:
        branch_path = os.path.join(path, br_path)
        click.echo("Pull the branch %s to %s" % (br_name, branch_path))
        __execute_git_command('pull', repo.get('path'), branch_path, br_name, depth, folder_name=repo_arg)
...
@click.option('--depth', type=click.INT, default=1)
@click.pass_context
def status(ctx, repo, path, branch, all, depth):
    ...
    branch_paths = [x[0] for x in branches]
    branch_names = [x[1] for x in branches]
    click.echo("Status of the repository %s in %s" % (repo.get('path'), path))
    click.echo("Branches %s, depth %s " % (branch_names, depth))
    if not os.path.isdir(path):
        click.secho('The path %s is not found' % path, fg='red')
        return
    ...
    for br_path, br_name in branches:
        branch_path = os.path.join(path, br_path)
        click.echo("Status of the branch %s in %s" % (br_name, branch_path))
        __execute_git_command('status', repo.get('path'), branch_path, br_name, depth, folder_name=repo_arg)
...
@click.option('--depth', type=click.INT, default=1)
@click.pass_context
def log(ctx, repo, path, branch, all, depth):
    """Show log  of a repository"""
    all_keys = __get_all_keys(ctx)
    repo_arg = repo
    repo = all_keys.get('repos', {}).get(repo_arg, False)
    if not repo:
    ...
    branch_paths = [x[0] for x in branches]
    branch_names = [x[1] for x in branches]
    click.echo("Log of the repository %s in %s" % (repo.get('path'), path))
    click.echo("Branches %s, depth %s " % (branch_names, depth))
    if not os.path.isdir(path):
        click.secho('The path %s is not found' % path, fg='red')
        return
    ...
    for br_path, br_name in branches:
        branch_path = os.path.join(path, br_path)
        click.echo("Logs of the branch %s in %s" % (br_name, branch_path))
        __execute_git_command('log', repo.get('path'), branch_path, br_name, depth, folder_name=repo_arg)
