...
@click.option('--version', '-v', type=click.INT, default=10, help="The version of Odoo")
...
def fix_addon_version(_ap):
    _ap_full = os.path.join(_ap, '__openerp__.py')
    if os.path.isfile(_ap_full):
        ___set_openerp()
...
@cli.command()
@click.argument('addon', type=click.STRING, required=False)
@click.option('model', '-m', type=click.STRING, default=None, required=False)
@click.pass_context
def models(ctx, addon, model):
    """Show addon models"""
    all_keys = __get_all_keys(ctx)
    ...
    ast_models = astobj.get_models(model_arg=model)
    x = PrettyTable()
...
def fields(ctx, addon, model):
    ...
    for ast_model, ast_model_data in ast_models:
        ...
        x = PrettyTable()
        x.field_names = ['Field', 'Type', 'Main', 'Required', 'Compute', 'Inverse', 'Search', 'Store']
        for f in x.field_names:
            x.align[f] = 'l'
        for field, field_data in ast_model_data.get('fields', {}).iteritems():
            ...
            x.add_row([field, l(field_type), l(field_relation), l(field_required), l(field_compute), l(field_inverse),
