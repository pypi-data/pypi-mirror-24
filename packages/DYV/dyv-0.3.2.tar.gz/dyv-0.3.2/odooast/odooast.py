        ...
        for path in paths:
            ...
            for item in ast.iter_child_nodes(obj):
                if type(item) == ast.ClassDef:
                    ...
                    if object_name:
                        self.data[path].append((class_name, class_base, object_name, inherits, fields))

    def get_models(self, model_args=[], field_args=[]):
        data = self.get_processed()
        root = []
        for model, model_data in data.iteritems():
            ...
            if field_args:
                ...
                for field_arg in field_args:
                    data_fields = model_data.get('fields', {}).keys()
                    if field_arg not in data_fields:
                        found = False
                if not found:
                    continue
