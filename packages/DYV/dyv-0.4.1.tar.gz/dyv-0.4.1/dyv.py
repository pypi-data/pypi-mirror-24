...
def clone(ctx, repo, path, branch, all, depth):
    ...
    click.echo("Branches %s, depth %s " % (branches, depth))
    if not os.path.isdir(path):
        click.secho('The path %s is not found' % path, fg='red')
        return
    __init_path_with_branches(path, branches)
    for br in branches:
        branch_path = os.path.join(path, br)
        click.echo("Clone the branch %s to %s" % (br, branch_path))
        __execute_git_command('clone', repo.get('path'), branch_path, br, depth, folder_name=repo_arg)
...
def pull(ctx, repo, path, branch, all, depth):
    ...
    for br in branches:
        branch_path = os.path.join(path, br)
        click.echo("Pull the branch %s to %s" % (br, branch_path))
        __execute_git_command('pull', repo.get('path'), branch_path, br, depth, folder_name=repo_arg)
