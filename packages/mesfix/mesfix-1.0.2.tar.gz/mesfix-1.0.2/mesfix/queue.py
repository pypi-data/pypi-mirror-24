...
class MesfixRabbitMQConnector(object):
    ...
    def message_publisher_factory(self, routing_key, **kargs):
        ...
        if routing_key == 'end-auction':
            message = end_auction(user_account_object, offer_object, auction_object, 
                document_objects, investor_entity_object, company_object, link_object)

        if routing_key == 'transaction-received':
            message = transaction_received(user_account_object, entity_object, company_object, 
                auction_object)
