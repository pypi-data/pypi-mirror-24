...
def check_performance_for_discipline(discipline, textvalue, decimals=2):
    """
    Fix up and return what they typed in,  or raise ValueError
    """
    # print("checkperf %s %s (%d dp)" % (discipline, repr(textvalue), decimals))
    ...
    if discipline in FIELD_EVENTS:
        ...
        if len(chunks) == 1:
            hours = 0
            minutes = 0
            seconds = Decimal(chunks[0])
        elif len(chunks) == 2:
            hours = 0
            minutes = int(chunks[0])
            seconds = Decimal(chunks[1])
        elif len(chunks) == 3:
            hh, mm, ss = chunks
            hours = int(hh)
            minutes = int(mm)
        ...
        if distance == 400 and minutes > 45:
            # e.g. they wrote 63:40 instead of 63.40 - fix it for them
            seconds = minutes + Decimal('0.01') * seconds
            hours = 0
            minutes = 0


        # round up seconds if needed.

        q = Decimal("0." + ("0" * decimals))
        seconds = seconds.quantize(q, rounding=ROUND_UP)
        # print q, seconds


        if (minutes == 0) and (seconds >= Decimal('100')):
            raise ValueError(
        ...
        if distance and duration:
            ...
            velocity = distance * 1.0 / float(duration)
        ...
        if hours and minutes:
            if seconds < Decimal('10'):
                t = '%d:%02d:0%s' % (hours, minutes, seconds)
            else:
                t = '%d:%02d:%s' % (hours, minutes, seconds)
        elif minutes:
            if seconds < Decimal('10'):
                t = '%d:0%s' % (minutes, seconds)
            else:
                t = '%d:%s' % (minutes, seconds)
        else:
            t = '%s' % seconds
