...
#
#
#     Sample configuration for 'no, YOU talk to the hand' (nytth)
#
#
...
cfg = None
def get_config(config='~/.nytth/config.yml'):
...
@cli.command('start')
@click.option('--config', '-c', default='~/.nytth/config.yml', help='specify config file')
def start(config):
    """
    ...
    # try :
    cmd = "supervisord".format(cfg['basedir']);
    print 'running: ', cmd
    subprocess.call(cmd, cwd=cfg['basedir'])
