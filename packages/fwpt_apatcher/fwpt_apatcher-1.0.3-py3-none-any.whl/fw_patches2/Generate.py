...
############################
    ...
    def _run_scripts(self, readonly, account, descr, list_):
        ...
        for pos, file in enumerate(list_):
            ...
            line = (SQLPLUS_GENERATE.format(self.scriptPrefix + file, account=account)).format(schema=self.schema, password=self.password)
            print ("\n------\nExecute: {0}\n".format(line))
            if not readonly:
    ...
    def _update_schema_version(self):
        ...
        if self.options is not None:
            for option in self.options:
                self._update_schema_project_version(option)
            
    def _update_schema_project_version(self, option):
        '''
