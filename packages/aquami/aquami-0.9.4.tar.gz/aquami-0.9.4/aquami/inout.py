...
def pdfSaveOverlay(pdf, image, mask, title=''):
    overlay = display.overlayMask(image, mask, return_overlay=True)
    plt.imshow(overlay)
    plt.title(title)
    plt.axis('off')
    pdf.savefig()
...
def pdfSaveHist(pdf, data, title=None, xlabel=None,
    ...
    n, bins, patches = plt.hist(data, bins=numBins, normed=1, edgecolor='black')
    if log:
...
def pdfSaveSkel(pdf, skeleton, mask, dialate=False, title=None):
    """
    ...
    skel = np.uint8(plt.cm.spectral(skel)*255) # apply colormap to skel data.
    for i in range(3):
        skel[:,:,i] += outlines
