...
def _batch_service_create(driver_file, service_name, title, verb, inputs,
    ...
    if context.env_is_k8s:
        docker_url = create_docker_image(driver_file, dependencies=[], context=context,
                                         service_name=service_name,
                                         target_runtime='spark-py', verb=verb,
                                         custom_ice_url=custom_ice_url)
        if docker_url is None:
            # we already logged the error--short circuit processing
            return
