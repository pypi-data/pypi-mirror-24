...
def _get_hosting_account_info():
    ...
    mms_swagger_location = host_account['model_management_swagger_location']
    mms_swagger_resp = requests.get(mms_swagger_location)

    if mms_swagger_resp.status_code == 200:
        mms_swagger = get_json(mms_swagger_resp.content, pascal=True)
        base_url = 'https://{}'.format(mms_swagger['Host'])
    else:
        raise CLIError('Error attempting to retrieve host account swagger:\n{}\n{}'.format(mms_swagger_resp.headers, mms_swagger_resp.content))
    resource_group = host_account['id'].split('/')[4]
