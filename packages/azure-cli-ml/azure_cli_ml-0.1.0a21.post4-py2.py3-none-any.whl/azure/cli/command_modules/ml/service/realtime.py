...
def realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
                            ...
                            num_replicas, image_id, image_type, conda_file,
...
def _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
    ...
    if not use_ice:
        return _mms_service_create(image_id, image_type, service_name, driver_file, model_file, schema_file,
                                   ...
                                   model_data_collection_enabled, num_replicas, conda_file, cpu, memory,
...
def _mms_service_create(image_id, image_type, service_name, driver_file, model_file, schema_file, dependencies, runtime,
                        ...
                        conda_file, cpu, memory, replica_max_concurrent_requests, verb, context):
    auth_header = get_auth_header()
    headers = {'Content-Type': 'application/json', 'Authorization': auth_header}
    params = {'api-version': MMS_API_VERSION}
    ...
    if cpu:
        json_payload['containerResourceReservation']['cpu'] = cpu
    else:
        del(json_payload['containerResourceReservation']['cpu'])
    if memory:
        json_payload['containerResourceReservation']['memory'] = memory
    else:
        del(json_payload['containerResourceReservation']['cpu'])
    if replica_max_concurrent_requests:
        json_payload['maxConcurrentRequestsPerContainer'] = replica_max_concurrent_requests
    else:
        del(json_payload['maxConcurrentRequestsPerContainer'])
