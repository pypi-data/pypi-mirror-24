...
def _process_logs(log_string, requestId):
            ...
            if logger and logger == 'root':
                ...
                if requestId is not None:
                    if message_json['requestId'] == requestId:
                        result_log.append(_construct_log_line(json_line['timestamp'],
                                                              json_line['level'],
                    ...
                    result_log.append(_construct_log_line(json_line['timestamp'],
                                                          ...
                                                          message_json['apiName'],
                                                          message_json['message']))
            elif logger and logger == 'logger_stdout' or logger == "logger_stderr":
                ...
                if requestId is None:
                    result_log.append(_construct_log_line(json_line['timestamp'],
                                                          ...
                                                          '',
                                                          json_line['message']))
                elif requestId and requestId == json_line.get(requestId, ''):
                    result_log.append(_construct_log_line(json_line['timestamp'],
                                                          'PRINT_STATEMENT',
                                                          json_line['requestId'],
                                                          '',
                                                          json_line['message']))
            elif logger and (logger == 'gunicorn.error' or logger == 'gunicorn.access' or logger == 'create_app'):
                empty_requestId = '00000000-0000-0000-0000-000000000000'
                if 'exc_info' in json_line:
                    json_line['message'] = json_line['message'] + '\n' + json_line['exc_info']
                if requestId == empty_requestId:
                    result_log.append(_construct_log_line(json_line['timestamp'],
                                                          json_line['level'],
                                                          empty_requestId,
                                                          '',
                                                          json_line['message']))
                elif requestId is None:
                    result_log.append(_construct_log_line(json_line['timestamp'],
                                                          json_line['level'],
                                                          empty_requestId,
                                                          '',
    ...
    for line in result_log:
        print(line)
    print("Received {0} lines of log".format(len(result_log)))
...
def _kubectl_get_logs(pod_name, kubeconfig, context, log_offset=1, kubectl_location='/tmp/kubectl-1.6.4'):
    tail_cmd_str = "{0} logs {1} --kubeconfig {2} --timestamps --tail=1".format(kubectl_location, pod_name, kubeconfig)
    tail_time = context.run_cmd(shlex.split(tail_cmd_str))[0].split(' ')[0]
    try:
        tail_time = parser.parse(tail_time)
    except:
        raise CLIError("Failed to parse tail time")
    final_time = tail_time - timedelta(hours=log_offset)
    log_cmd = '{0} logs {1} --kubeconfig {2} --since-time={3}'.format(kubectl_location, pod_name, kubeconfig,
...
def get_logs_from_kubernetes(service_id, kubeconfig, context, requestId=None, log_offset=1, kubectl_location='kubectl', verb=False):
    ''' 
    ...
    :param log_offset (optional): This is an int in hours that we will use to offset our logs. Basically gets the last log and
                        subtracts whatever the timestamp of that last log is with this value and gets the log from that point
                        onwards
