...
from .._get_logs import get_logs_from_docker, get_logs_from_kubernetes
...
def realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
                            ...
                            num_replicas, image_id, image_type, conda_file, autoscale_enabled, autoscale_min_replicas,
                            autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds,
                            cpu, memory, replica_max_concurrent_requests, verb, context=cli_context, use_ice=False):
    _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
                             target_runtime, app_insights_logging_enabled, model_data_collection_enabled, model_file,
                             num_replicas, image_id, image_type, conda_file, autoscale_enabled, autoscale_min_replicas,
                             autoscale_max_replicas, autoscale_target_utilization, autoscale_refresh_period_seconds,
                             cpu, memory, replica_max_concurrent_requests, verb, context, use_ice)
...
def _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
    ...
    if not use_ice:
        return _mms_service_create(image_id, image_type, service_name, driver_file, model_file, schema_file,
                                   ...
                                   model_data_collection_enabled, num_replicas, conda_file, autoscale_enabled,
                                   autoscale_min_replicas, autoscale_max_replicas, autoscale_target_utilization,
                                   autoscale_refresh_period_seconds, cpu, memory, replica_max_concurrent_requests,
                                   verb, context)
    else:
...
def realtime_service_list(service_name=None, use_ice=False, verb=False, context=cli_context):
    _, result = _realtime_service_list(service_name, use_ice, verb, context)
    return result, verb
...
def _realtime_service_update(service_id, image_id, num_replicas, model_data_collection_enabled, app_insights_enabled,
                             ...
                             autoscale_target_utilization, autoscale_refresh_period_seconds,
                             cpu, memory, replica_max_concurrent_requests, verb, context=cli_context):
    return _mms_service_update(service_id, image_id, num_replicas, model_data_collection_enabled, app_insights_enabled,
                               autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas,
                               autoscale_target_utilization, autoscale_refresh_period_seconds,
                               cpu, memory, replica_max_concurrent_requests, verb, context)
...
def realtime_service_usage(service_name, service_id, verb, context=cli_context):
    _realtime_service_usage(service_name, service_id, verb, context)


def _realtime_service_usage(service_name, service_id, verb, context):
    ...
    if not context.in_local_mode():
        ...
        print('    az ml service run realtime -n {} -d "{}"'.format(service_name, DEFAULT_INPUT_DATA))
    print()
    print('Sample CURL call:')
    print('    curl -X POST -H "Content-Type:application/json" -H "Authoriztion:Bearer <key>" --data "{}" {}'.format(DEFAULT_INPUT_DATA, scoring_url))
    return SUCCESS_RETURN_CODE


def realtime_service_logs(service_name, request_id, kube_config, service_id, verb, context=cli_context):
    _realtime_service_logs(service_name, request_id, kube_config, service_id, verb, context)


def _realtime_service_logs(service_name, request_id, kube_config, service_id, verb, context):
    if not service_name and not service_id:
        raise CLIError('Error, one of service name or service ID must be provided.')
    if context.in_local_mode():
        if not service_name:
        ...
        if kube_config is None and os.path.exists(default_kube_path):
            if verb:
                print("Found default kubeconfig in {0} using it".format(default_kube_path))
            kube_config = default_kube_path
        if not kube_config:
            raise CLIError('Kubeconfig needs to be provided to get logs in cluster mode. You can the following command to get it:'
                           'az ml env get-credntials -g <resource group> -n <cluster name> -i')
        if verb:
            print("Using kubeconfig file: {0}".format(kube_config))
        if not check_for_kubectl(context):
            print('')
            print('kubectl is required to get logs from webservices. Please install it on your path and try again.')
            return
        get_logs_from_kubernetes(service_id, kube_config, context, requestId=request_id, verb=verb)


# MMS functions
def _mms_service_create(image_id, image_type, service_name, driver_file, model_file, schema_file, dependencies, runtime,
                        requirements, app_insights_logging_enabled, model_data_collection_enabled, num_replicas,
                        conda_file, autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas,
    ...
    if not image_id:
        image_create_success, image_id = _image_create(service_name[:30], image_type, '', None, driver_file, schema_file, dependencies,
                                                       runtime, requirements, conda_file, model_file, verb, context)
...
def _mms_service_update(service_id, image_id, num_replicas, model_data_collection_enabled, app_insights_enabled,
                        autoscale_enabled, autoscale_min_replicas, autoscale_max_replicas,
                        autoscale_target_utilization, autoscale_refresh_period_seconds,
                        cpu, memory, replica_max_concurrent_requests, verb, context):
    auth_header = get_auth_header()
