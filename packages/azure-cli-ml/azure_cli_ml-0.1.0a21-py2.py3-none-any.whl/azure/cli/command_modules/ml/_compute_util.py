...
def get_compute_resource(resource_group, cluster_name):
    try:
        ...
        raise CLIError('Unable to get compute resource with resource group "{}" and cluster name "{}": {}'.format(
            resource_group, cluster_name, exc))
...
def create_compute_resource(resource_group, cluster_name, sp_id, sp_pw,
    ...
    oc = OperationalizationCluster(
        app_insights=AppInsightsCredentials(
            app_id=app_insights_account_name,
            api_key=app_insights_account_key
        ),
        location=location,
