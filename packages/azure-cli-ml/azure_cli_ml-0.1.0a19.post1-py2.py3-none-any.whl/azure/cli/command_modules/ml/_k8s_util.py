...
def deploy_realtime_frontend(context, k8s_ops, acr_email):
    ...
    try:
        ...
        # need to do this in order--service first, then deployment
        for kind, fn in [('Service', k8s_ops.expose_frontend),
                         ('Deployment', k8s_ops.deploy_deployment)]:
            # load_all gives a use-once iterator, so load_all twice
            docs = yaml.load_all(yaml_to_deploy.content)
