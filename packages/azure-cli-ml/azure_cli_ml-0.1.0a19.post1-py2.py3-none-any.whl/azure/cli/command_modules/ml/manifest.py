...
def manifest_create(driver_file, manifest_description, schema_file, dependencies, runtime, requirements, model_ids,
                    model_name, model_file, verb, context=cli_context):
    _manifest_create(driver_file, manifest_description, schema_file, dependencies, runtime, requirements, model_ids,
                     model_name, model_file, verb, context)


def _manifest_create(driver_file, manifest_description, schema_file, dependencies, runtime, requirements, model_ids,
                     model_name, model_file, verb, context):
    ...
    if not model_file and not model_name and not model_ids:
        print("No model information provided, skipping model creation")
        model_info = None
    else:
        model_info = handle_model_file(model_ids, model_name, model_file, base_url, subscription, resource_group,
...
def _manifest_show(manifest_id, verb, context):
    base_url, subscription, resource_group, host_account_name = get_sub_and_account_info()
    manifest_url = MMS_MANIFEST_URL.format(base_url, subscription, resource_group, host_account_name) + '/{}'.format(manifest_id)
    auth_token = get_auth_token()
