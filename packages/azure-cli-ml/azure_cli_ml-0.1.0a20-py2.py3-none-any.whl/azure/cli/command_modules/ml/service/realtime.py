...
from .._constants import MMS_SERVICE_URL_FMT
from .._constants import MMS_SERVICE_LIST_KEYS_URL_FMT
from .._constants import MMS_SERVICE_REGEN_KEYS_URL_FMT
...
def realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
                            ...
                            num_replicas, image_id, image_type, conda_file, verb, context=cli_context, use_mms=False):
    _realtime_service_create(driver_file, dependencies, requirements, schema_file, service_name, custom_ice_url,
                             target_runtime, app_insights_logging_enabled, model_data_collection_enabled, model_file,
                             num_replicas, image_id, image_type, conda_file, verb, context, use_mms)
...
def realtime_service_run_kubernetes(context, service_name, service_id, input_data, verbose):
    ...
    if service_id:
        ...
        service_url = MMS_SERVICE_URL_FMT.format(base_url, subscription, resource_group, host_account_name) + \
                       '/{}'.format(service_id)
...
def realtime_service_update(service_id, image_id, image_type, driver_file, model_file, schema_file,
                            dependencies, target_runtime, requirements, verb, context=cli_context):
    _realtime_service_update(service_id, image_id, image_type, driver_file, model_file, schema_file,
                             dependencies, target_runtime, requirements, verb, context)


def _realtime_service_update(service_id, image_id, image_type, driver_file, model_file, schema_file,
                             dependencies, target_runtime, requirements, verb, context=cli_context):
    return _mms_service_update(service_id, image_id, image_type, driver_file, model_file, schema_file,
                               dependencies, target_runtime, requirements, verb, context)
...
def _realtime_service_keys_handling(service_id, regen, verb, context=cli_context):
    ...
    if regen:
        regen_keys_url = MMS_SERVICE_REGEN_KEYS_URL_FMT.format(base_mms_url, subscription, resource_group, host_account_name, service_id)
        try:
            resp = context.http_call('post', regen_keys_url, headers=headers, timeout=MMS_SYNC_TIMEOUT_SECONDS)
        except requests.ConnectionError:
            raise CLIError('Error connecting to {}.'.format(regen_keys_url))
        except requests.Timeout:
            raise CLIError('Error, request to {} timed out.'.format(regen_keys_url))
    else:
        list_keys_url = MMS_SERVICE_LIST_KEYS_URL_FMT.format(base_mms_url, subscription, resource_group, host_account_name, service_id)
        try:
            resp = context.http_call('get', list_keys_url, headers=headers, timeout=MMS_SYNC_TIMEOUT_SECONDS)
        except requests.ConnectionError:
...
def _mms_service_create(image_id, image_type, service_name, driver_file, model_file, schema_file,
    ...
    auth_token = get_auth_token()
    headers = {'Content-Type': 'application/json', 'Authorization': 'Bearer {}'.format(auth_token)}
    service_url = MMS_SERVICE_URL_FMT.format(base_mms_url, subscription, resource_group, host_account_name)
    ...
    operation_url = MMS_OPERATION_URL_FMT.format(base_mms_url, subscription, resource_group, host_account_name,
                                                 create_operation_status_id)
    operation_headers = {'Authorization': 'Bearer {}'.format(auth_token)}

    sys.stdout.write('Creating service')
    sys.stdout.flush()
    service_id = poll_mms_async_operation(operation_url, operation_headers,
...
def _mms_service_update(service_id, image_id, image_type, driver_file, model_file, schema_file,
                        dependencies, target_runtime, requirements, verb, context):
    base_url, subscription, resource_group, host_account_name = get_sub_and_account_info()
    auth_token = get_auth_token()
    headers = {'Content-Type': 'application/json', 'Authorization': 'Bearer {}'.format(auth_token)}
    ...
    if not image_id:
        image_create_success, image_id = _image_create(image_type, '', None, driver_file, schema_file, dependencies,
                                                       target_runtime, requirements, model_file, verb, context)
