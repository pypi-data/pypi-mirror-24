...
class SurveyorDialog(QtGui.QDialog):
    def __init__(self, parent=None, parameterfile=None, outputdir=None):
        ...
        QtCore.QObject.connect(self._fileSysWatcher, QtCore.SIGNAL("directoryChanged(QString)"), self,QtCore.SLOT("slotDirChanged(QString)"))
        ...
        for name in files:
            ...
            if OPTION_XEUSS:
                ...
                for j in range(len(infos)):
                    self.ui.tableWidget.setItem(i, 4+j, QtGui.QTableWidgetItem(infos[j]))
            i += 1
        self.listoffileVerif = glob.glob(os.path.join(self.fp, self.ext))#filetools.listFiles(self.fp,self.ext)
        self.listoffileVerif = listoffile
        #display first
