...
__all__ = ["bytes2human","get_file_stat","ownerinfo","created_time","last_accesse_time","last_motified_time","get_file_size"]
...
def created_time(file_stat):
    created_time = datetime.fromtimestamp(
        int(file_stat.st_ctime)).strftime('%Y-%m-%d %H:%M:%S')
    return created_time


def last_accesse_time(file_stat):
    last_accessed_time = datetime.fromtimestamp(
        int(file_stat.st_atime)).strftime('%Y-%m-%d %H:%M:%S')
    return last_accessed_time


def last_motified_time(file_stat):
    last_motified_time = datetime.fromtimestamp(
        int(file_stat.st_mtime)).strftime('%Y-%m-%d %H:%M:%S')
...
if __name__ == "__main__":
    filestat = get_file_stat(
        "/Volumes/4TB/PROJETS/Code/PyQT/MTB_QT_Widgets/README.md")
