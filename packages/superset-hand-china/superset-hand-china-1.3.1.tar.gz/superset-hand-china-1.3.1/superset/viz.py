...
class BaseViz(object):
    ...
    def __init__(self, datasource, form_data):
        if not datasource:
            raise Exception(__("Viz is missing a datasource"))
    ...
    def query_obj(self):
        ...
        if from_dttm > to_dttm:
            raise Exception(__("From date cannot be larger than to date"))
...
class TableViz(BaseViz):
    ...
    def should_be_timeseries(self):
        ...
        if fd.get('include_time') and not conditions_met:
            raise Exception(
                __("Pick a granularity in the Time section or "
    ...
    def query_obj(self):
        ...
        if fd.get('all_columns') and (fd.get('groupby') or fd.get('metrics')):
            raise Exception(
                __("Choose either fields to [Group By] and [Metrics] or "
                + "[Columns], not both"))
...
class PivotTableViz(BaseViz):
    ...
    def query_obj(self):
        ...
        if not groupby:
            groupby = []
        if not groupby:
            raise Exception(__("Please choose at least one [Group by] field "))
        if not metrics:
            raise Exception(__("Please choose at least one metric"))
        if (
                any(v in groupby for v in columns) or
                any(v in columns for v in groupby)):
            raise Exception(__("[Group By] and [Columns] can't overlap"))
        return d
...
class BubbleViz(NVD3Viz):
    ...
    def query_obj(self):
        ...
        if not all(d['metrics'] + [self.entity]):
            raise Exception(__("Pick a metric for x, y and size"))
        return d
...
class BulletViz(NVD3Viz):
    ...
    def query_obj(self):
        ...
        if not self.metric:
            raise Exception(__("Pick a metric to display"))
        return d
...
class BigNumberViz(BaseViz):
    ...
    def query_obj(self):
        ...
        if not metric:
            raise Exception(__("Pick a metric!"))
        d['metrics'] = [self.form_data.get('metric')]
...
class NVD3TimeSeriesViz(NVD3Viz):
    ...
    def get_data(self, df):
        ...
        if fd.get("granularity") == "all":
            raise Exception(__("Pick a time granularity for your time series"))
...
class NVD3DualLineViz(NVD3Viz):
    ...
    def query_obj(self):
        ...
        d['metrics'] = [m1, m2]
        if not m1:
            raise Exception(__("Pick a metric for left axis!"))
        if not m2:
            raise Exception(__("Pick a metric for right axis!"))
        if m1 == m2:
            raise Exception(__("Please choose different metrics on left and right axis"))
        return d
...
class HistogramViz(BaseViz):
    ...
    def query_obj(self):
        ...
        if numeric_column is None:
            raise Exception(__("Must have one numeric column specified"))
...
class DistributionBarViz(DistributionPieViz):
    ...
    def query_obj(self):
        ...
        if (
            ...
            raise Exception(__("Can't have overlap between Series and Breakdowns"))
        if not fd.get('metrics'):
            raise Exception(__("Pick at least one metric"))
        if not fd.get('groupby'):
            raise Exception(__("Pick at least one field for [Series]"))
        return d
...
class SankeyViz(BaseViz):
    ...
    def query_obj(self):
        ...
        if len(qry['groupby']) != 2:
            raise Exception(__("Pick exactly 2 columns as [Source / Target]"))
        qry['metrics'] = [
...
class DirectedForceViz(BaseViz):
    ...
    def query_obj(self):
        ...
        if len(self.form_data['groupby']) != 2:
            raise Exception(__("Pick exactly 2 columns to 'Group By'"))
        qry['metrics'] = [self.form_data['metric']]
        return qry
...
class FilterBoxViz(BaseViz):
    ...
    def query_obj(self):
        ...
        if len(groupby) < 1 and not self.form_data.get('date_filter'):
            raise Exception(__("Pick at least one filter field"))
        qry['metrics'] = [self.form_data['metric']]
        ...
        if not fd.get('groupby'):
            ...
            if label_col and len(label_col) >= 1:
                if label_col[0] == "count":
                    raise Exception(
                        __("Must have a [Group By] column to have 'count' as the [Label]"))
