...
class Hand(BaseSupersetView):
    ...
    def getSliceData(self):
        ...
        return json.dumps({"dashboard_data": {"slices": slices}, "datasources": {ds.uid: ds.data for ds in datasources} })

    # 获取提示器组合所有子切片id及名称
    @has_access
    @expose("/getFilterBoxs/", methods=['GET', 'POST'])
    def getFilterBoxs(self):
        filterBoxs = db.session.query(superset_models.Slice).filter(superset_models.Slice.viz_type.in_(['filter_box', 'filter_box_tree'])).all()
        filters = []
        for f in filterBoxs:
            filters.append({
                'id': f.id,
