    ...
    def verify(self):
        ...
        if type(pact_helper) is Right:
            ...
            if type(tests) is Right:
                info(str(len(tests.value)) + ' test(s) found.')
                info('Execute Pact Helper setup: START')
                self.pact_helper.setup()
                info('Execute Pact Helper setup: DONE')
                test_results = Right(list(map(self.verify_test, tests.value)))
                info('Execute Pact Helper tear down: START')
                self.pact_helper.tear_down()
                info('Execute Pact Helper tear down: DONE')
                info('Verify consumers: DONE')
                return test_results
    ...
    def verify_test(self, test):
        ...
        if type(validity_check) is Right:
            ...
            if type(pact) is Right:
                ...
                info(str(len(interactions)) + ' interaction(s) found')
