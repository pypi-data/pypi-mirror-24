...
def compare_reverse_test(l,a,dims,degs):
    ...
    for row in pos:
        ...
        row = row.split(',')
        pos_out.append(row)
        pos_rev.append(list(reversed(row)))
        how_long.append(len(row))
    ...
    # match 'k' addresses for the signature elements of the original and the inverse paths
    pos_out = [str(x) for x in pos_out]
    pos_rev = [str(x) for x in pos_rev]
    tmp_out = list(zip(pos_out, how_long, out_sig))
    tmp_rev = list(zip(pos_rev, how_long, out_rev))
    tmp_out = pd.DataFrame(tmp_out)
    tmp_rev = pd.DataFrame(tmp_rev)
    out_and_rev = pd.merge(tmp_out,tmp_rev, left_on=0, right_on=0)
    ...
    out = out_and_rev['1_x']*out_and_rev['2_x'] + out_and_rev['2_y']
    if out[0] != 2:
        raise ValueError
    for row in out[1:]:
