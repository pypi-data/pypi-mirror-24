...
def setup_app(app_name, app_directory):
    ...
    set_engine_config(test_mode)
    setup_features()

    data_provider.set_base_dir(app_directory)
    if not test_mode and not quickweb.is_production:
        print_warn('Running in development mode, set QUICKWEB_PRODUCTION for production mode')
...
def set_engine_config(test_mode):
    ...
    # Disable logging when running on CloudFoundry
