...
class DeltaTimeFeatureExtractor(TransformerMixin):
    ...
    def transform(self, X):
        ...
        #X['Delta_f'] = X.sort_values(self.date_field_name).groupby(self.agg_by, group_keys=False)[self.date_field_name].diff()
        #X['Delta_t'] = X.sort_values(self.date_field_name).groupby(self.agg_by)[self.date_field_name].diff()
        #return X
        ...
        for shift in range(1, 1 + self.how_many):
            ...
            for nth in range(shift):
                df2 = sorted_df.groupby(self.agg_by, group_keys=False).nth(nth)
