...
class FakeDate(with_metaclass(FakeDateMeta, real_date)):
    ...
    def today(cls):
        ...
        if self.time_offset:
            result = result + self.time_offset
...
class FakeDatetime(with_metaclass(FakeDatetimeMeta, real_datetime, FakeDate)):
    ...
    def now(cls, tz=None):
        ...
        if self.time_offset:
            now = real_datetime.now() + self.time_offset
    ...
    def utcnow(cls):
        ...
        if self.time_offset:
            result = real_datetime.utcnow() + self.time_offset
...
def fixtime(ntp_server='br.pool.ntp.org'):
    string_type = str
    ...
    ignore.append('Queue')
    return _freeze_time(ntp_server, ignore).fix()
