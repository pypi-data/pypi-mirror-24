...
def create_creds_file():
    ...
    if not os.path.exists('/root/.spotinst/agent'):
        os.makedirs('/root/.spotinst/agent')
    creds_path = '/root/.spotinst/agent/credentials'
...
def handle_prerequisites(s3_base_module_link, module_name):
    try:
        response = urllib2.urlopen(s3_base_module_link + '/' + module_name + '.req')
