...
def get_worker_files(modules_path, s3_base_workers_link, worker_name):
    with open(modules_path + '/' + worker_name + '/' + worker_name + '.py', 'wb') as worker:
        response = urllib2.urlopen(s3_base_workers_link + '/' + worker_name + '.py')
        worker.write(response.read())
    with open(modules_path + '/' + worker_name + '/' + worker_name + '.yml', 'wb') as worker_cfg:
        response = urllib2.urlopen(s3_base_workers_link + '/' + worker_name + '.yml')
        worker_cfg.write(response.read())


def get_collector_files(collector_name, modules_path, s3_base_collectors_link):
    with open(modules_path + '/' + collector_name + '/' + collector_name + '.py', 'wb') as collector:
        response = urllib2.urlopen(s3_base_collectors_link + '/' + collector_name + '.py')
        collector.write(response.read())
    with open(modules_path + '/' + collector_name + '/' + collector_name + '.yml', 'wb') as collector_cfg:
        response = urllib2.urlopen(s3_base_collectors_link + '/' + collector_name + '.yml')
