...
from aiopvapi.helpers.shade_types import SHADE_TYPES
    ...
    def _init_data(self):
        for _type in SHADE_TYPES:
            if self.shade_type in _type[ATTR_TYPES]:
                ...
                self._open_data = _type.get(ATTR_OPEN_POSITION)
                self._close_data = _type.get(ATTR_CLOSE_POSITION)
                for _allowed_position in _type[ATTR_ALLOWED_POSITIONS]:
