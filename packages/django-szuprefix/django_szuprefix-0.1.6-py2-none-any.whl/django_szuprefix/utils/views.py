...
class ContextJsonDumpsMixin(object):
    ...
    def render_to_response(self, context, **response_kwargs):
        if self.request.is_ajax() or self.request.GET.get("format_") == 'json':
            return JsonResponse(dict(code=0, msg='ok', data=self.get_json_contexts(context)), encoder=datautils.JSONEncoder)
...
class SearchFormMixin(object):
    ...
    def get_search_fields_label(self):
        return u"搜索" + ",".join([modelutils.get_related_field_verbose_name(self.model, f) for f in self.search_fields])
    ...
    def get_search_limit_choices(self):
        ...
        for f in self.get_search_form_fields():
            ...
            if hasattr(self, func_name):
                d[f] = getattr(self, func_name)()
            else:
                d[f] = modelutils.get_related_field(self.model, f).choices
        return d
