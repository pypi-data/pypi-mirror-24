...
class Clazz(models.Model):
    ...
    entrance_session = models.ForeignKey(Session, verbose_name=u"入学届别", related_name="entrance_clazzs", null=True,
                                         blank=True)
    ...
    def save(self, **kwargs):
        if self.entrance_session is None:
            from . import helper
            self.entrance_session, created = helper.gen_default_grades(self.school, self.grade.number - 1)
...
class Student(models.Model):
    ...
    entrance_session = models.ForeignKey(Session, verbose_name=u"入学届别", related_name="entrance_students",
                                         on_delete=models.PROTECT)
    graduate_session = models.ForeignKey(Session, verbose_name=u"毕业届别", related_name="graduate_students",
                                         on_delete=models.PROTECT)
