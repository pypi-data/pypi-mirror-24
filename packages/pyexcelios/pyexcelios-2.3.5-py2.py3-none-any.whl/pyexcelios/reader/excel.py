...
from pyexcelios.compat import unicode, file
...
# package imports
from pyexcelios.utils.exceptions import InvalidFileException
from pyexcelios.xml.constants import (
    ARC_SHARED_STRINGS,
    ARC_CORE,
    ARC_WORKBOOK,
    ARC_STYLE,
...
from pyexcelios.workbook.names.external import detect_external_links
from pyexcelios.workbook.names.named_range import read_named_ranges
from pyexcelios.packaging.relationship import get_dependents
...
from pyexcelios.workbook.properties import read_properties, DocumentProperties
from pyexcelios.worksheet.read_only import ReadOnlyWorksheet
from .worksheet import WorkSheetParser
from pyexcelios.comments.reader import read_comments, get_comments_file
...
def _validate_archive(filename):
    ...
    if not is_file_like and os.path.isfile(filename):
        ...
        if file_format not in SUPPORTED_FORMATS:
            if file_format == '.xls':
                msg = ('pyexcelios does not support the old .xls file format, '
                       'please use xlrd to read this file, or convert it to '
                       'the more recent .xlsx file format.')
            elif file_format == '.xlsb':
                msg = ('pyexcelios does not support binary format .xlsb, '
                       'please convert this file to .xlsx format if you want '
                       'to open it with pyexcelios')
            else:
                msg = ('pyexcelios does not support %s file format, '
...
def load_workbook(filename, read_only=False, use_iterators=False, keep_vba=KEEP_VBA, guess_types=False, data_only=False):
    ...
    :rtype: :class:`pyexcelios.workbook.Workbook`

    .. note::

        When using lazy load, all worksheets will be :class:`pyexcelios.worksheet.iter_worksheet.IterableWorksheet`
