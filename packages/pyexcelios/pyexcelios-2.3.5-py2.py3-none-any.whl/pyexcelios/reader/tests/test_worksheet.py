...
def test_get_xml_iter():
    ...
    #3 string
    #4 zipfile
    from pyexcelios.reader.worksheet import _get_xml_iter
    ...
    f = TemporaryFile(mode='rb+', prefix='pyexcelios.', suffix='.unpack.temp')
    stream = FUT(f)
    assert stream == f
    f.close()
...
def Workbook():
    ...
    from pyexcelios.styles import numbers
...
@pytest.fixture
def Translator():
    from pyexcelios.formula import translate
