...
from pyexcelios.xml.functions import fromstring, tostring, xmlfile
from pyexcelios.reader.excel import load_workbook
from pyexcelios import Workbook

from .. worksheet import write_worksheet
from .. relations import write_rels

from pyexcelios.tests.helper import compare_xml
from pyexcelios.worksheet.properties import PageSetupProperties
from pyexcelios.xml.constants import SHEET_MAIN_NS, REL_NS
...
@pytest.fixture
def ColumnDimension():
    from pyexcelios.worksheet.dimensions import ColumnDimension
...
def test_col_style(write_cols, ColumnDimension, DummyWorksheet):
    from pyexcelios.styles import Font
    ws = DummyWorksheet
...
def test_lots_cols(write_cols, ColumnDimension, DummyWorksheet):
    from pyexcelios.styles import Font
    ws = DummyWorksheet
    from pyexcelios.cell import get_column_letter
    for i in range(1, 15):
...
def test_write_height(worksheet, write_rows):
    from pyexcelios.worksheet.dimensions import RowDimension
    ws = worksheet
    ws['F1'] = 10
...
def worksheet_with_cf(worksheet):
    from pyexcelios.formatting import ConditionalFormatting
...
def test_conditional_formatting_customRule(worksheet_with_cf, write_conditional_formatting):
    ...
    from pyexcelios.formatting.rule import Rule
...
def test_conditional_font(worksheet_with_cf, write_conditional_formatting):
    ...
    # Create cf rule
    from pyexcelios.styles import PatternFill, Font, Color
    from pyexcelios.formatting.rule import CellIsRule
...
def test_formula_rule(worksheet_with_cf, write_conditional_formatting):
    from pyexcelios.formatting.rule import FormulaRule
