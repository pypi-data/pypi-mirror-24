...
class Util():
    ...
    def getNquard(subject, predicat, object, label, isBlankNode):
        ...
        nquard = {
            ...
            'predicate': ('<' + predicat.strip() + '>').replace('<<', '<').replace('>>', '>'),
            'object': ('"' + object.strip() + '"').replace('""', '"')
        }
        if isBlankNode is not None:
            # nquard['subject'] = ('<' + isBlankNode + '>').replace('<<', '<').replace('>>', '>')
            nquard['subject'] = isBlankNode
        if label is not None:
            # nquard['label'] = ('<' + label + '>').replace('<<', '<').replace('>>', '>')
            nquard['label'] = ('"' + label + '"').replace('""', '"')
        return nquard
        ...
        for key in jsonData:
            ...
            if isinstance(jsonData[key], dict):
                ...
                if isBlankNode is None:
                    subjectTmp = ('<' + subject + '>').replace('<<', '<').replace('>>', '>')
                    obj = '_:LQ@' + subjectTmp + '.' + predicate
                else:
