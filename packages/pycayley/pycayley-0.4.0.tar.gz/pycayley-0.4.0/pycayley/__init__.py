...
class CayleyClient(object):
    def __init__(self, url="http://localhost:64210", version="v2"):
        # self.url = "%s/api/%s/query/gizmo" % (url, version)
        self.restful_url_prefix = "%s/api/%s/" % (url, version)
    ...
    # save data to cayley
    def Write(self, data):
        write_url = self.restful_url_prefix + "write"
        r = requests.post(write_url, json=Util.objects2Nquard(data))
        return self.on_result(r)

    # save NQuadFile to cayley
    def WriteNQuadFile(self, data):
        pass

    # query data from cayley this method must need v2
    def Read(self):
        write_url = self.restful_url_prefix + "read"
        r = requests.post(write_url)
        return CayleyResponse(r, r.text)

    # delete data from cayley
    def Delete(self, data):
        write_url = self.restful_url_prefix + "delete"
        r = requests.post(write_url, json=Util.objects2Nquard(data))
        return self.on_result(r)
    ...
    def on_result(self, r):
        if r.status_code == 200:
            ...
            return CayleyResponse(r, {"result": r.reason})
