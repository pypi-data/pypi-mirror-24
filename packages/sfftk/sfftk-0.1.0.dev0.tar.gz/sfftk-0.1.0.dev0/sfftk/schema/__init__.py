...
class SFFType(object):
    ...
    def __init__(self, var=None, *args, **kwargs):
        ...
        #3 - SFFSegmentation("file.sff") # build from a file of given name
        ...
        #4 - SFFSoftware(name="name", version="version", processingDetails="details") # use gds_type kwargs
        ...
        if self.gds_type:
            if var:
                if isinstance(var, self.gds_type): # 2 - gds_type to SFFType
                    self._local = var
                elif isinstance(var, str): # a file name # 3 - SFFSegmentation from filename
                    self._local = sff.parse(var, *args, **kwargs)
            else:
                self._local = self.gds_type(*args, **kwargs) #1 and #4 - SFFType from (*a, **kw)
                ...
                if len(self.repr_args) == self.repr_string.count('{}'):
                    ...
                    for arg in self.repr_args:
                        if arg == 'len()':
                            ...
                            repr_args.append(getattr(self, arg))
...
class SFFBiologicalAnnotation(SFFType):
    ...
    def as_hff(self, parent_group, name="biologicalAnnotation"):
        ...
        if self:
            ...
            group['description'] = self.description if self.description else None
...
class SFFThreeDVolume(SFFType):
    ...
    def from_hff(cls, hff_data):
        ...
        assert isinstance(hff_data, h5py.Dataset)    
        obj = cls()        
        obj.file = hff_data['file']
        obj.format = hff_data['format']
        try:
            obj.contourLevel = hff_data['contourLevel']
        except ValueError:
        ...
        try:
            obj.objectPath = hff_data['objectPath'] 
        except ValueError:
            obj.objectPath = None
...
class SFFBoundingBox(SFFType):
    ...
    repr_attr = ('xmin', 'xmax', 'ymin', 'ymax', 'zmin', 'zmax')
#     iter_dict = dict()
    # attributes
...
#     externalReferences = SFFAttribute('externalReferences', sff_type=SFFExternalReferences)
    ...
    def as_json(self, f, sort_keys=True, indent_width=2):
        ...
        for segment in self.segments:
            seg_data = dict()
            seg_data['id'] = segment.id
            seg_data['parentID'] = segment.parentID
            bioAnn = dict()
            ...
            if segment.colour.name:
                ...
                seg_data['colour'] = segment.colour.rgba.value
