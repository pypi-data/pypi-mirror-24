...
def handle_convert(args):
    ...
    if re.match(r'.*\.mod$', args.from_file, re.IGNORECASE):
        ...
        seg = schema.SFFSegmentation(args.from_file, silence=True)
        ...
        import h5py
        with h5py.File(args.from_file) as h:
        ...
        seg = schema.SFFSegmentation.from_json(args.from_file)
        if args.verbose:
            print_date("Created SFFSegmentation object")
    else:
...
def _handle_notes_modify(args):
    ...
    print_date("Using temporary file shorthand '{}'".format(temp_file_ref))
