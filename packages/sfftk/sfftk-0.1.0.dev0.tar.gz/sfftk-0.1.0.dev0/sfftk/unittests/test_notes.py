...
class TestNotes_modify(unittest.TestCase):
    ...
    def _test_add(self):
        ...
        print >> sys.stderr, 'b'
        if re.match(r'.*\.sff$', self.sff_file, re.IGNORECASE):
            seg = schema.SFFSegmentation(self.sff_file, silence=True)
        elif re.match(r'.*\.hff$', self.sff_file, re.IGNORECASE):
            with h5py.File(self.sff_file) as h:
                seg = schema.SFFSegmentation.from_hff(h)
        elif re.match(r'.*\.json$', self.sff_file, re.IGNORECASE):
            seg = schema.SFFSegmentation.from_json(self.sff_file)
        print >> sys.stderr, 'c'
