...
class Appliance(object):
    ...
    def keyword_gen(self, name):
        ...
        :return: method
        """
        # capitalization of keywords don't matter so force lower
        # the split argument is a nonplussed face (>'_')>[giff methods plz]
        keyword = name.lower().split('_')
        keyword_prefix = keyword[0]
        if keyword_prefix in dynamic_keywords:
            if keyword_prefix in task_keywords:
                command_action = 'udstask'
            elif keyword_prefix in info_keywords:
            ...
            def method(raise_for_error=True, **update_cmds):
                r = self.cmd(command, **update_cmds)
