...
class H3TextClassifier(H3BaseActor):
   ...
   """
    ...
    def __init__(self,
        ...
        super(H3TextClassifier, self).__init__(**kwargs)
    ...
    def visualize(self, savepath=None, show=True):
        '''
        ...
        for p in self.preprocessor:
            if p._FEATURE_SIZE:
                if p._FEATURE_SIZE != len(p._FEATURE_NAMES):
                    ...
                    ticks.extend(
                        range(feature_idx, feature_idx + p._FEATURE_SIZE))
...
if __name__ == '__main__':
    ...
    from h3mlcore.utils.H3Plot import DataViz
    ...
    # Xtr = mydata.data
    # ytr = mydata.target
    Xtr, ytr = make_classification(
        n_samples=1000, n_classes=3, n_features=10, n_informative=4, n_clusters_per_class=2)
    clf = H3TextClassifier()
    # split the dataset as train/test
    X_train, X_test, y_train, y_test = train_test_split(
        Xtr, ytr, test_size=0.2, random_state=30)
    ...
    dv = DataViz(config)
    f1 = dv.feature_scatter1d(
        Xtr, names=[str(d) for d in np.arange(Xtr.shape[1])])
