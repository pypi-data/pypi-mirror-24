...
from amino import Boolean, Task, List, Try, __, Lists, _, Path, L
...
def location_info(file: Path, func: str, stack: List[FrameInfo]) -> List[str]:
    ...
    return stack.find(failure_location) / format_location | List()
...
class FailedUnsafeSpec(Expectation):
    ...
    @property
    def evaluate(self) -> Task[ExpectationResult]:
        return Task.now(FailedUnsafeSpecResult(self.line, self.error))
