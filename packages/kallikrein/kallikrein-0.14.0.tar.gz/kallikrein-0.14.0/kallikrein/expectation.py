...
class Expectation(Generic[A], abc.ABC):
    ...
    def fatal_eval(self) -> None:
        ...
        if not result.exists(_.success):
            raise ExpectationFailed(result.map(_.report_lines).value_or(_.lines))
...
__all__ = ('Expectation', 'SingleExpectation', 'UnsafeExpectation', 'unsafe_expectation_result', 'FatalSpec',
           'FailedUnsafeSpec', 'pending', 'SingleStrictExpectation')
