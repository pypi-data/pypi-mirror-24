...
class TVNext(with_metaclass(ABCMeta, Loadable, StartStopable)):
    ...
    def show_update(self, key, force_check=False, auto_save=False):
        ...
        :type auto_save: bool
        :return: Was this show updated (Successfully loaded)
        :rtype: bool
        """
    ...
    def show_update_all(self, force_check=False, auto_save=False):
        ...
        :type auto_save: bool
        :return: Was any show updated (Successfully loaded)
        :rtype: bool
        """
        shows = self._shows
        updated = False
        for key in sorted(shows):
            updated = updated or self.show_update(key, force_check, auto_save)
        if not auto_save and updated:
            self.show_save_all()
        return updated
    ...
    def check(self, key, force_check=False, delta_min=None, delta_max=None,
        ...
        with self._lock_update:
            ...
            # More likely that recent episode selected -> backwards
            for ep in episodes[::-1]:
                d = ep['aired']
                if not d:
