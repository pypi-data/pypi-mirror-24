...
class Bin():
    ...
    def groopm2(self, bin_file_out):
        ...
        if len(self.bam_files) == 0:
            self.logger.warning('GroopM2 requires BAM files as input. Skipping method.')
            return
        
        self.logger.info("Running 'groopm2 parse'.")
        gm_dir = os.path.join(self.output_dir, 'groopm2')
        make_sure_path_exists(gm_dir)
        output_db = os.path.join(gm_dir, 'groopm.db')
        cmd = 'groopm2 parse -t %d -c %d %s %s %s' % (self.cpus,
                                                        self.min_contig_len,
                                                        output_db, 
                                                        self.assembly_file, 
                                                        ' '.join(self.bam_files))
        ...
        cmd = 'groopm2 core %s -c %d --save_dists' % (output_db,
                                                        self.min_contig_len)
        success, exception = run(cmd)
        if not success:
            self.logger.warning('Failed to execute: %s' % ' '.join(exception.cmd))
        ...
        bin_prefix = os.path.join(gm_dir, 'gm2')
    ...
    def binsanity(self, bin_file_out):
        ...
        bs_dir = os.path.join(self.output_dir, 'binsanity')
        make_sure_path_exists(bs_dir)
        bs_cov_file = self._create_binsanity_cov_file(self.cov_file, bs_dir)
        
        self.logger.info("Running 'Binsanity-wf'.")
        assembly_dir, assembly_filename = ntpath.split(self.assembly_file)
        cmd = 'Binsanity-wf --threads %d -x %d -l %s -c %s -o %s' % (self.cpus,
                                                                        self.min_contig_len,
                                                                        assembly_filename,
                                                                        bs_cov_file,
                                                                        bs_dir)
        ...
        os.system('mv %s %s' % (os.path.join(bs_dir, 'BinSanity-Final-bins', '*'), bs_dir))
