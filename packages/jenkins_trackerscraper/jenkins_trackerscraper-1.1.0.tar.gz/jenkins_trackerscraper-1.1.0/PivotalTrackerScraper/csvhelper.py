...
def writeTestData (ROOT_DIR, test_data):
...
def writeTopFiles (ROOT_DIR, top_files):
...
def trackTopFiles (ROOT_DIR, project_name, top_files_counter):
    #Jenkins Plot Plugin has a bug where the left half of the data is not displayed if there is only a single build displayed
    #Therefore, placeholder values (0) are added to force all the relevant data to the right half of the data
...
def writeStories (ROOT_DIR, stories):
